#!/usr/bin/env python3
"""Verify scheduler_dirs are scanned before config scheduler injection."""
...
def main() -> int:
    ...
    scheduler_dir = pathlib.Path(args.scheduler_dir).resolve()
    ...
    config = workspace / "scheduler_autoscan.yaml"
    config.write_text(
        "\n".join(
            [
                ...
                "scheduler_hp_type: cpu_work_stealing_example",
                "scheduler_rt_type: serial_debug_example",
    ...
    expected = {
        "hp_type": "cpu_work_stealing_example",
        "rt_type": "serial_debug_example",
        "source": "scheduler_dirs scanned before graph load",
    }
    ...
    actual = {
        **actual_types,
        ...
        "scheduler_dir": str(scheduler_dir.relative_to(repo)) if scheduler_dir.is_relative_to(repo) else str(scheduler_dir),
    }
