...
TEST(M33CpuWorkStealingScheduler, SubmitTaskFromWorker) {
  ...
  tasks.push_back([&scheduler, &counter, &nested_executed]() {
    ...
    scheduler.submit_ready_task_from_worker([&counter, &nested_executed, &scheduler]() {
      counter.fetch_add(1);
...
TEST_F(M33SchedulerIntegrationTest, SubmitComputeViaRuntime) {
  GraphRuntime::Info info{
    "m33_test",
    "sessions/m33_test_session",
    "",
  ...
  // Try to submit compute (will fail without proper graph setup,
  // but tests the routing)
  ComputeOptions opts;
  ...
  // This will throw because no graph is loaded, but it proves routing works
