...
// 为测试添加一个虚拟的操作函数
ps::NodeOutput dummy_op(const ps::Node&,
                        const std::vector<const ps::NodeOutput*>&) {
...
void test_plugin_loading() {
  ...
  print_test_status(
      "Custom CPU plugin op 'invert_op_custom_example' is registered",
      custom_op_found);
  ...
  if (custom_op_found) {
    ...
    is_from_plugin =
        (source.find("invert_op_custom_example") != std::string::npos);
  }
  print_test_status(
      "Registered custom op correctly attributes its source to the plugin file",
      is_from_plugin);
}

void test_device_preference_metadata() {
  std::cout << "\n--- Running Test: Device Preference Metadata ---"
            << std::endl;
...
void test_metal_op_registration_and_context() {
#ifdef __APPLE__
  std::cout << "\n--- Running Test: Metal GPU Op Registration & Context (Apple "
               "Only) ---"
            << std::endl;
  ...
  if (!metal_op_found) {
    // Fallback: consult global registry directly (defensive against op->source
    // bookkeeping)
    auto keys = ps::OpRegistry::instance().get_keys();
    for (const auto& k : keys) {
      if (k == std::string("image_generator:perlin_noise_metal")) {
        metal_op_found = true;
        break;
      }
    }
  ...
  print_test_status(
      "Metal GPU op 'perlin_noise_metal' is registered via plugin",
      metal_op_found);
  ...
  if (metal_op_found) {
    ...
    if (context_valid) {
      print_test_status("GraphRuntime provides a valid Metal device context",
                        true);
    } else {
      std::cout << "[ SKIP ] Metal device not available in this environment"
                << std::endl;
    }
    // No explicit unload to avoid sanitizer/teardown races; main() exits
    // without running globals.
  }
#else
  std::cout << "\n--- Skipping Test: Metal GPU Op Registration & Context (Not "
               "on Apple platform) ---"
            << std::endl;
#endif
}
...
int main() {
  try {
    ...
    std::cerr << "Tests failed with an unhandled exception: " << e.what()
              << std::endl;
    return 1;
  }
  ...
  std::cout << "\n✅ All milestone 3 tests completed successfully!"
            << std::endl;
  // Avoid global destructor order issues under sanitizers by exiting
  // immediately
  _Exit(0);
}
