...
namespace {
ps_operation_port_constraint_v5 port = {
    sizeof(ps_operation_port_constraint_v5), PS_OPERATION_PORT_VALUE_V5, 0,
    0};  // NOLINT(whitespace/indent_namespace)
alignas(ps_operation_port_constraint_v5) unsigned char misaligned[32]{};
int unreachable(void*, const ps_operation_value_view_v5*, std::uint32_t,
                const ps_operation_parameter_value_v5*, std::uint32_t,
                std::uint32_t, ps_operation_cancelled_v5, void*,
                const ps_operation_output_sink_v5*, char*, std::size_t) {
  return PS_OPERATION_RESULT_FAILURE_V5;
}
void destroy(const ps_operation_descriptor_v5*, std::uint32_t) {}
ps_operation_descriptor_v5 descriptors[2]{};
ps_operation_plugin_api_v5 api{};
}  // namespace
...
ps_operation_plugin_get_api_v5(void) {
  descriptors[0] = {sizeof(ps_operation_descriptor_v5),
                    "valid.prefix",
                    12,
                    0,
                    ...
                    {sizeof(ps_operation_port_constraint_v5),
                     PS_OPERATION_PORT_VALUE_V5, 0, 0},
                    unreachable,
                    nullptr};
  ...
  descriptors[1].input_schema =
      reinterpret_cast<const ps_operation_port_constraint_v5*>(misaligned + 1);
  ...
  port.kind = PS_OPERATION_PORT_FLOAT32_SCALAR_V5;
  port.maximum_bits = 0x7f800000U;
#elif PS_BAD_PORT_CASE == 8
  descriptors[1].output_schema.struct_size -= 1;
#endif
  api = {sizeof(ps_operation_plugin_api_v5), 2, descriptors, destroy};
  return &api;
}
