...
const ps_operation_port_constraint_v4 value_port = {
    sizeof(ps_operation_port_constraint_v4), PS_OPERATION_PORT_VALUE_V4, 0U,
 ...
 * @return One closed `ps_operation_result_v4` value.
 * @throws Nothing.
 * @note No pointer is retained after return.
 */
int execute_double(void* user_data, const ps_operation_value_view_v4* inputs,
  ...
  if (!inputs || input_count != 1U || !parameters || parameter_count != 1U ||
      parameters[0].struct_size != sizeof(ps_operation_parameter_value_v4) ||
      parameters[0].key_size != 5U ||
      std::memcmp(parameters[0].key, "scale", 5U) != 0 ||
      parameters[0].type != PS_OPERATION_PARAMETER_FLOAT64_V4 ||
      backend != 1U || !sink || !sink->publish ||
      inputs[0].element_type != PS_OPERATION_ELEMENT_FLOAT64_V4 ||
      ...
      inputs[0].facets[0].struct_size != sizeof(ps_operation_facet_view_v4) ||
  ...
  return sink->publish(sink->context, PS_OPERATION_ELEMENT_FLOAT64_V4, shape,
 ...
 * @param diagnostic Writable failure diagnostic.
 * @param diagnostic_capacity Diagnostic buffer capacity.
 * @return `PS_OPERATION_RESULT_SUCCESS_V4` after the sink rejects the malformed
 ...
 */
int execute_bad_facet(void* user_data, const ps_operation_value_view_v4* inputs,
                      std::uint32_t input_count,
                      const ps_operation_parameter_value_v4* parameters,
                      std::uint32_t parameter_count, std::uint32_t backend,
                      ps_operation_cancelled_v4 cancelled,
  ...
  const ps_operation_facet_view_v4 facet = {
      sizeof(ps_operation_facet_view_v4),
      key,
 ...
 */
int publish_input(const ps_operation_output_sink_v4* sink,
                  const ps_operation_value_view_v4& input) noexcept {
 ...
 */
int publish_bad_facet(const ps_operation_output_sink_v4* sink,
                      const ps_operation_value_view_v4& input) noexcept {
  const char key[] = "bad.facet";
  const ps_operation_facet_view_v4 facet = {
      sizeof(ps_operation_facet_view_v4),
 ...
 * @param diagnostic Writable failure diagnostic.
 * @param diagnostic_capacity Diagnostic buffer capacity.
 * @return One closed `ps_operation_result_v4` value, or intentional `99` for
...
int execute_callback_result(
  ...
  if (state->mode >= kDuplicateValidThenSuccess) {
    ...
    if (state->mode == kDuplicateThenUnavailable) {
      ...
      return PS_OPERATION_RESULT_BACKEND_UNAVAILABLE_V4;
    }
  ...
  return publish_input(sink, inputs[0]) ? PS_OPERATION_RESULT_SUCCESS_V4
                                        : PS_OPERATION_RESULT_FAILURE_V4;
}
 ...
 */
void destroy_fixture(const ps_operation_descriptor_v4* operations,
 ...
 * @throws Nothing.
 */
ps_operation_parameter_descriptor_v4 make_double_parameter() noexcept {
  return {sizeof(ps_operation_parameter_descriptor_v4), "scale", 5U,
          PS_OPERATION_PARAMETER_FLOAT64_V4, 1U};
}
 ...
 */
ps_operation_descriptor_v4 make_result_descriptor(
    const char* key, std::uint32_t key_size,
    CallbackResultState* state) noexcept {
  return {sizeof(ps_operation_descriptor_v4),
          key,
          key_size,
          1U,
 ...
 */
std::array<ps_operation_descriptor_v4, kFixtureOperationCount>
