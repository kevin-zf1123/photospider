 ...
 * @return One closed `ps_operation_result_v8` value.
 * @throws Nothing.
 * @note No pointer is retained after return.
 */
int execute_double(void* user_data, const ps_operation_value_view_v8* inputs,
  ...
  if (!inputs || input_count != 1U || !parameters || parameter_count != 1U ||
      parameters[0].struct_size != sizeof(ps_operation_parameter_value_v8) ||
      parameters[0].key_size != 5U ||
      std::memcmp(parameters[0].key, "scale", 5U) != 0 ||
      parameters[0].type != PS_OPERATION_PARAMETER_FLOAT64_V8 ||
      backend != 1U || !sink || !sink->publish ||
      inputs[0].element_type != PS_OPERATION_ELEMENT_FLOAT64_V8 ||
      ...
      inputs[0].facets[0].struct_size != sizeof(ps_operation_facet_view_v8) ||
  ...
  return sink->publish(sink->context, PS_OPERATION_ELEMENT_FLOAT64_V8, shape,
 ...
 * @param diagnostic Writable failure diagnostic.
 * @param diagnostic_capacity Diagnostic buffer capacity.
 * @return `PS_OPERATION_RESULT_SUCCESS_V8` after the sink rejects the malformed
 ...
 */
int execute_bad_facet(void* user_data, const ps_operation_value_view_v8* inputs,
                      std::uint32_t input_count,
                      const ps_operation_parameter_value_v8* parameters,
                      std::uint32_t parameter_count, std::uint32_t backend,
                      ps_operation_cancelled_v8 cancelled,
  ...
  const ps_operation_facet_view_v8 facet = {
      sizeof(ps_operation_facet_view_v8),
      key,
 ...
 */
int publish_input(const ps_operation_output_sink_v8* sink,
                  const ps_operation_value_view_v8& input) noexcept {
 ...
 */
int publish_bad_facet(const ps_operation_output_sink_v8* sink,
                      const ps_operation_value_view_v8& input) noexcept {
  const char key[] = "bad.facet";
  const ps_operation_facet_view_v8 facet = {
      sizeof(ps_operation_facet_view_v8),
 ...
 * @param diagnostic Writable failure diagnostic.
 * @param diagnostic_capacity Diagnostic buffer capacity.
 * @return One closed `ps_operation_result_v8` value, or intentional `99` for
...
int execute_callback_result(
  ...
  return publish_input(sink, inputs[0]) ? PS_OPERATION_RESULT_SUCCESS_V8
                                        : PS_OPERATION_RESULT_FAILURE_V8;
}
 ...
 * @throws Nothing.
 */
ps_operation_parameter_descriptor_v8 make_double_parameter() noexcept {
  return {sizeof(ps_operation_parameter_descriptor_v8), "scale", 5U,
 ...
 */
ps_operation_descriptor_v8 make_result_descriptor(
 ...
 */
std::array<ps_operation_descriptor_v8, kFixtureOperationCount>
...
ps_operation_plugin_api_v8 make_api() noexcept {
  ...
  return {sizeof(ps_operation_plugin_api_v8), kFixtureOperationCount,
