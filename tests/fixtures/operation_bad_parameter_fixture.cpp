 ...
 */
int execute_never(void* user_data, const ps_operation_value_view_v4* inputs,
 ...
 * @throws Nothing.
 */
ps_operation_parameter_descriptor_v4 make_parameter() noexcept {
  return {sizeof(ps_operation_parameter_descriptor_v4) - 1U, "value", 5U,
 ...
 */
ps_operation_parameter_descriptor_v4 make_parameter() noexcept {
  return {sizeof(ps_operation_parameter_descriptor_v4), "x", 1025U,
          PS_OPERATION_PARAMETER_FLOAT64_V4, 1U};
}
#else
/**
 ...
 */
ps_operation_parameter_descriptor_v4 make_parameter() noexcept {
  return {sizeof(ps_operation_parameter_descriptor_v4), "value", 5U,
          PS_OPERATION_PARAMETER_FLOAT64_V4, 1U};
}
#endif

/** @brief Case-specific parameter record. */
const ps_operation_parameter_descriptor_v4 parameter = make_parameter();

#if PS_BAD_PARAMETER_CASE == 5
/** @brief Exact byte count required for one deliberately misaligned record. */
constexpr auto kStorageSize = sizeof(ps_operation_parameter_descriptor_v4) + 1U;
 ...
 */
const ps_operation_parameter_descriptor_v4* parameter_pointer() noexcept {
#if PS_BAD_PARAMETER_CASE == 1
  return nullptr;
#elif PS_BAD_PARAMETER_CASE == 5
  return reinterpret_cast<const ps_operation_parameter_descriptor_v4*>(
 ...
 */
ps_operation_descriptor_v4 make_descriptor() noexcept {
...
const ps_operation_descriptor_v4 descriptor = make_descriptor();
...
ps_operation_plugin_api_v4 make_api() noexcept {
  return {sizeof(ps_operation_plugin_api_v4), 1U, &descriptor, destroy_fixture};
}
