...
int gaussian_workflow() {
  ...
  for (std::size_t i = 0; i < pixels.size(); i += 3)
    reference_values.push_back(pixels[i]);
  const auto reference = samples({2, 3}, reference_values);
  for (const std::string boundary : {"zero", "clamp"}) {
    WorkflowDocument doc;
    doc.inputs = {declaration(1, "image", input),
                  declaration(2, "reference", reference)};
    doc.nodes = {{1,
    ...
    for (bool joint : {false, true}) {
      ...
      auto frozen = execution
                        .freeze(compiled.value().plan,
                                {{{"image", input}, {"reference", reference}}})
                        .take_value();
      ExecutionOptions options;
      options.enable_joint = joint;
    ...
    ExecutionContext execution(registry);
    auto kernel = execution.execute(
        kernel_plan, {{{"image", {}, source}, {"reference", reference}}});
    if (!kernel.ok())
