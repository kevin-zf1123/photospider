...
namespace {
/** @brief Publishes a legal native image whose Float32 samples are unaligned.
 */
ps::Result<ps::Value> padded_image(const ps::OperationInvocation& call) {
  ...
  auto allocated = call.allocator.allocate(17);
  if (!allocated.ok())
  ...
  if (api->buffer(api->context, call.inputs[0].bytes().data(), 16, 0,
                  &input_token) ||
      api->buffer(api->context, output.data(), 17, 1, &output_token))
  ...
  const char shader[] =
      ...
      "device uchar* b [[buffer(1)]], uint i [[thread_position_in_grid]])"
      "{b[i+1]=a[i];}";
  ps_gpu_buffer_binding_v6 buffers[] = {
      {sizeof(ps_gpu_buffer_binding_v6), 0, input_token, 0, 16, 0},
      {sizeof(ps_gpu_buffer_binding_v6), 1, output_token, 0, 17, 1}};
  ...
  return ps::Value::from_storage(
      call.inputs[0].descriptor(), call.output_region, {1, {16, 16, 4}},
      std::move(output).freeze(), call.inputs[0].facets());
}
/** @brief Native predecessors may expose unaligned bytes; opacity must fall
 * back.
 */
void check_native_alignment(
    const std::shared_ptr<ps::OperationRegistry>& base) {
  ...
  s4_fixture::require(
      registry->register_operation({"test.pad", traits, padded_image}).ok(),
      "pad registration");
