...
int get_tile_size(const ps::Node& node) {
    auto meta_opt = ps::OpRegistry::instance().get_metadata(node.type, node.subtype);
    if (meta_opt) {
        if (meta_opt->tile_preference == ps::TileSizePreference::MICRO) return TILE_SIZE_MICRO;
        if (meta_opt->tile_preference == ps::TileSizePreference::MACRO) return TILE_SIZE_MACRO;
    }
...
void handle_tiles(ps::InteractionService& svc, const std::string& graph_name, const std::string& target_node_str) {
    std::vector<int> node_ids_to_query;
    ps::GraphModel& model = svc.kernel().runtime(graph_name).model();

    if (target_node_str == "all") {
        for (const auto& pair : model.nodes) node_ids_to_query.push_back(pair.first);
        std::sort(node_ids_to_query.begin(), node_ids_to_query.end());
    } else {
        try {
            int id = std::stoi(target_node_str);
            if (model.nodes.count(id)) node_ids_to_query.push_back(id);
            else { std::cerr << "Error: Node " << id << " not found." << std::endl; return; }
        } catch (...) { std::cerr << "Error: Invalid node id." << std::endl; return; }
    }
    ...
    if (ending_nodes && !ending_nodes->empty()) {
        for(int end_node_id : *ending_nodes) {
             auto fut_opt = svc.cmd_compute_async(graph_name, end_node_id, "int8", false, false, true);
             if (fut_opt) fut_opt->get();
        }
    }
    
    for (int id : node_ids_to_query) {
        const auto& node = model.nodes.at(id);
        std::string pref_str = "UNDEFINED";
        int width = 0, height = 0;

        // 尝试从缓存中获取尺寸
        if (node.cached_output && node.cached_output->image_buffer.width > 0) {
            width = node.cached_output->image_buffer.width;
            height = node.cached_output->image_buffer.height;
        } else {
            std::cout << id << " UNKNOWN (not computed or no image output) - - - -" << std::endl;
            continue;
        }
        
        auto meta_opt = ps::OpRegistry::instance().get_metadata(node.type, node.subtype);
        if (meta_opt) {
            if (meta_opt->tile_preference == ps::TileSizePreference::MICRO) pref_str = "MICRO";
            if (meta_opt->tile_preference == ps::TileSizePreference::MACRO) pref_str = "MACRO";
        }
        ...
        std::cout << id << " " << pref_str << " " << h_tiles << " " << v_tiles << " " << width << " " << height << std::endl;
    }
}


void handle_dirty(ps::InteractionService& svc, const std::string& graph_name, int start_node_id, const std::string& tile_coords_str) {
    ps::GraphModel& model = svc.kernel().runtime(graph_name).model();
    if (!model.nodes.count(start_node_id)) {
        std::cerr << "Error: Start node " << start_node_id << " not found." << std::endl;
        return;
    }
    ...
    if (iss.fail() || comma != ',') {
        std::cerr << "Error: Invalid tile format. Use x,y (e.g., 3,1)." << std::endl;
        return;
    }

    const auto& start_node = model.nodes.at(start_node_id);
    ...
    auto projected_roi_opt =
        svc.cmd_project_roi(graph_name, start_node_id, initial_roi, target_end_node);
    ...
    std::map<int, std::vector<ps::GraphEventService::ComputeEvent>> events_by_node;
    for(const auto& event : *events) {
        if (event.source == "rt_update" || event.source == "hp_update" || event.source == "computed") {
    ...
    for (const auto& pair : events_by_node) {
        std::cout << "Node " << pair.first << " (" << pair.second[0].name << ") was recomputed." << std::endl;
    }
}
...
int main(int argc, char** argv) {
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (command == "exit") break;
            ...
            int node_id;
            std::string tile_str;
            if (!(iss >> node_id >> tile_str)) std::cerr << "Usage: dirty <id> <x,y>" << std::endl;
            else handle_dirty(svc, graph_name, node_id, tile_str);
        } else if (command == "tiles") {
            std::string target;
            if (!(iss >> target)) std::cerr << "Usage: tiles all|<id>" << std::endl;
            else handle_tiles(svc, graph_name, target);
        } else if (!command.empty()) std::cerr << "Unknown command." << std::endl;
