...
#endif
static_assert(
    ps::plugin::element_type_value(PS_OPERATION_ELEMENT_FLOAT64_V5) ==
        PS_OPERATION_ELEMENT_FLOAT64_V5,
    "the installed C++ operation wrapper must preserve ABI enum values");
static_assert(
    noexcept(ps::plugin::element_type_value(PS_OPERATION_ELEMENT_FLOAT64_V5)),
...
int main() {
  if (photospider_sdk_version_sum() !=
      PS_OPERATION_ABI_VERSION_5 + PS_DATA_PROVIDER_ABI_VERSION_1) {
    return 4;
  }
