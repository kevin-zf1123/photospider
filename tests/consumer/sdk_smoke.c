...
_Static_assert(PS_OPERATION_RESULT_SUCCESS_V8 == 0,
               "operation callback success must remain zero");
_Static_assert(PS_OPERATION_RESULT_FAILURE_V8 !=
                   PS_OPERATION_RESULT_BACKEND_UNAVAILABLE_V8,
...
unsigned int photospider_sdk_version_sum(void) {
  return PS_OPERATION_ABI_VERSION_8 + PS_DATA_PROVIDER_ABI_VERSION_1;
}
