...
_Static_assert(PS_OPERATION_RESULT_SUCCESS_V9 == 0,
               "operation callback success must remain zero");
_Static_assert(PS_OPERATION_RESULT_FAILURE_V9 !=
                   PS_OPERATION_RESULT_BACKEND_UNAVAILABLE_V9,
...
unsigned int photospider_sdk_version_sum(void) {
  return PS_OPERATION_ABI_VERSION_9 + PS_DATA_PROVIDER_ABI_VERSION_1;
}
