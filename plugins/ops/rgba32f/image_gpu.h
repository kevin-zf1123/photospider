...
static float ps_gpu_image_sample(const ps_operation_value_view_v6* v,
...
static void ps_gpu_image_geometry(ps_image_gpu_parameters* p, uint32_t base,
                                  const ps_operation_value_view_v6* v) {
...
static int ps_execute_gpu_image(
    uint32_t kind, const ps_operation_value_view_v6* inputs, uint32_t count,
  ...
  const ps_gpu_service_v6* api = sink->gpu;
  ...
  for (uint32_t i = 0; i < parameter_count; ++i) {
    const ps_operation_parameter_value_v6* v = &parameters[i];
  ...
  ps_gpu_buffer_binding_v6 buffers[5];
