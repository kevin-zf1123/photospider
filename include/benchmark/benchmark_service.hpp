...
public:
    ...
    BenchmarkResult Run(const std::string& benchmark_dir, const BenchmarkSessionConfig& config, int runs = 10);
     ...
     * @return 解析后的基准测试配置列表。
     */
    std::vector<BenchmarkSessionConfig> LoadConfigs(const std::string& benchmark_dir);
...
private:
    ...
    std::vector<BenchmarkSessionConfig> load_configs_internal(const std::string& benchmark_dir);
};
