...
// =============================================================================
// TaskGroup: 支持同一 compute domain 内的 Micro Tile 聚合为 Macro Task。
// 用于优化批处理：将多个小 tile 聚合为一个大任务以减少调度或 kernel launch
// 开销。
// =============================================================================
struct TaskGroup {
  // 组 ID
  ...
  // 原始的 Micro tile 列表，尺寸由所属 compute domain 决定。
  std::vector<cv::Rect> micro_tiles;

  // 聚合后的边界框，用于同一 compute domain 内的 Macro 处理。
  ...
  void add_tile(const cv::Rect& tile) {
    if (micro_tiles.empty()) {
      ...
      int x1 =
          std::max(bounding_box.x + bounding_box.width, tile.x + tile.width);
      int y1 =
          std::max(bounding_box.y + bounding_box.height, tile.y + tile.height);
...
class IScheduler {
 public:
  ...
  /// @return TaskGroup 实例
  virtual TaskGroup create_task_group(int node_id,
                                      const std::vector<cv::Rect>& dirty_tiles,
                                      ComputeIntent intent, uint64_t epoch) {
    TaskGroup group;
  ...
  /// @return true 如果应该在同一 compute domain 内聚合
