...
`Dockerfile.ci` defines the GitHub Linux test environment. `healthcheck-published-image` is a container job, and the published-image healthcheck execution and build/test integration jobs run inside `ghcr.io/<owner>/<repo>/photospider-ci:latest`. Its `Fetch pull request base history` and `Fetch CI branch main history` steps explicitly set `shell: bash`, so their `set -Eeuo pipefail` prologues run under Bash instead of relying on the container default shell. Protected-path, change-classification, and stable result-gate jobs remain lightweight `ubuntu-latest` jobs and do not configure or compile the project.
