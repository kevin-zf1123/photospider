...
When a pull request or push changes the CI image inputs (`Dockerfile.ci`, `.dockerignore`, or `.github/workflows/build-ci-image.yml`), the healthcheck and integration workflows build `photospider-ci:local` inside the workflow and run the same scripts there. Pull-request image detection fetches the base repository branch, verifies the event's exact base SHA, and compares from that base rather than relying on a possibly absent fork `origin/<base>`. The published-image and local-image healthcheck jobs each independently bind `CI_BASE_SHA` to the event base, fetch the target branch from the base-repository URL, and verify that exact commit inside their own job. Published-image verification completes before `healthcheck.sh`; local-image verification completes before the head Dockerfile is built or the mounted workspace is executed. A fetch or exact-commit parse failure stops the job before the script can use its fallback base selection. Both healthcheck paths pass the exact event SHA as `CI_BASE_REF`, so the local container can resolve it even when the fork checkout's `origin` lacks the base tip. A `CI/**` push fetches `origin/main` and detects image inputs cumulatively from the branch merge base, so a later documentation-only push cannot hide an earlier image-input commit.
...
- `ci/scripts/ci_routing_test.sh`: whitespace-normalizes and exact-locks both production `protected-ci-paths.if` expressions, then extracts and executes the real stable-gate, pre-checkout fork-rejection, and protected-path shell blocks. Isolated Git fixtures prove that both production guards reject a newline-containing `ci/**` path, safely record it, and fail closed on producer or reader failure. Job-scoped assertions separately lock the published-image and local-image exact-base fetch, refspec, verification, `CI_BASE_REF`, and execution order; detector fixtures retain exact/cumulative bases, empty comparisons, newline paths, and changed-path failure propagation. The source assertion deliberately does not claim to execute GitHub's expression evaluator.
