...
validated path sets. The only missing-base exception is a real first
introduction whose head adds a strict self-including regular lock blob at the
exact diff path; it always routes `changed=true`. Lock additions, removals,
duplicates, traversal, malformed JSON, a missing/malformed head, or an unproved
bootstrap therefore cannot emit `changed=false`. A fork head is rejected before
...
the exact archived control graph. It accepts only bytewise canonical explicit
`.cmake` include paths (never module-mode lookup), bytewise canonical maintained
subdirectories,
test/property commands, one inert `<target>_TESTS` assignment that exactly
repeats the tests registered in that file, and the GoogleTest
NOT_BUILT-wrapper shape. Explicitly quoted empty command arguments remain data;
unknown `set` state cannot alter parsing or module search. Alternate
`PARENT_SCOPE` and `CACHE ... [FORCE]` signatures are rejected before local-list
equality, and raw `./`, repeated-separator, dot-component, or trailing-separator
spellings are never normalized into an accepted control member. Every argument
retains its quoted/unquoted/bracket kind and raw spelling. Include/subdirectory
paths reject any lexical backslash before decoding; an inert `set` rejects every
unquoted semicolon spelling, including an escaped semicolon, before equality.
Quoted or bracket semicolon values remain one safe lexical value, and ordinary
command/filter escapes retain their reviewed decoding. The verifier then
requires equality among raw complete records, both targeted
...
separate read-only mounts; the only writable bind is a mode-0700 fresh direct
`runner.temp` child. A no-follow directory descriptor measures and retains that
child's exact device, inode, mode, UID, and GID; the wrapper remeasures the same
identity immediately before Docker and passes its positive owner UID/GID through
`--user`. This intentionally follows the directory's actual GID rather than the
host process's primary group when a setgid parent assigns them differently.
Before candidate configure/build/test, the protected container
entrypoint verifies that its effective UID/GID is identical and completes a
real create/read/remove probe in `/work`. It never uses `0777`. The container
has no network or registry token. This
...
and its vcpkg commit. Platform preparation verifies the executable, parses the
semantic version from one canonical `cmake version` line (allowing only the
reviewed Android SDK `-g<7 lowercase hex>` vendor suffix), and separately binds
`CMAKE_ROOT` plus the module bytes before vcpkg or configure. Wrong semantic
versions, foreign suffixes, control bytes, or extra lines fail. CMake 3.31.5
deliberately emits the
...
- `ci/scripts/ci_image_changed.sh`: delegates the exact base/head comparison to the canonical manifest helper, which strictly validates required revisions of the self-including `input_paths` lock, compares their union with the NUL-delimited unfiltered diff, routes a proved no-lock-base/strict-head first introduction `true`, and exits without a route output on malformed head, unproved bootstrap, later missing lock, or Git failure.
...
- `ci/scripts/ctest_runtime_closure.py`: derives the recursive post-build ordinary CTest control/runtime closure, exposes strict raw-record parsing, and reconstructs the exact archived control graph through an allowlisted pure-data parser before attestation. Includes and subdirectories must use their bytewise canonical explicit POSIX spelling with no lexical backslash before decoding; includes must name `.cmake` paths rather than module lookup. The only accepted `set` is one inert local `<target>_TESTS` list equal to that file's registrations, with parent-scope/cache signatures and every unquoted semicolon spelling rejected before equality while quoted/bracket semicolon values remain single lexical values. Structured root normalization is component-aware; unknown/side-effect syntax, prefix siblings, embedded or quoted/list ambiguity, and physical relocation fail. The post-attestation runtime path separately revalidates restored inventory, executables, dynamic libraries, plugins, trust inputs, and build-tree data immediately before execution.
...
- `ci/scripts/linux_security_profile.sh` and `ci/scripts/linux_security_container_entrypoint.sh`: run Linux ASan, TSan, fuzz, and manual sanitizer profiles from a real-host retained identity. The host wrapper authenticates and pulls only the exact digest, removes the token, and starts the profile with no network, a read-only root filesystem, separate read-only candidate/control/inventory/identity mounts, one mode-0700 fresh `runner.temp` work root, and Docker `--user` bound to that root owner's positive numeric UID/GID. It obtains that owner from a no-follow descriptor measurement of the actual directory, retains device/inode/mode/owner, and remeasures immediately before Docker. The protected entrypoint verifies the effective identity and a real create/read/remove work probe before candidate execution.
