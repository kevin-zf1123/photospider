...
command/provider/context audit 会检查该下限的 command 与 policy inventory，当前 integration
则在受支持 CI toolchain 上执行 fresh static package consumer。只有 compatibility-sensitive change
或 release check 确有需要时，才补充针对性的真实旧版本 producer/install/consumer 运行；常规
integration workflow 不会通过专用最低版本 job 锁定 Ubuntu 或 CMake。
...
issue replay 与 evidence/provenance orchestration 都必须排除。后者应放在 `tests/results/...` 下的
issue 专属 personal-overlay 证据中，或作为明确记录的手工开发工具；clean primary checkout
不能 import 这些内容。
...
Issue #34 第 15 轮本地验证不使用这些 Docker 命令或 `linux/amd64` 模拟。实现迭代只运行受影响
target 与 focused test；最终源码冻结后，只执行一次本机原生 clean configure/full
build/CTest-JUnit，并让所有正式本地 gate 复用该结果。Current-head GitHub Actions 提供独立的
远程 integration 结果。
