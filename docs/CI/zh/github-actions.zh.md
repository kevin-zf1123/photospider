...
当 pull request 或 push 修改 CI 镜像输入（`Dockerfile.ci`、`.dockerignore` 或 `.github/workflows/build-ci-image.yml`）时，healthcheck 和 integration workflow 会在当前 workflow 内构建 `photospider-ci:local`，并在该镜像中运行相同脚本。Pull-request 镜像检测会拉取 base 仓库分支、校验 event 的精确 base SHA，再从该 base 开始比较，而不依赖 fork 中可能不存在的 `origin/<base>`。Published-image 与 local-image healthcheck job 都会在各自 job 内把 `CI_BASE_SHA` 绑定到 event base，从 base-repository URL 拉取目标分支，并校验该精确 commit。Published-image 的校验先于 `healthcheck.sh`；local-image 的校验先于构建 head Dockerfile 或执行挂载 workspace。Fetch 或精确 commit 解析失败时，job 会在脚本可能使用 fallback base 选择之前停止。两条 healthcheck 路径都把 event 的精确 SHA 作为 `CI_BASE_REF` 传入，因此即使 fork checkout 的 `origin` 不包含 base tip，本地容器也能解析它。`CI/**` push 会拉取 `origin/main`，从 branch merge base 开始累计检测镜像输入，因此后续纯文档 push 无法隐藏更早的镜像输入 commit。
...
- `ci/scripts/ci_routing_test.sh`：对两份生产 `protected-ci-paths.if` 表达式做空白归一化并锁定精确源码，再抽取并执行真实 stable-gate、checkout 前 fork-rejection 与 protected-path shell block。隔离 Git fixture 会证明两份生产门禁都拒绝含换行的 `ci/**` 路径、安全记录该路径，并在 producer 或 reader 失败时 fail closed。Job-scoped 断言分别锁定 published-image 与 local-image 的精确 base fetch、refspec、校验、`CI_BASE_REF` 和执行顺序；detector fixture 继续覆盖精确/累计 base、空比较、含换行路径及 changed-path 失败传播。该源码断言明确不声称执行了 GitHub expression evaluator。
