...
在 targeted artifact attestation 前，`verify-targeted-artifacts` 会把精确 candidate 再次 fresh checkout 到一份不执行代码的 data root。该 root 与精确 protected `workflow_commit` control checkout、raw inventory、control manifest 及 targeted artifact 互不重叠。只能执行 control checkout 中的 Python。Verifier 会把 control `HEAD` 绑定到 `workflow_commit`、把 candidate-data `HEAD` 绑定到 `candidate_commit`，并在使用后重新检查两个 directory object。随后，它把精确 raw bundle 重新绑定到 route-control digest。唯一严格的受保护纯数据 parser 会读取归档中的精确 control graph，且不调用 CTest、CMake 或 candidate-defined control program。它只接受显式 canonical `.cmake` include path（绝不使用 module-mode lookup）、维护中的 subdirectory、test/property command、一个与该文件内注册测试完全一致的惰性 `<target>_TESTS` assignment，以及 GoogleTest NOT_BUILT-wrapper 形状。显式引用的空 command argument 仍作为数据保留；未知 `set` state 不能改变解析或 module search。随后，它要求 raw 完整 record、两份 targeted control/runtime closure record 与 control-file digest，以及只排除精确 `build-smoke` 后的普通 closure 全部相等。Unknown/side-effect syntax、reduction、addition、relabeling、identity/digest mismatch、link、overlap 或 path/commit drift 会在 attestation 与 readiness 前失败。Root token 只能 normalize 完整结构化 path，且该 path 必须等于 source/build root 或 component descendant；prefix sibling、embedded/quoted/list ambiguity、物理 CTest relocation 与不受限 string replacement 都会被拒绝。
...
Linux 与 Darwin 都会把 ASan、TSan 和有界 fuzz 调度为不同的 profile result。每个共享 Linux profile 以及手动 sanitizer workflow 都会先 sparse-checkout 精确受保护 host control，并在 candidate checkout 前于真实 host 创建 retained `ubuntu-24.04` identity。随后它登录 GHCR、拉取精确 digest-qualified image、unset token，并使用 `linux_security_profile.sh` 只在该 image 内运行 candidate code。Candidate source、受保护 `ci`、profile inventory 与 retained identity 分别只读 mount；唯一可写 bind 是 mode-0700、fresh 且直属 `runner.temp` 的 child。Wrapper 通过 Docker `--user` 传入拥有该目录的精确正数 numeric UID/GID。Candidate configure/build/test 前，受保护 container entrypoint 会验证 effective UID/GID 完全一致，并在 `/work` 完成一次真实 create/read/remove probe；它不使用 `0777`。Container 没有 network 或 registry token。这样既不会把 job-container 中缺失的 `ImageOS` 解释为 hosted-runner provenance，也保持 ASan、TSan 与 fuzz 为 sibling-independent host job。
...
- `ci/scripts/ctest_runtime_closure.py`：派生递归的构建后普通 CTest control/runtime 闭包，为 raw record 提供严格解析，并在 attestation 前通过 allowlist 纯数据 parser 重建归档中的精确 control graph。Include 必须是显式 canonical `.cmake` path，不能使用 module lookup；唯一接受的 `set` 是与该文件注册测试相等的惰性 local `<target>_TESTS` list。结构化 root normalization 按 component 判断；unknown/side-effect syntax、prefix sibling、embedded 或 quoted/list ambiguity 及物理 relocation 都会失败。Post-attestation runtime path 会在执行前另行复核恢复后的 inventory、executable、dynamic library、plugin、trust input 与 build-tree data。
