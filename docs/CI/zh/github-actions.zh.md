...
- `.github/workflows/ci-integration.yml`：运行纯文档路由、动态 label-driven integration 规划、一个可复用 default build、独立分片的完整 CTest 与逐测试 build-smoke matrix job、`graph_cli` 脚本测试、propagation 脚本测试、插件加载测试、scheduler 重复测试，并由一个稳定的 `integration` 结果门禁汇总。
...
commit，并使用 `ctest --show-only=json-v1` 发现所有携带精确 `build-smoke` 标签的测试。Planner
会在输出矩阵前校验 `ctestInfo` 版本、完整测试名唯一性、property/label 形状、enabled 状态、
可执行 command、非空选择与矩阵大小。JSON malformed、测试或 property 重复、label 非法、
带标签测试 disabled/commandless，以及 build-smoke 集合为空，都会使规划失败。矩阵按不区分
大小写的测试名稳定排序；artifact key 由有界 ASCII slug 与测试名 SHA-256 digest 派生，精确测试名
则保留为 JSON matrix value。Workflow 不再维护测试名清单。
...
前五个 product driver 会创建或校验隔离的 nested build profile；OpenCV safety companion 校验其
nested-build driver 的破坏性清理与配置布局边界；public-header self-containment 会调用专用
compile target。它们属于长期 product、package、configuration、safety 与 compile 边界，不是
migration 或 source-layout 检查。
...
- `full-ctest` 使用 CTest label filter 排除每个精确 `build-smoke` 标签，不再维护按名称排除的
  build-smoke 清单。
- `build-smoke` 通过 `fail-fast: false` 消费 planner 输出的 JSON include matrix。每个 matrix
  entry 都会显示为独立的 `Build smoke (<精确 CTest 名>)` job，下载 `ci-build-default`，获得独立
...
default profile，读取同一个 planner 输出的 NUL 分隔精确名称清单，在 full CTest 中排除该标签，
...
- `ci/scripts/build_smoke_inventory.py`：严格解析 CTest JSON v1，输出确定性非空 matrix 与 NUL 分隔精确名称，并在基于索引执行前重新校验一个 matrix selection。Focused regression 会覆盖 malformed JSON/schema、重复名称/property/label、非法或缺失 label、disabled/commandless entry、空 selection、确定性排序、JSON round trip、安全 artifact key 与敌意测试名字符。
- `ci/scripts/integration_plan.sh`：配置一个启用测试的小型规划 build tree，并只根据精确 CTest label 输出 JSON matrix。
- `ci/scripts/integration_suite.sh`：消费 planner 输出的精确名称，为 local-image fallback 顺序运行所得 integration 分片。
- `ci/scripts/build_integrity.sh`：构建 default profile 的 required targets 与完整 tree，校验带标签的 CTest inventory，并为 build 加可复用 stamp。
...
`CI-results/integration-plan/build_smoke_matrix.json` 输出的任一精确名称；runner 会拒绝 absent、
