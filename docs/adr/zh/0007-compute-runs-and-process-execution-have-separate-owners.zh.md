...
作为目标架构已接受。Issue #68 的非 realtime HP 切片已经是当前软件行为：除了 issue #67
提供的 `ComputeRun` descriptor、state、storage、lease 与复合 task identity，embedded
composition root 现在还会显式注入一个 CPU `ExecutionService`。内建 CPU 的非 dirty
full-HP dispatch 会跨越 move-only `ReadyTaskSubmission` 边界，并由 service 按精确 planning
worker grant 同时最多执行一个完整 Run。`RunGroup`、dirty/realtime lease 覆盖、多 Graph
service 执行、`RunLifecycleRegistry`、权威 `GraphRevision` commit validation、目标
`ResourceLedger`、有界 ready storage，以及仅负责策略的新一代 scheduler 仍是目标行为。
...
当前实现仍让每个 `GraphRuntime` 分别拥有一个 HP 和一个 RT `IScheduler`。每个过渡期
scheduler 都拥有 worker、ready queue、epoch、completion counter、异常发布与排序策略。
私有 route metadata 现在会把 HP scheduler owner 与 legacy execution domain，或者 issue #68
的进程 CPU route 及其精确 worker grant 原子配对。内建 CPU full-HP 选择后者；serial、GPU、
plugin、dirty 与 realtime 工作保留前者。

显式注入的 service 拥有一个 concrete CPU scheduler，并串行化一个 Run 从完整 setup 到
settlement 的区间。其 host-context proxy 只会在该 Run 活跃时转发 worker/epoch trace state。
函数内静态对象 `SchedulerWorkerBudget::process()` 仍把所有 embedded Host 的过渡期
Graph-owned scheduler 保守计费限制为 32，但它既不计费也不拥有 service pool，也不拥有
ready-store、Run、内存、scratch、device、I/O 或 plugin process 预算。
...
临时结果、依赖状态与 runner。每个真实 ready task 都保留不可伪造 Run lease 与
`(RunId, RunLocalTaskId)`；failure publication 由匹配 identity 守卫。内建 CPU planning 会把
这些值具化为传入注入 service 的 move-only `ReadyTaskSubmission`。Legacy full-HP route 保留
owned callback 路径，并且只用空 borrowed-handle batch 建立 scheduler epoch。如果在没有自己的
稳定 lease 时就把剩余 dirty handle 移入进程级队列，仍会引入 use-after-free 风险。
...
当前 issue #68 的值实现了不可变 metadata、复合 identity、owned executable 与匹配
`ComputeRunLease` 这一子集。当前 service 会拒绝 borrowed handle、匿名 raw callback，以及
不属于 active Run 的任何 initial 或 dependent submission。`TaskSubmissionPlan` 仍负责发现
initial readiness、释放 dependency、拥有 result index 并减少 logical completion count。
多 Run admission、有界 ready storage、resource estimate/grant 与 policy re-entry 仍是目标行为。
...
singleton。测试会创建和销毁彼此隔离的 domain。当前 service 没有外部 process configuration
object 或 lifecycle registry；它会从可信内建 scheduler plan 的精确 worker grant 延迟配置
自己的 CPU pool。
...
本决策冻结依赖契约。Issue #66、#67 与 #68 已作为当前非 realtime HP 切片实现；后续切片仍按以下
目标顺序推进：
...
| #68（当前） | 显式注入的单 Run CPU-only `ExecutionService`，只接受 ready 输入 | 多 Graph 迁移与最终 ledger |
