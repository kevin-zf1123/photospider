...
冻结 topology closure、operation/device/callable、DSO lease 与完整 service root。一个
umbrella root 会在整个 connected closure 中把共享 Run control、prepared result 与预计缺失的
staging entry 恰好计费一次，同时计入自身私有 carrier 与 ledger reservation-state envelope；
每个 node root 只计入自身独有的 callback、ready-store 与 service-envelope 所有权。只有在
全部 node root 与最终 cancellation-slot 容量均已确定后，才会准备 umbrella。安装后，每个
prepared phase 会在进入 provider 前执行 reserved start，provider output 则在已安装 Run 内
驱动依赖 output 的 dirty plan。Preparation rollback、安装失败与 provider failure 都会恰好
一次释放全部 root；这些后续 failure 使用 terminal finalization，而不是 candidate rollback。
...
在 lifecycle transition 之前，close-owner selection 受 generation 限定。如果所选 owner 在
`Open -> Closing` 之前失败，coordinator 会把精确 exception 发布给所有已经加入该 generation
的 caller，并且在最后一个旧 joiner 消费它之前不准入 fresh owner。随后下一个 caller 会拥有
新的、不复用的 generation，因此 joiner 不会 hang，retry 也不会观察到 ABA-completed
generation。析构会重试这种普通的线性化前 failure。

Kernel 的 graph-name registry 是独立 ownership index，不是 admitted-Run registry。一个短
graph-registry 临界区会保留精确 shared `GraphRuntime` root，并执行非等待 close-generation
owner/joiner selection。如果旧 failed generation 仍有未消费的 joiner，selection 会返回显式 retry
token；caller 会在释放 graph-registry lock 后等待，然后同时重验 retained runtime owner 与
`GraphInstanceId`，再重新 selection。随后 lifecycle settlement 与 lane drainage 也都不持有该
lock。唯一允许的嵌套顺序是 graph registry 在非等待 close-coordinator access 或 lifecycle
registration 之前；不允许反向嵌套获取。

Kernel 的 best-effort `LastError` observation 存储在精确 `GraphRuntime` 中，而不是
process-global identity-keyed map 或可复用的 graph-name key。public inspection 会先保留
当前 runtime，再只复制其由 mutex 保护的 optional slot。从旧 runtime 保留下来的
calling-thread result translation 只能修改旧 slot；该 slot 随最后一个 shared runtime owner
销毁，既不能覆盖、也不能清除同名 replacement。
...
Kernel 会先在不发生 mutation 的前提下验证 shutdown caller 不是 same-service worker 或 policy
callback。被拒绝时，Kernel publication gate 保持 open，lifecycle telemetry 保持
`Accepting`，且 shutdown generation 为零。随后，它持有 graph-registry lock 的时间只够发布
对迟到 GraphRuntime registration 的单调拒绝；service transition 与预分配 cancellation
fanout 会在释放该 lock 后运行。lifecycle fence 会把已经构造的 load 与
`ServiceStopping` 排序：它要么先完成 publication 并进入 drainage，
要么输给已关闭 publication gate，且永不注册 lifecycle row。cancellation fanout 阻塞期间，
Graph listing 仍可用。gate 关闭是不可逆边界；由于 publication 无法安全重开，gate 关闭后的
意外 observer 或 transition failure 会 fail-stop。

1. 在同一 lifecycle-registry fence 下把 service 改为 `Stopping`，把每个 graph row 改为 `Closing`，
...
6. 把所有物理 executor thread 转移给一个已 armed 的本地 join guard；即使后续 shutdown
   bookkeeping 栈展开，该 guard 仍会 join 并计入全部剩余 worker；
