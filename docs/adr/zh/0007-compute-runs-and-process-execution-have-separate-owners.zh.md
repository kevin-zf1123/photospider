...
graph-registry 临界区会一起保留精确 shared `GraphRuntime` root，并选择其 close-generation
owner/joiner。随后 lifecycle settlement 与 lane drainage 都不持有该 lock。唯一允许的嵌套顺序是
graph registry 在 close coordinator 或 lifecycle registration 之前；不允许反向嵌套获取。
...
Kernel 会在 graph-registry lock 内开始 service shutdown，并发布对迟到 GraphRuntime registration
的单调拒绝。因此，并发 load 要么先完成 publication 并进入 drainage，要么永不注册 lifecycle row。
