...
作为目标架构已接受。Issue #69 已成为 CPU 执行切片的当前软件行为：embedded composition root
会注入一个固定的 `ExecutionService`；内建 CPU 的 HP、RT、connected-parameter preflight 和
dirty source/downstream 工作会跨越 move-only `ReadyTaskSubmission` 边界。来自多个 Graph 的
...
`GraphRuntime` 中不拥有 owner；serial、GPU 与 plugin route 仍保留过渡期 per-Graph
scheduler。`RunLifecycleRegistry`、权威 `GraphRevision` commit validation、目标
`ResourceLedger`、有界 ready storage、最终公平性 policy、取消、supersession，以及仅负责策略的
新一代 scheduler 仍是目标行为。
...
completion、异常发布与排序 policy。内建 CPU binding 则选择 issue #69 的进程 service，不分配
Graph-owned scheduler，也不携带 per-Graph worker grant。

显式注入的 service 直接拥有一个固定 CPU worker pool。它会在首次 CPU 使用前冻结一个解析后的
`[1,8]` 数量，接受多个 active Run，使用相互独立的 high/normal ready FIFO，并按 Run 隔离
completion、first exception、in-flight drainage、trace Host 与 settlement。函数内静态对象
`SchedulerWorkerBudget::process()` 会把所有 embedded Host 的保守 worker 计费限制为 32；它现在
既对每个 Kernel 的固定 service pool 计费一次，也对 legacy Graph-owned worker 计费，但仍不拥有
ready store、Run、内存、scratch、device、I/O 或 plugin-process budget。
...
当前 issue #69 的值实现了不可变 metadata、复合 identity、owned executable、匹配
...
释放 dependency、拥有 result index 并减少 logical completion count。Production Run
admission、有界 ready storage、resource estimate/grant 与 policy re-entry 仍是目标行为。
...
当前 `SchedulerWorkerBudget` 在对应迁移切片之前继续作为当前过渡行为存在。它会为每个固定 CPU
service 计入一项 pool-lifetime reservation，并计算 legacy per-Graph scheduler worker。它不会
被包装、重命名或 alias 成目标 ledger，并会在最终 resource-ledger ownership 取代该
containment model 时消失。
...
本决策冻结依赖契约。Issue #66 到 #69 已作为当前切片实现；后续切片仍按以下目标顺序推进：
...
| #69（当前） | 多 Graph 与 HP/RT 共享 CPU domain；删除 per-Graph 内建 CPU worker；dirty/preflight 使用 owned submission | 完整 admission/policy 模型与 `RunGroup` |
...
- 当前行为（包括 issue #69 的固定多 Graph HP/RT CPU pool、child Run、owned dirty
  submission 与 completion/failure isolation）继续以
