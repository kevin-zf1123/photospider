...
Kernel 的 best-effort `LastError` mirror 也使用同一个精确 `GraphInstanceId`，而不是可复用的
graph-name key。public inspection 会先解析当前 runtime，并且只复制其 identity 的 diagnostic，
因此从旧 runtime 保留下来的 calling-thread result translation 既不能覆盖、也不能清除同名
replacement。
...
callback。随后，它持有 graph-registry lock 的时间只够发布对迟到 GraphRuntime registration 的
单调拒绝；service transition 与预分配 cancellation fanout 会在释放该 lock 后运行。lifecycle
fence 会把已经构造的 load 与 `ServiceStopping` 排序：它要么先完成 publication 并进入 drainage，
要么输给已关闭 publication gate，且永不注册 lifecycle row。cancellation fanout 阻塞期间，
Graph listing 仍可用。由于 publication 无法安全重开，gate 关闭后的意外 transition failure 会
fail-stop。
