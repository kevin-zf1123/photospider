...
作为目标架构已接受。Issue #70 至 #75 已成为 CPU 执行/资源、policy 与私有 route 切片的当前软件
...
封闭的私有 execution route。`RunLifecycleRegistry` 与最终 close/shutdown/telemetry（#76）仍是目标
行为。
...
当前私有 Run 截至 Issue #75 已采用、完整目标也继续保留的阶段推进顺序如下：
...
Run submission 与 dirty 阶段都不会调整 pool 大小。当前 active-Run map
...
Issue #74 已在该当前 predicate 上增加：精确 supersession key/generation 与 live Graph
...
Issue #75 当前尚未引入 registry 的 close 已经保留这一本地双 lane 顺序：先停止 request
admission，在 graph-state 仍开放时排空已接受的 request work，再排空 graph-state。Graph
destruction 不会停止 process-owned route。Issue #76 必须把 registry fence 与该顺序组合，或显式
取代它；上述目标步骤不会恢复旧的 single-lane close 模型。Issue #73 会让已经取消的私有 Run
继续排空到 quiescence 并完成精确释放，但不会让 Graph close 成为 cancellation requester。
...
本决策冻结依赖契约。Issue #66 到 #75 已作为当前切片实现；#76 保持以下目标顺序。
...
- 当前行为（包括 issue #69 的固定多 Graph HP/RT CPU pool、issue #70 的 admission/ledger 边界、
  ...
  current-generation commit predicate 与 realtime `RunGroup`，以及 issue #75 的纯 C policy ABI、
  Host 编写的 frontier/fallback、reserved start 与私有 execution route）继续以
  [计算边界](../../kernel-architecture/zh/Compute-Boundaries.zh.md)、
  [计算流程](../../kernel-architecture/zh/Compute-Flow.zh.md) 和
  [策略与执行架构](../../kernel-architecture/zh/Policy-and-Execution-Architecture.zh.md) 为权威；#76
  final lifecycle/telemetry 只有在实现并经过持久验证后才成为当前事实。
