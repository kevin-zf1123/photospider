...
作为目标架构已接受。Issue #70 已成为 CPU 执行/资源切片的当前软件行为：embedded composition root
...
通过按 entry/byte 有界的 ready storage。`RunLifecycleRegistry`、权威 `GraphRevision` commit
validation、最终公平性 policy、取消、supersession，以及仅负责策略的新一代 scheduler 仍是目标行为。
...
completion、异常发布与排序 policy。内建 CPU binding 则选择 issue #70 的进程 service，不分配
Graph-owned scheduler，也不携带 per-Graph worker grant。

显式注入的 service 直接拥有一个固定 CPU worker pool。它会在首次 CPU 使用前冻结一个解析后的
`[1,8]` 数量，接受多个 active Run，使用按 entry/byte 有界且相互独立的 high/normal ready FIFO，
...
当前 issue #70 的值实现了不可变 metadata、复合 identity、owned executable、匹配
...
initial/dependent submission 都需要来自同一个有界 store 的匹配 child grant。Policy re-entry
仍是目标行为。
...
child grant。未来 policy 可以排列工作并建议有界 quantum；可信 service code 必须在铸造 authority
前验证建议。
...
本决策冻结依赖契约。Issue #66 到 #70 已作为当前切片实现；后续切片仍按以下目标顺序推进：
