...
作为目标架构已接受。Issue #70 至 #73 已成为 CPU 执行/资源与调度切片的当前软件行为：embedded
...
报告 `cancellable: false`。Latest-wins supersession（#74）、用于替换现有 ABI 且仅负责策略的
新一代 scheduler（#75），以及 `RunLifecycleRegistry` 与最终 close/shutdown/telemetry（#76）
仍是目标行为。
...
当前私有 Run 截至 Issue #73 已采用、完整目标也继续保留的阶段推进顺序如下：
...
- `Failed`：携带精确的 host-owned failure 或 exception category，包括 admission failure；
  或
- `Cancelled`：携带稳定原因。当前 #73 私有 source 覆盖显式请求与过期 monotonic deadline；
  Graph-close、process-shutdown 与 supersession source wiring 仍分别属于 #76 和 #74。
...
完整目标只会在该当前 predicate 上增加：#76 的 registry graph row 为 `Open`、Run 已注册且
graph-lifetime lease 有效，以及 #74 的 supersession generation 为 current。

不能因为 topology 相等或输出相似，就推断 revision compatibility。任何未来 compatible-revision
优化都需要新的显式决策。

#74 的 supersession 目标会让较新 generation 成为 current，并请求取消匹配的较旧 Run。它不会
...
`Pending`、`RtCommitted` 或 `Denied`。当前软件截至 Issue #73 会让经过验证的 RT publication
执行 `Pending -> RtCommitted`，而 publication 前的 RT failure 或 cancellation 会执行
`Pending -> Denied`；cancellation 还会请求 HP cancellation。即使迟到 RT work 完成，`Denied`
也永远不会重新开放。HP child 会应用独立的 revision、cancellation、terminal 与 staged-output
predicate，因此之后出现 stale 或 cancelled HP result 不会回滚 RT。Issue #74 会增加
supersession-generation predicate，#76 则会增加 Graph-closing 与 process-shutdown denial source。
...
本决策冻结依赖契约。Issue #66 到 #73 已作为当前切片实现；#74 到 #76 保持以下目标顺序。
...
- 当前行为（包括 issue #69 的固定多 Graph HP/RT CPU pool、issue #70 的 admission/ledger 边界、
  ...
  publication 边界，以及 issue #73 的私有 cooperative cancellation 与 Run-owned commit
  arbitration）继续以
  [计算边界](../../kernel-architecture/zh/Compute-Boundaries.zh.md)、
  [计算流程](../../kernel-architecture/zh/Compute-Flow.zh.md) 和
  [调度器架构](../../kernel-architecture/zh/Scheduler-Architecture.zh.md) 为权威；#74
  supersession、#75 scheduler ABI replacement 与 #76 final lifecycle/telemetry 只有在实现并经过
  持久验证后才成为当前事实。
