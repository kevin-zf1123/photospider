...
作为目标架构已接受。Issue #70 至 #74 已成为 CPU 执行/资源与调度切片的当前软件行为：embedded
...
请求会创建一个 request-owned `RunGroup`，其中包含不同的 HP 与 RT child Run。内建 CPU intent
binding 在
...
publication 不受之后 stale HP result 的回滚影响。Issue #73 增加私有 cooperative Run
cancellation：显式请求与 monotonic deadline 会与 failure 和 Run-owned commit contender 竞争；
匹配的 queued work 会被清除，已经进入的 work 会排空，dependent re-entry 会被拒绝，且已接受的
cancellation 不能发布 staged state。Sibling gate 仍为 `Pending` 时，RT cancellation 会拒绝 HP
commit 并请求 HP cancellation。Installed Host、CLI 与 IPC v1 surface 仍不可取消，IPC job 继续
报告 `cancellable: false`。Issue #74 增加 checked per-Graph supersession generation、精确 key 的
latest pending coalescing、每个 admitted key 的一个 persistent continuation ticket、稳定
supersession cancellation、current-generation commit authority 与确定性 `RunGroup` aggregate
settlement。现有有界 compute-request lane worker 是唯一 logical active runner；不会增加每个 Graph
的 background runner 或每个 generation 的 thread。Reserved start 与用于替换现有 ABI 且仅负责
策略的新一代 scheduler（#75），以及 `RunLifecycleRegistry` 与最终 close/shutdown/telemetry（#76）
仍是目标行为。
...
completion、异常发布与排序 policy。内建 CPU binding 则选择 Issue #70 至 #73 的进程 service，不分配
...
当前私有 Run 截至 Issue #74 已采用、完整目标也继续保留的阶段推进顺序如下：
...
- `Cancelled`：携带稳定原因。当前私有 source 覆盖显式请求、过期 monotonic deadline 与 Issue
  #74 supersession；Graph-close 与 process-shutdown source wiring 仍属于 #76。
...
failure 由精确 Run failure channel 捕获，且不能提交部分输出。Issue #73 cancellation 会保持同一个
不变量；后续 #76 lifecycle-registry 切片必须继续保持它。
...
scheduler-owner access。Issue #73 使私有 cancellation 与 commit-terminal arbitration 成为该当前
边界的一部分。
...
Issue #74 会在该当前 predicate 上增加：精确 supersession key/generation 与 live Graph
coordinator 相等。剩余完整目标会加入 #76 的 registry graph row 为 `Open`、Run 已注册且
graph-lifetime lease 有效。
...
predicate，因此之后出现 stale 或 cancelled HP result 不会回滚 RT。#76 会增加 Graph-closing 与
process-shutdown denial source。
...
恢复旧的单 lane close 模型。Issue #73 会让已经取消的私有 Run 继续排空到 quiescence 并完成精确
释放，但不会让 Graph close 成为 cancellation requester。
...
本决策冻结依赖契约。Issue #66 到 #74 已作为当前切片实现；#75 与 #76 保持以下目标顺序。
...
- 当前行为（包括 issue #69 的固定多 Graph HP/RT CPU pool、issue #70 的 admission/ledger 边界、
  ...
  arbitration，以及 issue #74 的 latest-wins generation、有界 ticket-backed coalescing、
  current-generation commit predicate 与 realtime `RunGroup`）继续以
  [计算边界](../../kernel-architecture/zh/Compute-Boundaries.zh.md)、
  [计算流程](../../kernel-architecture/zh/Compute-Flow.zh.md) 和
  [调度器架构](../../kernel-architecture/zh/Scheduler-Architecture.zh.md) 为权威；#75 scheduler
  ABI replacement 与 #76 final lifecycle/telemetry 只有在实现并经过持久验证后才成为当前事实。
