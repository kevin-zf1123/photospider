...
| [#57 / F-6](https://github.com/kevin-zf1123/photospider/issues/57)，kernel buffer primitive | [稠密图像 Value 内存契约](../../kernel-architecture/zh/Dense-Image-Value-Memory-Contract.zh.md)、[数据模型](../../kernel-architecture/zh/Data-Model.zh.md)和[脏区传播](../../kernel-architecture/zh/Dirty-Region-Propagation.zh.md) | 应用 [ADR 0002](0002-external-libraries-are-kernel-adapters.zh.md) 与[依赖中立目标](../../roadmap/zh/Kernel-Evolution.zh.md#依赖中立内核) | 只有 padded-row 产品路径测试通过后，才记录当前 stride-aware tiled normalization 与 metrics。 |
