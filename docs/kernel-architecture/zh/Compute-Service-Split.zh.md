...
| `ComputeTaskPlanner` | 将 compute request 和 graph topology 转换为请求级 `ComputePlan` / `ComputeTaskGraph` 语义。 | 已作为内部规划边界实现；plugin ABI 仍是 TODO |
...
| `ComputeTaskDispatcher` | 执行 `ComputeTaskPlanner` 的 plan 语义：收集 source task、检查 task-graph readiness、裁剪 dirty work、通过 `SchedulerTaskRuntime` dispatch ready task，并提交结果。 | dirty task split 的目标边界 |
...
消费 compute request 和 graph topology，然后产生请求级静态 plan 语义，供 sequential、
parallel、HP 和 RT 路径在具体执行分发前共同使用。Dirty-region state 会在静态分析之后应用：
每个 dirty snapshot 都会从 plan 中为当前 update queue 激活或裁剪 `DirtyUpdateWorkSet`。
执行模式只应在 task pool、scheduler policy 和资源选择上不同。
只要该 planner 边界仍保持内聚，就不额外引入 task-graph composer。

`ComputeTaskPlanner` 必须枚举该请求的真实 task graph，包括 node task 和 tile task。
Dirty snapshot 会裁剪这个已经 planned 的 graph；它不会展开 tile，也不会创建新的可执行 task shape。
这样 full-frame tiled compute 与 HP/RT dirty update 会共享同一个 task model。
...
`GlobalHighPrecision` plan，并从 HP dirty snapshot 中裁剪 HP work。RT 路径使用
`RealTimeUpdate` plan，并从 RT dirty snapshot 中裁剪 RT work。单次 `ComputeTaskPlanner`
调用不得同时发出 HP 和 RT 两份 task pool。未来扩展新的 task pool 时也应保持这种按 domain
分别调用 planner 的模式。
...
目标 HP dirty update 必须接收 dirty ROI，并从 high-precision task graph 中裁剪 HP work，使大图和大图像不需要为未受影响的 work 付出代价。现有完整重算行为只是尚未接入优化 HP dirty planning 的入口路径的兼容 fallback。
