...
Parallel 执行会并发启动 HP 与 RT dirty sibling，每个 sibling 再把 ready dirty work 提交到各自
intent-specific scheduler task runtime；coordinator 会先等待 RT、再等待 HP，然后返回 RT 输出。
Single-threaded 执行会 inline 运行同一份 intent 工作。
...
`RealTimeUpdate` plan，并从 RT dirty snapshot 中裁剪 RT work。单次 full graph expansion 或
pruner pass 不得同时发出 HP 和 RT 两份 task pool。未来扩展新的 task pool 时也应保持这种按
domain 分别 expansion 和 pruning 的模式。
