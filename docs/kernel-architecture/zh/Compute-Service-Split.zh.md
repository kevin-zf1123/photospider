...
在当前 `DirectGraphCommit` policy 下，coordinator 会先 inline 运行 HP dirty sibling，再
inline 运行 RT dirty sibling。每个 sibling 内部仍可以把 ready dirty work 提交到各自
intent-specific scheduler task runtime，但跨 intent 的 HP/RT sibling 并发会推迟到 buffered
commit 覆盖两个输出 domain 后再恢复。
...
`RealTimeUpdate` plan，并从 RT dirty snapshot 中裁剪 RT work。RT dirty node execution 会通过
`RealtimeDirtyWriteBuffer` stage 代理输出，并且只在 RT work set drain 后提交到 `GraphModel`。
单次 full graph expansion 或 pruner pass 不得同时发出 HP 和 RT 两份 task pool。未来扩展新的
task pool 时也应保持这种按 domain 分别 expansion、pruning 和 commit 的模式。
...
并从 high-precision task graph 中裁剪 HP work，使大图和大图像不需要为未受影响的 work 付出代价。
coordinator 会为该路径记录 `intent_coordinator_global_dirty_update`。
