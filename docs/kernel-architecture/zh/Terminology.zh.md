...
**`GraphRuntime`**
每图资源容器。它拥有一个 `GraphModel`、图状态访问、事件、当前 scheduler 和平台运行时资源，
但不拥有计算依赖规划。
...
**`GraphStateExecutor`**
当前图状态操作和可见计算请求使用的每图独占访问机制。每次提交目前都会启动一个
`std::async(std::launch::async)` operation；该 operation 会等待同一个 per-graph mutex，随后才调用
callback。它保证 mutual exclusion，但不保证 FIFO，也不限制等待 thread 的数量。它与 scheduler
dispatch 分离。
...
携带 scheduler metadata，但不会把 `GraphModel`、task graph 或 dirty propagation ownership 转交
scheduler。
