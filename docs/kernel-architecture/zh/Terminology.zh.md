...
本词汇表定义当前内核实现使用的语言。仍只属于已接受目标决策或
[内核演进目标](../../roadmap/zh/Kernel-Evolution.zh.md)的术语，不得被描述为当前运行时对象。
[ADR 0007](../../adr/zh/0007-compute-runs-and-process-execution-have-separate-owners.zh.md)
定义的 Issue #76 所有权，在本文明确说明时已经是当前事实。
...
并发 closer 会等待自己加入的持久 completion generation。Executor 的通用 failure recovery
不是 Graph-close reopen path：lifecycle close 在 marker 后保持单调，绝不会重建任一 lane。它与
ready dispatch 分离；其 worker 是不计费的基础设施，不是 Run execution grant。
...
lifecycle check 起保留 lease，直至 bundle 安装或 rollback；已安装 Run 会保留 lease，直至
commit/discard、resource settlement 与 registry unregistration。Anchor 还会让预分配的单调 close
coordinator 存活到 lane 与 runtime retirement。它不拥有 `GraphModel`、worker、route、policy 或
resource authority。
...
一个 `ExecutionService` 内由 process 拥有的 admission 与 settlement authority。其唯一
lifecycle fence 会索引 `Open`/`Closing` Graph row、pending candidate、standalone Run、realtime
bundle、finalization 与唯一 service `Accepting`/`Stopping` generation。它会原子安装 standalone
Run 或两个 realtime child，驱动 Graph-close/process-shutdown cancellation，只在精确
quiescence/resource settlement 后移除 Run，并在 lane destruction 前移除空 Graph row。它不拥有
plan、dispatcher、staged output、graph state、worker、policy decision 或 resource mint。
...
预分配 65,536 条固定 record，发布 15 种 event kind 与完整的 15-counter post-transition view，并
...
- Lifecycle telemetry 只是 observation，不是 admission、cancellation、resource、policy 或 close
  authority。
