...
snapshot。`GraphIOService` 会先通过 in-memory adapter，按 node id 升序 capture 一份脱离运行态且
...
map。每个 `GraphRuntime` 因此会停止 graph-state admission、drain 已准入 work，并在这些借用的
Kernel service 与全部注入 owner 仍存活时 join worker；只有之后的 service destruction 才会释放
它们。Owning Host 必须在 Kernel destruction 前停止外部 Kernel-call admission，因为 private graph
map 不是 concurrent-destruction API。Codec/document `GraphError` 保留各自文档化的 category，
`std::bad_alloc` 原样传播。
...
定义未来由 `ExecutionService` 拥有的 admitted-Run registry，以及一个原子的 Run
admission/Graph-close fence。该目标关系不会改变本文记录的当前 Host admission、
`GraphStateExecutor`、scheduler-drain 或 retryable close 行为。
