...
diagnostic state 重建。该 mirror 以精确 `GraphInstanceId` 为键，而不是可复用的 session label。
public name lookup 会先保留当前 runtime identity，再只复制该 identity 的 diagnostic，因此由旧
runtime 保留调用产生的迟到 result translation 既不能覆盖、也不能清除同名 replacement 的状态。
...
callback caller。随后，一个短 graph-registry transaction 会关闭单调 late-publication gate，并在
service transition 与 cancellation fanout 之前释放 mutex。lifecycle fence 会把已经通过构造的
...
gate 无法安全重开，gate 关闭后的意外 transition failure 会 fail-stop。直接拥有内部 Kernel 的
