...
内存 GraphDefinition seam 已是当前行为；filesystem 注入与 dependency-disabled persistence
仍属于内核演进路线图。
...
1. `GraphIOService` 通过私有 GraphDefinition/YAML translator 解析 YAML sequence。结果是
   一份有序、脱离运行态的 `GraphDefinition`；静态 parameter 与可选 output parameter 都是
   深度拥有、没有 YAML alias 的 `ParameterValue` tree。
2. 在生成 definition 时分类 parser、root-shape、parameter representation 与 node-field
   failure。Parameter 非 map value、不支持的 tag、规范化 key collision 以及 Int64/Double
   overflow 都属于 document failure。
...
topology、topology generation、cache、timing、dirty/planning state、runtime state 与 session
identity 都保持不变。这项保证同时覆盖 handled failure 和传播的 `std::bad_alloc`。
...
snapshot。In-memory adapter 会先按 node id 升序 capture 一份脱离运行态且排除全部 runtime state
的 definition；私有 YAML translator 随后在 destination open 前发出该 definition。Missing 或
...
candidate `NodeDefinition` parsing、强制 replacement-id assignment、in-memory
materialization 与 `GraphModel::replace_node()` 会在同一个 graph-state work item 内执行，因此
clear 或 reload 无法插入 lookup 与 mutation 之间。`get_node_yaml()` 使用相反方向的单节点
capture 与同一个私有 YAML translator；`Node` 不暴露 YAML conversion method。Missing 或
...
admission 会失败。Host 会在 lane 仍 accepting 时等待该 marker 之前已准入的同步调用完成 caller-visible
...
ingestion contract。`GraphDefinition` 与 `InMemoryGraphDocumentAdapter` 现在是当前
in-memory document boundary。Issue #61 仍负责 filesystem adapter 注入与 public
format-neutral Host composition；Issue #62 和 #63 仍负责精确
