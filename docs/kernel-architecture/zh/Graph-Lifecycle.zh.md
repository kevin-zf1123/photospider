...
4. 把 runtime 标记为 retired，移除 route id/map ownership，并销毁 Graph state。
...
`ServiceStopped`。直接拥有内部 Kernel 的调用方同样必须在析构前停止并发调用。这些 joined
boundary 也是每个 live 或 staged `GraphModel` diagnostic store 的 lifetime fence；store 本身
不拥有 thread 或 detached lifetime。
