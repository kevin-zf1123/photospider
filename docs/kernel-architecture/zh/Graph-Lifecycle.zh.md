...
选择 owner 或 joiner。因此，lookup 与 generation selection 是一个 transaction：caller 不会观察
旧 name 后错误加入 replacement runtime，而 owner 会在 map removal 后继续保持 retired runtime
存活。之后每个 runtime operation 都采用相同的短 lookup-and-retain 规则；lifecycle settlement
或任何 Graph-owned lane 运行时都不持有 graph-registry lock。
...
`ServiceStopped`。Kernel 会在 graph-registry mutex 内完成初始 service-shutdown transition 与
单调 late-publication-rejection flag：load 要么在 shutdown 前发布并由 shutdown 排空，要么观察
拒绝且永不注册 lifecycle row。直接拥有内部 Kernel 的调用方同样必须在析构前停止并发调用。
这些 joined boundary 也是每个 live 或 staged `GraphModel` diagnostic store 的 lifetime fence；
store 本身不拥有 thread 或 detached lifetime。
...
- Graph registry lock 只串行化 runtime-root lookup/publication、close-generation selection、
  最终 erase/success publication 与 shutdown publication rejection。需要嵌套时，顺序只能是
  graph registry 在 close coordinator 或 lifecycle registration 之前；任何 lifecycle/lane wait
  都不持有它，也不允许反向嵌套获取。
