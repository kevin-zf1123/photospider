...
| 架构决策 | [`docs/adr/`](../../adr/zh/README.zh.md) | 持久决策及其决策时背景。 |
...
| 边界 | Consumer 可以依赖哪些值、所有权规则、不变量、限制与故障表面？ | [稠密图像 Value 内存契约](Dense-Image-Value-Memory-Contract.zh.md)、[插件 ABI](Plugin-ABI.zh.md)、[计算边界](Compute-Boundaries.zh.md)和[单租户 Job 纵向路径](Single-Tenant-Job-Vertical.zh.md) |
