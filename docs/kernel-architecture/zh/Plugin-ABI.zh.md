...
- operation ABI v5：copied semantic trait、closed typed parameter schema、
...
因为 ABI v5 不携带 output stride：loading 会独立要求 contiguous signed stride 与 uint64
...
ABI5 和布局不变的 provider ABI1 schema 中均为 code 4；provider code 1..3 保持原义。
Host image validation 和 callback scope 恢复 embedding 的浮点环境，参见
[图像算子](Image-Operations.zh.md)。

## ABI 5 区域视图与宿主分配

ABI 5 输入包含独立 storage origin、byte offset、signed strides、有效 coverage 和 demand。
...
输入/复制输出说明由本节替换，宿主在 API table 查询前拒绝 ABI 4。

## S3 缩放端口

ABI 5 增加 Shrink 形状/区域规则及必需的 spatial_factor_parameter 指针/长度。
有界 Int64 参数解析为 [1,16]，输出 H/W 向上取整，输入需求为裁剪 box。允许蒙版
输出。未知布局、指针/数量失配、非法范围和旧 ABI 4 在发布前拒绝。
