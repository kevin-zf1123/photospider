...
- `OperationOutput` 拥有 image descriptor、named parameter value、spatial metadata 与 debug metadata；
  host 先把完整 public value 转换成 private representation，再附着 private DSO lease。
...
- `ParameterTypeError` 报告明确的 `ParameterValue` alternative mismatch；parameter conversion 与
  allocation failure 会在进入 callback 前原样传播。

Callback entry 前与成功返回后的 host conversion 位于 plugin exception fence 之外，并保留其 host-owned
type。实际 plugin invocation 会保留显式 DSO lease，并在该 lease 可能释放前规范化每个 plugin-origin
...
| 导出 | 是否必需 | 含义 |
| --- | --- | --- |
| `get_abi_version` | 是，且为第一道 gate | 不抛异常的数字握手，签名是 `uint32_t() noexcept`；必须返回当前值为 `1` 的 `PS_SCHEDULER_PLUGIN_ABI_VERSION`。 |
| `get_count` | 是 | 插件中的调度器类型数量；必须至少为一。 |
| `get_name` | 是 | 某索引处的稳定类型名称；小于 count 的每个索引都必须返回非空指针、非空文本。 |
| `get_description` | 是 | 某索引处的人类可读类型描述。 |
| `create` | 是 | 为某类型创建调度器实例；`IScheduler` 已公开继承 `SchedulerTaskRuntime`。 |
...
version、count、name、description、create 或 destroy，也不会发布 candidate state。
