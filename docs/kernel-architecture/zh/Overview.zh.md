...
内建 CPU binding 不拥有 owner，并共享 Host composition 中固定的 `ExecutionService` worker。
Legacy serial、GPU 与 plugin scheduler instance 仍归 graph 和 intent 所有。每个 embedded Host
拥有独立 ledger，其默认 CPU 维度为 32；Graph 的 HP/RT legacy 计费会作为 pair 原子准入，而每个
CPU Run 会预留完整的 CPU/memory/scratch/ready vector。Legacy replacement 会保留 candidate
headroom，直到 publication 成功或 rollback 完成。Reservation 与 child grant 都是 move-only
RAII owner；concrete scheduler 销毁与 queued/executing work 都会恰好一次释放精确容量。
...
7. Embedded close admission 会先发布 lifecycle marker，拒绝新的 compute/scheduler work，以及
   ...
   graph-state 保持可用于最终 commit 时排空。Kernel 接着排空 `GraphStateExecutor`，之后才停止
   scheduler 并移除 runtime。如果 scheduler stop 失败，会先重启 graph-state、再重启
   compute-request admission，然后返回失败，因此保留的 session 仍可 inspect、compute 或再次
   close。即使 restart
...
handshake，才能进入 discovery 或 creation。`GraphStateExecutor` 仍是可见 Graph capture、mutation、
predicate validation 与 publication 的独立访问边界。Runtime 的第二条有界 lane 串行化
same-Graph compute 与 scheduler-owner lifetime。两条各有一个 worker/64-slot 等待 FIFO 的 lane
都由每个 Graph 独立拥有，不计入 `ExecutionService` ledger。

公共 worker 请求范围是零到八。零会在构造前解析为
`min(max(1, hardware_concurrency()), 8)`；显式一到八保持精确，更大的值会被事务性拒绝。内置 CPU
Graph binding 与 `serial_debug` 消耗零 owner slot；已注册 ABI v2 plugin 消耗解析后的 CPU grant，
内置 GPU/heterogeneous binding 则在该 CPU charge 上保守地增加一个潜在 device worker。Host 会
共同规划 HP 与 RT，并从默认 CPU limit 为 32 的 service ledger 中只预留 legacy owner 的合计
charge。内建 CPU Run 则会在 ready 发布前另行预留完整且经过 checked arithmetic 的 CPU、
retained-memory、scratch、ready-entry 与 ready-byte vector。容量不足是 graph `ComputeError`；
无效请求与未知 scheduler type 是 `InvalidParameter`。该 ledger 不是所有进程 thread 的上限。
...
- 多 graph/intent 的内建 CPU work 会共享每个 embedded Host 的一个固定 `ExecutionService`。
  ...
  dispatch aging 只在仲裁已选定的 class 内生效，不能替换该决策。受保护 headroom 只限制 active
  内建 Throughput root reservation；Interactive 与过渡期 Issue #70 legacy owner 不会扣减该 class
  quota，而 ledger 仍是共享物理容量的最终权威。Throughput charge 会一直保留到所有 child grant
  结束后的精确 root release。Legacy scheduler 还保留按 graph/intent 的物理所有权与有界
