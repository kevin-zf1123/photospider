...
`GraphRuntime` 实现并拥有 host context，也拥有已注册 per-Graph scheduler 的生命周期顺序。
Issue #68 的 `ExecutionService` 还拥有一个附着到稳定 service host proxy 的 concrete CPU
scheduler；该 proxy 只会在一个同步 service Run 执行期间，把 trace/TLS observation 委托给
active Graph。Per-Graph scheduler 必须先 attach 到 runtime，再启动；`GraphRuntime::start()`
会启动此前已注册的 scheduler，
...
提交的 callback 存活到 `wait_for_completion()` 返回。Request-local dirty executor 仍使用这条
同步路径。失败 batch 不提交任何 handle，执行零个 callback，因此这些 executor 可以立即展开栈，
而不会在 queue 中留下指向已销毁 stack storage 的 entry。Rollback 后，同一 scheduler object
可以接收下一 batch。Exception publication 会先经过相同的 queue transaction gate，再选择自己的
epoch，所以并发 batch 只会被观察为完整提交或完整不存在。Executor 的 virtual destructor 为
protected：scheduler code 可以运行借用对象，却不能通过 base pointer 删除 compute-owned
storage。

非 realtime scheduler-backed full HP work 使用不同的当前路径。`ComputeRun` 的共享 control
...
`(RunId, RunLocalTaskId)` 与 executable。注入的 `ExecutionService` 只接受 active Run 的这些值，
并拒绝 borrowed handle 或匿名 callback。
...
`GraphRuntime` 保存以 `ComputeIntent` 为 key 的 scheduler map。每个私有 map binding 会把
过渡期 scheduler owner 与 per-Graph execution route，或者内建 process CPU route 及其精确 grant
原子配对。Kernel load 与 replacement 会从冻结的 `SchedulerPlan` construction branch 派生
route，绝不会从 display name 推导。直接 runtime installation 默认使用 legacy route。该 key 与
route 本身并不定义 QoS、deadline、fairness、cancellation、resource reservation 或
process-wide priority class。当前 contract 不会从 `ComputeIntent` 推导这些概念。

Scheduler 不会拉取 plan。`ComputeTaskDispatcher` 识别 readiness，再通过
`SchedulerTaskRuntime` 推送具体 callback 或借用 task handle：scheduler-backed full HP 使用由
lease 支撑的 owned callback，request-local dirty execution 则保留 borrowed handle。选中的
scheduler 可以根据 queue、priority hint、epoch 与 available device 对这些 ready submission
排序和路由，但不会收到 graph topology、compute plan，也不拥有 dirty propagation。
...
对于当前内建 CPU 的非 realtime full HP，它会通过 `ExecutionService` 提交 ready value；serial、
GPU、plugin、dirty 与 realtime route 则经由 `SchedulerTaskRuntime` 使用已配置的 per-Graph
`IScheduler`。Request Run 拥有 materialized full-HP plan 与 temporary slot，dispatcher 在两条
物理 route 上都保留 dependency/ready/completion 语义。
...
这些 scheduler 仍按 graph 与 intent 拥有物理资源。此外，issue #68 会显式组合一个私有
`ExecutionService`，并且只把内建 CPU 的非 realtime、非 dirty full HP 路由到其 concrete CPU
runtime。Service 在完整 Run setup、initial/dependent ready submission、exception-fenced wait 与
host-proxy unbind 的整个区间持有一个 mutex，因此同时最多有一个 active Run。可信的 resolved
grant 改变时，它会延迟重建 pool。它既不提供并发多 Graph 共享，也不提供 fairness。

虽然 migrated full-HP work 在 service 上执行，Graph-owned 内建 CPU scheduler 在这个过渡切片中
仍保持分配和计费。当前 containment layer 改用一个进程生命周期的
`SchedulerWorkerBudget`，在所有 embedded `Host` 与 `Kernel` 之间共享固定 32-slot 上限。
Graph load 会先规划两个 intent scheduler，并在构造任意一方前原子预留二者的合计计费。
返回的 pair 为每个 intent 各含一个 move-only
...
replacement 转成对外失败，销毁过程会把被替换的 reservation 恰好归还一次。这个 32-slot ledger
只约束内置 planning 或受信任 plugin grant 所代表的 worker；它不约束 graph-state executor、
新的 service pool、daemon thread、frontend helper、operation 内部 thread，也不是整个进程的
OS thread 总上限。`GraphStateExecutor` 具有独立的结构性上限：每个已加载 Graph 有一个 worker，
最多有 64 个等待 callback。Issue #69 负责删除重复 per-Graph CPU worker 并实现真正的多 Graph
CPU 共享。
...
- 单实例 worker grant 与共享 32-slot admission ledger 只约束过渡期 per-graph ownership；它们
  既不 admission 也不计算 service pool，也不宣称限制整个进程的 OS thread 总数。
- 当前 `IScheduler` interface 仍同时包含 policy 和物理 worker 所有权。Issue #68 service 会在
  私有 ready-submission 边界后复用一个 concrete 内建 CPU scheduler；它还不是 multi-Run
  executor、ledger 或 policy architecture。
...
固定其详细的 Run、ready-task、completion、resource 与 lifecycle 所有权；有界的 issue #68
单 Run CPU service 与 ready-submission 切片已经是当前行为。精确的
