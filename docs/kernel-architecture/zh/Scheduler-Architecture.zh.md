...
Scheduler 实现只通过 `SchedulerHostContext` 与 runtime 交互。
...
`prefer_gpu_for_hp=true`、已配置 GPU worker 且 `SchedulerHostContext` 报告 `GPU_METAL` 可用时，才可能进入
...
- Scheduler attachment 只允许使用 `SchedulerHostContext`；新增 capability 必须版本化，且不得暴露
  graph/runtime ownership 或 native backend handle。
