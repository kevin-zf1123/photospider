...
Issue #70 会把内建 CPU 的 HP 与 RT 工作（包括 dirty 与 preflight 阶段）路由到 service 的固定
worker、原子 resource admission 与按 entry/byte 有界的 ready store。Service 会按 Run 隔离
completion count、first exception、in-flight drainage、trace Host 与 settlement。High/normal
FIFO 提供 priority hint，但并不是最终 fairness 或 policy authority。内建 CPU
...
- 当前 `IScheduler` interface 对 legacy route 仍同时包含 policy 和物理 worker 所有权。Issue
  #70 service 会在私有 ready-submission 边界后拥有一个直接固定 multi-Run CPU pool、原子 ledger
  admission 与有界 ready storage；fairness/policy 仍是未来工作。
...
Graph HP/RT service、ownerless CPU binding、child Run、owned dirty/preflight submission、
原子 resource vector 与有界 ready store 已经是当前行为。精确的
