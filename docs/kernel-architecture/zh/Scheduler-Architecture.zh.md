...
也不是通用 `ComputeRun` cancellation contract。Scheduler epoch 不是 Run identity：当前 full HP
completion isolation 会在稳定 lease 下使用 `(RunId, RunLocalTaskId)`。当前 HP Run 具有
exact-once terminal arbitration，但没有 product cancellation claimant。可运行的 cancellation
仍是 ADR 0007 后续切片，当前 epoch 未实现该能力。
