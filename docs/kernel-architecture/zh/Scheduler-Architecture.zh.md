...
`force_cpu_for_rt`、`rt_preempt_threshold_ms` 以及 scheduler-local implementation priority table
这类旧字段，不再是当前代码路径的 active configuration。

### 每图物理资源所有权

每个 `GraphRuntime` 分别拥有一个 HP scheduler object 与一个 RT scheduler object。Graph load 会创建
二者，runtime start 会启动二者。默认使用 `cpu_work_stealing` 时，worker count 为零会由每个
instance 独立解析为 `hardware_concurrency()`。因此，默认 thread count 近似为：

```text
graph count x 2 intent schedulers x hardware_concurrency
```

`gpu_pipeline` 同样按 instance 拥有 CPU/GPU thread vector。`serial_debug` 是例外，会在 calling
thread 上同步执行。当前实现没有 process-wide worker pool、global thread budget 或 cross-graph
fairness mechanism。
...
每个 scheduler DSO 必须先由 `ps_scheduler_plugin_get_abi_version() noexcept` 返回精确的当前数字 ABI
值。缺少或不匹配 handshake 时，会在调用 implementation version、discovery metadata 或 instance
creation 前失败。Implementation version 随后只查询一次并缓存为 diagnostic metadata，不作为兼容性 gate。
...
`../codebase-structure/IPC-Protocol-v1.md` 持续维护。
...
也不是通用 `ComputeRun` cancellation contract。
...
- Graph-state 命令和可见 graph compute request 位于 `GraphStateExecutor` 边界内。
...
- 插件 scheduler lifecycle 遵循 `Plugin-ABI.md`。
- Scheduler attachment 只使用 `SchedulerHostContext`；该 context 不暴露 graph/runtime ownership
  或 native backend handle。
- 当前 interface 同时包含 policy 和物理 worker 所有权。ADR 0003 记录了供后续实现的另一项
  已接受 ownership decision；本文描述当前可执行契约。
