...
Run row 使用 `ceil(cost / weight)`。Interactive 排序偏好存在且更早的单调时钟 deadline；
throughput 排序采用加权且确定的规则。Class 仲裁首先执行：两个 class 都持续 ready 时，service
会在至多三次连续 Interactive dispatch 后选择 Throughput。随后，只有已选 class 内的 ready item
才会在八次成功 dispatch 后 aging；aging 不能跨越或替换已选 class。Initial 与 dependent work
共用同一路径，Run fairness row 会跨越临时为空的阶段继续存在。这些策略不拥有 worker、物理
ready store、token、budget、Run 或 Graph。
...
限制；Interactive admission 可以使用该保留量，但每个 Run 仍须获得 ledger 授权。过渡期 legacy
scheduler owner 保留 Issue #70 的 full-ledger admission；它们不会被分配 Throughput Run policy，
并可能使用该 headroom 内的共享容量。
