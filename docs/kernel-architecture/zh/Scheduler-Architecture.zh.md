...
throughput 排序采用加权且确定的规则。Ready item 在八次成功 dispatch 后 aging。throughput 持续
ready 时，即使跨越 aging threshold，service 也会在至多三次连续 interactive dispatch 后保证其
进展。Initial 与 dependent work 共用同一路径，Run fairness row 会跨越临时为空的阶段继续存在。
这些策略不拥有 worker、物理 ready store、token、budget、Run 或 Graph。
