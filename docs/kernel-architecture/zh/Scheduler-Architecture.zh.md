...
installation 默认使用 legacy route。该 key 与 route 本身并不定义 QoS、deadline、最终 fairness、
cancellation、resource reservation 或 process-wide priority class。当前 contract 不会从
`ComputeIntent` 推导这些概念。
...
ready work 排序和路由；service 使用 high/normal queue 与按 Run 隔离的 settlement。两者都不会
收到 graph topology、compute plan，也不拥有 dirty propagation。
...
Issue #69 会把内建 CPU 的 HP 与 RT 工作（包括 dirty 与 preflight 阶段）路由到 service。来自
多个 Graph 的独立 Run 共享其固定 worker。Service registry 会按 Run 隔离 completion count、
first exception、in-flight drainage、trace Host 与 settlement。High/normal FIFO 提供当前
priority hint，但并不是最终 fairness、有界 admission 或 policy authority。内建 CPU
`GraphRuntime` binding 不拥有 scheduler。
...
当前 containment layer 使用一个进程生命周期的 `SchedulerWorkerBudget`，在所有 embedded
`Host` 与 `Kernel` 之间共享固定 32-slot 上限。每个固定 service pool 都在完整生命周期内持有
一项 reservation。Graph load 只会在构造 owner 前原子预留 legacy HP/RT binding 的合计计费。
每项返回的 reservation 都会移入自己的 `ReservationOwnedScheduler`。`GraphRuntime` 仍负责
legacy scheduler shutdown 与 detach；reservation owner 只保证 concrete scheduler 的销毁发生在
slot 释放之前。因此 attach/start/YAML publication 失败、成功的 graph close 与 Host 销毁都会把
legacy slot 恰好归还一次。Close 失败则保留 runtime 与 legacy reservation，以便重试。关闭所有
Graph 不会释放已配置的 service pool；Kernel/service 析构才会释放它。
...
这个 32-slot budget 约束固定 service worker，以及 legacy 内建/plugin planning 所代表的 worker；
它不约束 graph-state executor、daemon thread、frontend helper、operation 内部 thread，也不是
整个进程的 OS thread 总上限。`GraphStateExecutor` 具有独立的结构性上限：每个已加载 Graph
有一个 worker，最多有 64 个等待 callback。
...
- 共享 32-slot 过渡期 budget 会对每个固定 CPU service pool 计费一次，并计算 legacy
  per-instance grant；它不是 production Run admission，也不宣称限制整个进程的 OS thread 总数。
- 当前 `IScheduler` interface 对 legacy route 仍同时包含 policy 和物理 worker 所有权。Issue
  #69 service 会在私有 ready-submission 边界后拥有一个直接固定 multi-Run CPU pool；它还不是
  最终 ledger、admission、fairness 或 policy architecture。

Ready-task-only 边界让 scheduler ordering、worker lifecycle 与 failure publication 可以在不拥有
graph topology 或 cache 的情况下测试。进程 budget 约束当前 fixed-service 与 legacy 物理
所有权，但不会把 reservation 重新解释为一条当前正在运行的 thread。
...
固定其详细的 Run、ready-task、completion、resource 与 lifecycle 所有权；有界的 issue #69
固定多 Graph HP/RT service、ownerless CPU binding、child Run 与 owned dirty/preflight
submission 切片已经是当前行为。精确的
