...
非 realtime scheduler-backed full HP work 使用另一条当前路径。`ComputeRun` 的共享 control
拥有 `TaskSubmissionPlan`、dependency state、runner 与 temporary slot。Dispatcher 先启动一个
空 borrowed-handle batch，仅用于建立 scheduler epoch；随后把每个真实 ready task 作为保留
...
并通过相关 `ComputeIntent` 配置的 `IScheduler` 实例，经由 `SchedulerTaskRuntime` 提交 ready
work。对于当前非 realtime full HP 工作，request Run 拥有 materialized plan 与 temporary slot，
dispatcher 仍拥有 dependency/ready/completion 语义。
...
固定其详细的 Run、ready-task、completion、resource 与 lifecycle 所有权；有界的 issue #67 HP
Run lease 与 full-HP completion-isolation 切片已经是当前行为。精确的
