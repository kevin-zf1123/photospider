...
`GraphRuntime` 实现并拥有 host context，也拥有已注册 legacy per-Graph scheduler 的生命周期
顺序。Issue #69 的 `ExecutionService` 则直接拥有一个固定 CPU worker pool，并在每个 active
Run state 中存储借用的 Host context；当独立 Graph 重叠时，worker TLS 仍会把 trace 与
completion 路由到匹配 Run。Per-Graph scheduler 必须先 attach 到 runtime，再启动；
`GraphRuntime::start()`
会启动此前已注册的 scheduler，
`GraphRuntime::set_scheduler()` 在 runtime 已运行时只会在 attach 之后启动新
scheduler，`GraphRuntime::stop()` 会关闭已注册 scheduler。Ownerless 内建 CPU binding 不执行
这些 scheduler lifecycle call。`Kernel` 的 bootstrap 代码必须通过 `GraphRuntime` 注册
binding，而不是预先启动 legacy scheduler。
...
提交的 callback 存活到 `wait_for_completion()` 返回。Legacy request-local dirty scheduler
仍使用这条同步路径；内建 CPU dirty/preflight 工作改用 heap-owned `ReadyTaskSubmission`
context。失败的 legacy batch 不提交任何 handle，执行零个 callback，因此这些 executor 可以立即
展开栈，而不会在 queue 中留下指向已销毁 stack storage 的 entry。Rollback 后，同一 scheduler
object 可以接收下一 batch。Exception publication 会先经过相同的 queue transaction gate，再选择
自己的 epoch，所以并发 batch 只会被观察为完整提交或完整不存在。Executor 的 virtual destructor
为 protected：scheduler code 可以运行借用对象，却不能通过 base pointer 删除 compute-owned
storage。

Full HP work 使用不同的当前路径。`ComputeRun` 的共享 control
拥有 `TaskSubmissionPlan`、dependency state、runner 与 temporary slot。对于内建 CPU planning，
dispatcher 会为每个 dependency-ready task 创建一个 move-only `ReadyTaskSubmission`。每个值
拥有不可变 request/trace metadata、匹配且不可伪造的 `ComputeRunLease`、
`(RunId, RunLocalTaskId)` 与 executable。注入的 `ExecutionService` 只接受匹配 active Run
的这些值，并拒绝 borrowed handle、匿名 callback 或混合 Run identity。

Serial、GPU 与 plugin full HP 保留 legacy owned-callback 路径。该路径先启动一个空
borrowed-handle batch，仅用于建立 scheduler epoch；随后把每个真实 ready task 作为保留相同 lease
与 identity 的 owned callback 提交。两条 full-HP 路径都不借用 raw `TaskExecutor`，并由匹配 task
identity 守卫 failure publication。

内建 CPU connected-parameter preflight 与 dirty HP/RT 阶段也以 owned value 跨越 service
边界。Heap-owned phase context 会复制不可变 plan/selection/dependency state、保留 child Run
lease，并在不导出 stack `TaskExecutor*` 的情况下 materialize 新 ready dependent。
...
success 或 first-exception path。在 issue #73 下，这也是 legacy cooperative cancellation path：
terminal callback 会跳过 operation 与 dependency release，同时只 retire 自己拥有的 unit。
Cancellation authority 仍位于 Run/plan 边界，并且不会增加 scheduler/plugin ABI 行为。
...
`GraphRuntime` 保存以 `ComputeIntent` 为 key 的 execution-binding map。每个私有 binding 会把
route 与一个可选的过渡期 scheduler owner 原子配对。内建 process CPU route 有意不拥有
Graph-owned scheduler，也不携带 per-Graph grant。Kernel load 与 replacement 会从冻结的
`SchedulerPlan` construction branch 派生 route，绝不会从 display name 推导。直接 runtime
installation 默认使用 legacy route。该 key 与 route 本身并不定义 QoS、deadline、cancellation、
resource reservation 或 process-wide priority class。内建 service Run 会携带显式
`ComputeRunQos` class、deadline 与 weight；这些值都不会从 `ComputeIntent`、output quality 或
maximum parallelism 推断。

物理 domain 不会拉取 plan。`ComputeTaskDispatcher` 与 dirty phase context 识别 readiness，再
通过 `SchedulerTaskRuntime` 推送具体 owned submission、callback 或 legacy borrowed task
handle。选中的 legacy scheduler 可以根据 queue、priority hint、epoch 与 available device 对
ready work 排序和路由；service 使用 policy-aware 有界 ready store 与按 Run 隔离的 settlement。
两者都不会收到 graph topology、compute plan，也不拥有 dirty propagation。

对于 `RealTimeUpdate`，`IntentUpdateCoordinator` 会通过两个独立 asynchronous call 先启动 RT
dirty sibling，再启动 HP dirty sibling；当两个物理 execution domain 都可用时，两个 sibling
可以并发计算。该 request 拥有不同的 HP 与 RT child Run。内建 CPU child 共享固定 service；
legacy child 使用各自的 intent scheduler。RT 写入会 stage 到
...
每个物理 domain 只处理 ready callback 与自身的 queue/settlement mechanism；它不会创建
sibling、聚合目标 `RunGroup`，也不会让 RT output 成为正式 HP cache source。
...
对于当前内建 CPU 的 HP、RT、connected-parameter preflight 与 dirty work，它会通过
`ExecutionService` 提交 owned ready value；serial、GPU 与 plugin route 则经由
`SchedulerTaskRuntime` 使用已配置的 per-Graph `IScheduler`。Full HP Run control 拥有
...
| 类型 | 含义 |
| --- | --- |
| `cpu_work_stealing` | 内建 CPU planning label；Kernel 会把它路由到固定 process service，而直接 factory 测试仍保留 concrete scheduler 实现。 |
...
### 固定 CPU service 与过渡期 legacy 所有权

每个 `GraphRuntime` 分别拥有一个 HP binding 与一个 RT binding。Binding 要么选择 ownerless
内建 CPU service，要么拥有过渡期 serial、GPU 或 plugin scheduler。配置的 worker 请求只有零到
八有效。在构造第一个内建 CPU pool 或 legacy scheduler/plugin 前，零会解析为：
...
首次选择内建 CPU 会冻结解析后的数量，并启动一个直接 `ExecutionService` pool。之后的零或
相同请求保持幂等；冲突的正数请求会被拒绝。Graph load、replacement、Run execution 与 dirty
阶段都不会调整该 pool。

Issue #70 与 #71 会把内建 CPU 的 HP 与 RT 工作（包括 dirty 与 preflight 阶段）路由到 service
的固定 worker、原子 resource admission 与 policy-aware、按 entry/byte 有界的 ready store。
Service 会按 Run 隔离 completion count、first exception、in-flight drainage、trace Host 与
settlement。私有 interactive 与 throughput 策略比较不可变 descriptor；store 拥有 fairness row
与物理 entry。内建 CPU `GraphRuntime` binding 不拥有 scheduler。

Dispatch cost 为 `work_units + ceil(complete_ready_grant_bytes / 4096)`。Graph row 使用原始 cost，
并分别记入 Interactive 与 Throughput accumulator；每个 Run 只有一个不可变 class，并在该 class
中使用 `ceil(cost / weight)`。Interactive 排序偏好存在且更早的单调时钟 deadline；throughput
排序采用加权且确定的规则。Class 仲裁首先执行：两个 class 都持续 ready 时，service 会在至多三次
连续 Interactive dispatch 后选择 Throughput。随后，只有已选 class 内的 ready item 才会在八次
成功 dispatch 后 aging；aging 不能跨越或替换已选 class。Initial 与 dependent work 共用同一路径，
Run fairness row 会跨越临时为空的阶段继续存在。这些策略不拥有 worker、物理 ready store、token、
budget、Run 或 Graph。

注册 plugin 会按完整解析 grant 计费，可以少创建 worker，但不得超过授权。`gpu_pipeline` 与
`heterogeneous` 按解析后的 CPU grant 再加一个已配置的潜在 GPU worker 计费，即使准入时设备
不可用也一样；因此当前单实例绝对计费上限为九。`serial_debug` 是唯一的零 slot 例外，会在
calling thread 上同步执行。

每个 embedded Host composition 都会为其 `ExecutionService` 提供一个具有不可变上限的五维
`ResourceLedger`。固定 service thread 属于基础设施；active Run 会预留 CPU 执行权，以及声明的
retained-memory、scratch、ready-entry 与 ready-byte 容量。Graph load 会在构造任一 owner 前，
原子预留 legacy HP/RT binding 的保守 CPU-slot 合计计费。每项返回的 reservation 都会移入自己的
`ReservationOwnedScheduler`。`GraphRuntime` 仍负责 legacy scheduler shutdown 与 detach；
reservation owner 只保证 concrete scheduler 的销毁发生在精确释放之前。因此 attach/start/YAML
publication 失败、成功 graph close 与 Host 销毁都会把 legacy slot 恰好归还一次。Close 失败则
保留 runtime 与 reservation，以便重试。

Legacy scheduler replacement 是 strong transaction：旧 binding 与 reservation 保持存活时，
先规划并预留 candidate，再 attach/start candidate 并发布，最后才退役旧 owner。因此 legacy
replacement 需要 transient headroom。内建 CPU replacement 则在需要时配置固定 pool，并发布
ownerless service binding。容量不足会在不扰动旧 compute 行为的情况下返回
`GraphErrc::ComputeError`；无效请求、冲突的固定数量与未知类型仍为 `InvalidParameter`。Legacy
发布后，旧 owner 的 shutdown 与 detach 仍会进行 best-effort sweep；销毁过程会把被替换的
reservation 恰好归还一次。
...
Issue #70 legacy owner 不会扣减这项 class quota；每个 owner 仍须获得 ledger 授权，并可能使用共享
物理容量。Throughput charge 跟随精确 root lifetime，只有 parent 与所有 child grant 都释放匹配的
ledger commitment 后才会扣回。
...
匹配 Host call 期间保持存活。Embedded Host 会在与 compute 相同的私有 per-graph
compute-request lane 内执行 scheduler name/statistics copy 与 replacement。因此，运行中的 compute
不能与该 graph 的 scheduler inspection 或 replacement 重叠；active compute 释放旧 scheduler
前，replacement 也不能销毁它。Visible Graph capture/publication 与 close mutation 仍位于独立的
graph-state lane。
...
调度器队列使用 epoch 丢弃陈旧排队工作。Epoch `0` 被视为不可取消兼容工作。只有携带真实 epoch
的 submission 才能作为 stale work 丢弃。Epoch filtering 不会停止已经运行的 callback，也不是
通用 `ComputeRun` cancellation contract。Scheduler epoch 不是 Run identity：当前 full HP
completion isolation 会在稳定 lease 下使用 `(RunId, RunLocalTaskId)`。
...
- 具体 ready parallel work 通过注入 service 或 per-Graph scheduler task runtime 路由；二者都
  不拉取 plan。
- Visible Graph capture、mutation、commit validation 与 publication 位于单 worker、64 个等待任务
  的 `GraphStateExecutor` FIFO lane 内。Operation execution 在该 lane 之外使用 request-owned
  snapshot；同一 Graph 的 compute 与 scheduler-owner access 使用具有相同上限的第二条 lane。
...
- Host ledger 会计算已准入 Run vector 与 legacy per-instance CPU grant；固定 service thread
  保持为基础设施，并且这些上限不宣称限制整个进程的 OS thread 或 allocation。
- 当前 `IScheduler` interface 对 legacy route 仍同时包含 policy 和物理 worker 所有权。Issue
  #70 与 #71 service 会在 ready-only 边界后拥有一个直接固定 multi-Run CPU pool、原子 ledger
  admission、policy-aware 有界 ready storage 与私有 interactive/throughput 策略。这些策略不拥有
  物理或资源权威；用于替换 plugin 的 policy ABI 仍是未来工作。

Ready-task-only 边界让 scheduler ordering、worker lifecycle 与 failure publication 可以在不拥有
graph topology 或 cache 的情况下测试。Ledger 会保留精确的已准入 authority，但不会把 reservation
重新解释为一条当前正在运行的 thread。
...
固定其详细的 Run、ready-task、completion、resource 与 lifecycle 所有权；issue #70 的固定多
Graph HP/RT service 与 ledger，以及 issue #71 的 policy-aware bounded store、公平性、aging、
headroom 与 throughput 进展已经是当前行为。精确的
