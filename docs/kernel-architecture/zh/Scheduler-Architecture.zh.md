...
提交的 callback 存活到 `wait_for_completion()` 返回。对于非 realtime full HP 工作，当前
`ComputeRun` 拥有 `TaskSubmissionPlan`、dependency state、handle 与 temporary slot，但栈上的
runner 和 completion identity 仍是借用关系。失败 batch 不提交任何 handle，执行零个 callback，
因此 request-local dirty executor 可以立即展开栈，而不会在 queue 中留下指向已销毁 stack
storage 的 entry。Rollback 后，同一 scheduler object 可以接收下一 batch。Exception publication
会先经过相同的 queue transaction gate，再选择自己的 epoch，所以并发 batch 只会被观察为完整
提交或完整不存在。Executor 的 virtual destructor 为 protected：scheduler code 可以运行借用对象，
却不能通过 base pointer 删除 compute-owned storage。
...
也不是通用 `ComputeRun` cancellation contract。当前 HP Run 已有 identity 与 exact-once
terminal arbitration，但没有 product cancellation claimant。稳定 lease、completion isolation 与
可运行的 cancellation 仍是 ADR 0007 后续切片，当前 epoch 未实现这些能力。
...
固定其详细的 Run、ready-task、completion、resource 与 lifecycle 所有权；当前只实现有界的
issue #66 HP Run storage/terminal 切片。精确的
