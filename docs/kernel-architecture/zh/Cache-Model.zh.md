...
Dirty RT execution 不会让 worker task 直接写 graph-owned RT 字段。它会先把代理输出、ROI
metadata、版本计数和 dirty-source commit generation stage 到 `RealtimeDirtyWriteBuffer`，
然后在 RT dirty work set drain 后把 staged state 提交到 `GraphModel`。这让当前
`DirectGraphCommit` 路径保持确定性，同时为后续 buffered commit policy 恢复跨 intent HP/RT
sibling 并发留出空间。
...
- 新 RT 代码将 `cached_output_real_time` 写为临时交互式状态；dirty worker 写入必须先经过
  `RealtimeDirtyWriteBuffer`，再提交到 graph。
