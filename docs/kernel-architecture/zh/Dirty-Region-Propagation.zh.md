...
层拥有，并由内部 `InteractionService`、dirty work-set materialization、测试和 debug tooling
消费。Frontend 通过 public `ps::Host` 方法消费复制后的 snapshot value。
...
- `dirty_updating_count`：内部 `DirtyRegionSnapshot` 保存、并复制到
  `DirtyControlLaneResult` 的派生计数，表示当前处于 begin/end dirty-region lifecycle 中的 dirty
  source node 数量。当它降到 0 时，executor 可以在最后一批相关 work 结束后结束当前 compute
  request。它不是 compute-task reference count。`DirtyControlLane::build_result`
  根据已完成的 snapshot 与 lifecycle event 派生 wakeup/cutoff 决策，并且只把这些决策保存到
  `DirtyControlLaneResult`；propagator 不拥有这些决策。
...
Dirty-node lifecycle event 会进入串行的图级 `DirtyControlLane`。Frontend caller 使用 public Host
lifecycle 方法，embedded adapter 再通过内部 `Kernel` / `InteractionService` 边界进行路由。
Control lane 更新 `DirtyRegionSnapshot` 中的 dirty source membership 和 lifecycle state；随后
propagator 从这些 source 派生 `actual_dirty_region`。`DirtyControlLane::build_result` 再根据该
snapshot 与当前 lifecycle event 派生用于 work-set materialization 的 wakeup/cutoff 决策。
它不是 scheduler 拥有的普通 compute task queue，也不应下放为 node-local compute ownership。

TODO：定义面向未来 GUI 的 node 到 backend subscription/event transport，以及对应的 public Host
delivery contract。Node 仍作为 realtime update event 和 dirty-region record 的来源；内部
`InteractionService` 执行 backend translation，Host 暴露复制后的 dirty inspection snapshot
与 lifecycle operation status，但不暴露内部 updating-source count 或 wakeup/cutoff 决策，也不
成为 dirty-region generator。该设计必须说明事件来源、dirty-region 生成责任、node/backend
边界以及 GUI 消费方式。
...
- 尺度映射：在 generic CPU-resizable pixel path 上，当前 RT 代理为原图宽高 1/4（像素约 1/16）。因此
  - RT Micro_16 ↔ HP Micro_64（×4 放大或 ÷4 缩小）；
  - RT Macro_64 ↔ HP Macro_256。
  - 没有匹配 device adapter 的非 CPU 或 context-only descriptor 不会由 generic path 缩放。Downsample
    会把 HP descriptor 与完整 extent 保留为 backend-preserving passthrough；不得把它解释为已缩小的 CPU
    pixel。只有匹配的 backend adapter 或 operation 真正生成 scaled proxy 后，4x 坐标规则才适用。

当前默认参数：

| Domain 粒度 | 当前值 | 坐标空间 | 说明 |
| --- | --- | --- | --- |
| RT downscale factor | 4 | N/A | Generic CPU-resizable 代理尺度，可调优；opaque passthrough 保留完整 extent。 |
...
- **数据依赖算子**（如 liquid/warp/displacement）无法通过参数静态推导。Built-in 在 `OpRegistry`
  注册，v2 plugin 则使用 `OperationPluginRegistrar`；两者都提供：
  - `DependencyLutBuilder`。Public callback 返回 owned `DependencyLutSnapshot`，host 完成验证后才把它
    转换为 private、网格化的 `SpatialDependencyMap`。
  - 当 dependency 会受图像内容影响时设置 `OperationMetadata::data_dependent`，让 cache identity 与
    scheduling 使用正确模式。
...
已验证 LUT 与精确 private reuse identity 会成对存入 `Node::dependency_lut_cache`。只有以下全部内容
相等才允许复用：deep-owned effective parameter、静态 parameter-document revision、每个 parameter-input
content revision、每个 image-input 的 source node/output identity 与 extent、dependency-builder ownership
revision，以及已解析 data-dependent-flag revision。对于 data-dependent operation，每个 image-input 的
HP content revision 也参与 identity；静态 dependency 不会仅因图像像素变化而失效。任一项变化都会在
下一次传播请求时重建 LUT。Builder result 与完整 identity 会先完成 staging、conversion 与 validation，
再通过 no-throw pair replacement 发布，因此 conversion、validation、allocation 或 callback failure
都会保留此前 cache。Private revision 不进入 public plugin SDK。

构建过程只遍历有限数量网格点，通常在毫秒级别即可完成，因此可以同步懒加载（`ComputeService` 或
调试命令在传播路径上直接触发），不会引发长时间卡顿。
...
- RT：
  - 对已经生成的 generic CPU proxy，当前粒度为 RT Proxy Micro_16，尽量只在 `dirty_roi` 及其传播影响
    范围内更新；
  - 如缺 `tiled_op_rt`，只有在该 scaled proxy 已存在时，才在 RT 代理尺度回退 `tiled_op_hp`；
  - 没有 device adapter 的 opaque non-CPU descriptor 会保持 full-extent backend-preserving descriptor。
    Generic RT code 不得把它当作已缩小的 CPU pixel；实际缩放需要匹配的 backend adapter 或 operation。

- HP：
  - 当前采用 Micro_64/Macro_256 混合（优先 Macro_256）推进吞吐；
  - Global HP dirty ROI 完成后可以通过 downsample 刷新 RT。Generic CPU path 会生成配置的 scale；不受支持
    的 opaque backend 会保留 HP descriptor 与完整 extent 进行 passthrough，直到匹配 backend
    implementation 可以完成缩放。RealTimeUpdate HP sibling work 会抑制直接 graph RT downsample 写入；
    后续 RT sibling 会 stage 并提交自己的代理输出。

对于已生成的 scaled proxy，planner 可以为了同步或 inspection 使用尺度转换表达相互对应的 HP/RT ROI；
opaque full-extent passthrough 不蕴含该转换。Task dependency 仍留在各自 domain 内：RT Micro_16 <->
RT Macro_64，HP Micro_64 <-> HP Macro_256。
...
- Public `ps::Host` 是面向 frontend 的 interface。它的 embedded adapter 使用内部
  `InteractionService`，把图级 snapshot/lifecycle value 复制给 frontend visualization；这两个
  边界都不是 dirty-region generation 或 propagation 的权威来源。
