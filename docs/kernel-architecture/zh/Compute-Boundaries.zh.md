...
独立 compute-request lane 拥有 same-Graph compute 顺序与 scheduler-owner lifetime。即使 ready
callback 在 scheduler worker 上执行，规划和派发仍属于 compute 职责。

当前排他机制是有界串行 FIFO lane。每个处于 accepting 状态的 `GraphStateExecutor` 恰好拥有一个
worker。其队列最多容纳 64 个等待 callback，不包括至多一个正在执行的 callback，因此每条 lane
最多拥有 65 个已经 admission 的 callback。队列已满时，`submit()` 会阻塞 caller；它
不会创建额外 lane worker，也不会丢弃或绕过已经 admission 的 work。Admission 之前不保证 producer
fairness，但已经 admission 的 work 会按 FIFO 执行。

两条 lane 都复用这种有界 FIFO 机制。每次 submission 都会返回 packaged-task future，精确保留 callable 的 value、reference、`void`
...
之前抛出 `std::logic_error`。Operation work 运行期间，compute-request worker 拥有该 request，
但只会为 snapshot capture 或最终精确 revision transaction 进入 graph-state。
...
随后排空 graph-state，最后才 teardown scheduler。如果显式 close 随后在 scheduler shutdown
中失败，Kernel 会先重启 graph-state、再重启 compute-request lane，然后返回失败，使保留的
session 仍可重试。不同 graph 具有独立的 worker 和队列。Host composition 的 resource ledger 不对这些 lane
...
| 模块 | 当前职责 | 不拥有 |
| --- | --- | --- |
| `ComputeService` | 请求验证、intent 协调、创建/settle 一个 HP Run 或不同的 HP/RT child Run、调用 staged commit policy、协作者构造和最终结果选择 | 前端值、worker thread、图文档、live Graph revision authority 或目标 `RunGroup` policy |
| `ComputeRun` | 带精确 Graph identity/revision 的不可变单 domain HP/RT descriptor、单调 phase、私有弱生命周期 cancellation source、read-only lease observation、唯一 terminal/commit arbiter、通过共享 control 对 full-plan/temporary storage 或 dirty-HP staging storage 的所有权、稳定 lease，以及复合 task identity | 配对 realtime grouping、Graph state、worker、revision mint/publication authority、公开 cancellation control 或 resource admission |
| `ComputeCommitPolicy` | 仅产品使用的精确 Run/staged/live provenance 验证、保留的 read-only Run lease、transaction 内 cancellation observation 与 Run-owned commit-contender resolution、延迟 HP cache persistence，以及在 Run success 前串行发布可见状态 | Planning、execution worker、cancellation source 或任意 cancellation authority、supersession、最终 lifecycle registry 或 public ABI |
...
| `ExecutionService` | 一个注入的固定内建 CPU worker domain、一个 Host 权威 `ResourceLedger`、policy-aware 且按 entry/byte 有界的 ready store、私有 interactive/throughput policy state、完整 Run resource vector 的原子准入、带精确 Run queued purge 与 running-callback drainage 的 cancellation notification、按 Run 隔离的 completion/failure/trace settlement，以及 HP/RT dirty/preflight 执行 | Planning、dependency、Graph/cache state、cancellation authority、GPU/plugin 执行、lifecycle admission registry 或 visible commit |
...
1. `Kernel` 解析 session、进入 compute-request lane，并在一个 graph-state work item 中捕获
   request-owned Graph/proxy snapshot。
...
3. 对于非 realtime HP，`ComputeService` 在 planning 前创建一个 `ComputeRun`；对于 realtime，
   它会在 preflight 前创建不同的 HP 与 RT child Run。每个 Run 都捕获 fresh Run id、session
   label、强 Graph instance identity、权威 revision、target、单 domain intent、full 或
   interactive quality 与显式 QoS。一个私有 request cancellation coordinator 会 attach HP Run
   或两个彼此独立的 realtime child；它可以把一次 explicit request 扇出到两个 child，而不创建
   目标 `RunGroup`。
...
7. 顺序执行 inline 遍历同一请求语义。每个内建 CPU 并行阶段都会 materialize 保留 Run lease
   ...
   Explicit cancellation 或过期的 injected monotonic deadline 会在既有 planning、queue、callback、
   dependency、phase 与 commit boundary 被观察。Service 只关闭并清除匹配 Run 的 queued entry；
   已进入的 callback 会排空，但不会释放新的 dependent 或发布 staged output。
...
9. 输出验证后，每个 Run 到达 `CommitPending`。产品 policy 验证精确 staged/live identity 与
   revision。它会保留 read-only Run lease、进入 graph-state work item、观察 cancellation，
...
配置的 interactive headroom 只把 active 内建 Throughput root reservation 限制在
`limits - interactive_headroom`。Interactive Run 与过渡期 Issue #70 legacy owner 不会扣减这项
class quota，但所有 owner 仍在唯一 ledger 中共享最终物理容量。Throughput quota check、ledger
...
request-owned RT proxy snapshot。Sequential HP、scheduler-backed full/dirty HP 与 RT worker 在
operation work 期间都不能修改 live Graph 或 proxy。Snapshot 磁盘写入保持关闭。
...
回滚已经提交的 RT proxy。Installed Host、CLI 与 IPC version 1 surface 不暴露 cancellation
entry；IPC job 继续报告 `cancellable: false`。
...
所有权契约。本文是截至 issue #73 的权威说明：固定多 Graph HP/RT CPU service、ownerless 内建
CPU binding、不同的 realtime child Run、owned dirty/preflight submission、原子 resource
vector admission、有界 ready storage、内建 interactive/throughput 公平性与 headroom，以及共用
一个 Host ledger 的 legacy per-Graph scheduler、强 Graph identity/revision、request-owned 产品
staging 与精确 revision visible commit。Private request/deadline cancellation、精确 Run queued
purge、running observation/drainage、dependent suppression、Run-owned commit arbitration 与
RT-denies-HP cancellation 已是当前行为。Latest-wins supersession 与 request-level `RunGroup`
（#74）、scheduler ABI 替换（#75）、带 graph-close/process-shutdown cancellation 与 telemetry
