...
Host、CLI 与 IPC version 1 surface 不暴露 cancellation entry；IPC job 继续报告
`cancellable: false`。
...
所有权契约。本文是截至 issue #74 的权威说明：固定多 Graph HP/RT CPU service、ownerless 内建
...
RT-denies-HP cancellation 已是当前行为。每个 Graph 的 latest-wins supersession、精确 current
generation commit authority、有界 ticket-backed coalescing，以及 request-level realtime
`RunGroup` ownership 也已是 Issue #74 当前行为。Reserved-start scheduler admission 与 scheduler
ABI 替换（#75）、带 graph-close/process-shutdown cancellation 与 telemetry 的最终 lifecycle
admission/lease（#76），以及 public Host/CLI/IPC cancellation entry point 仍是未来行为。
