...
`work_units + ceil(complete_ready_grant_bytes / 4096)` 计费。Graph row 累加原始 cost，Run row
累加 `ceil(cost / weight)`。显式 interactive QoS 会偏好存在且更早的单调时钟 deadline；
throughput 排序采用加权且确定的规则。Ready item 在八次成功 dispatch 后 aging。throughput 持续
ready 时，即使跨越 aging 边界，service 也会在至多三次连续 interactive dispatch 后强制保证其进展。
Run row 会跨越临时为空的阶段保持安装，因此 dependent re-entry 无法重置公平性历史。配置的
interactive headroom 不可用于 throughput Run admission；Interactive Run 可以使用它，每项
reservation 仍须获得 ledger 的最终授权。过渡期 legacy scheduler owner 保留 Issue #70 的
full-ledger admission，不会被分类为 Throughput。私有 policy 策略不拥有 worker、ready entry、
resource token、budget、Run 或 Graph。Revision 偏好、取消与 supersession 不属于该调度切片。
...
所有权契约。本文是截至 issue #71 的权威说明：固定多 Graph HP/RT CPU service、ownerless 内建
...
一个 Host ledger 的 legacy per-Graph scheduler。权威 revision、`RunGroup`、lifecycle admission、
