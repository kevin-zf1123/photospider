...
| `ComputeRun` | 带精确 Graph identity/revision 的不可变单 domain HP/RT descriptor、单调 phase、唯一 terminal outcome、通过共享 control 对 full-plan/temporary storage 或 dirty-HP staging storage 的所有权、稳定 lease，以及复合 task identity | 配对 realtime grouping、Graph state、worker、revision mint/publication authority、cancellation 或 resource admission |
| `ComputeCommitPolicy` | 仅产品使用的精确 Run/staged/live provenance 验证、延迟 HP cache persistence，以及在 Run success 前串行发布可见状态 | Planning、execution worker、cancellation、supersession、最终 lifecycle registry 或 public ABI |
...
| `ExecutionService` | 一个注入的固定内建 CPU worker domain、一个 Host 权威 `ResourceLedger`、policy-aware 且按 entry/byte 有界的 ready store、私有 interactive/throughput policy state、完整 Run resource vector 的原子准入、按 Run 隔离的 completion/failure/trace settlement，以及 HP/RT dirty/preflight 执行 | Planning、dependency、Graph/cache state、GPU/plugin 执行、lifecycle admission registry 或 visible commit |
...
3. 对于非 realtime HP，`ComputeService` 在 planning 前创建一个 `ComputeRun`；对于 realtime，
   ...
   interactive quality 与显式 QoS。目前尚无目标 `RunGroup`。
...
9. 输出验证后，每个 Run 到达 `CommitPending`。产品 policy 验证精确 staged/live identity 与
   revision，可选持久化变化的 HP artifact，并在同一个 graph-state work item 中发布完整
   Graph/proxy state。Run success 在该 publication 之后；stale 或失败的 commit 会变成 failure。
   Coordinator 只有在两个 child 都 settle 后才返回 RT output；随后结果、事件、计时和错误通过
   Host value 边界复制返回。
...
entry、resource token、budget、Run 或 Graph。Revision 偏好、取消与 supersession 不属于该调度
切片。
...
materialize 完整 publication copy。随后一个 graph-state work item 要求精确 staged owner、Run
domain/label、强 Graph identity 与权威 revision 同时匹配 descriptor 和 live Graph。只有有效 HP
transaction 可以持久化变化的 staged cache artifact；完整 Graph/proxy publication 使用 no-throw
state swap，并保留 revision。Run `Succeeded` 在 publication 后发生。Stale predicate 不发布 Graph、
proxy 或延迟磁盘输出，并转换为 `ComputeError`。

RT 会先应用该 predicate 并发布 proxy，再打开 sibling gate。HP 随后独立验证。因此，较新的 Graph
revision 可以拒绝 HP，而不会回滚已经胜出的 RT publication。Issue #72 这份契约不提供
cancellation、latest-wins supersession、最终 lifecycle registry/lease 或 request-level
`RunGroup` settlement。
...
所有权契约。本文是截至 issue #72 的权威说明：固定多 Graph HP/RT CPU service、ownerless 内建
...
staging，以及精确 revision visible commit。`RunGroup`、最终 lifecycle admission/lease、
cancellation、supersession 与 scheduler ABI 替换仍是未来行为。
