...
Compute 不会按请求获取 worker 容量。首次选择内建 CPU 的 configuration、load 或 replacement
会把 `0` 解析为 `min(max(1, hardware_concurrency()), 8)`，或保留显式 `1..8`，随后冻结该数量、
启动一个固定 service pool，并从进程级 32-slot budget 保留一项 pool-lifetime reservation。
之后的零或相同请求保持幂等，冲突的正数请求会被拒绝。Graph load 只会预留 legacy HP/RT
scheduler owner 的合计需求。内建 `serial_debug` 计费为零；ABI v2 plugin scheduler 按解析后的
grant 计费，内建 GPU/heterogeneous scheduler 还会计入潜在 device worker。该 containment
约束固定 service worker 与 legacy Graph-owned scheduler worker，而不是 callback 数，也不是
compute 及其 operation 使用的所有 thread。
...
创建后续 submission。Legacy full-HP route 保留 owned scheduler callback。Tiled 操作可能产生
微任务，并减少选定 runtime 的 logical completion count。

在提交 ready work 前，选定的 legacy scheduler 或固定 service pool 已保留其过渡期 worker
reservation。运行中的 compute 因而不会取得新的 budget slot。Service 绝不会按 Run 调整大小；
它会注册隔离的 Run settlement state，并向共享 queue 提交 owned value。Scheduler inspection
与 replacement 和 compute 共用 per-graph `GraphStateExecutor` 边界。Replacement 会在一个
...
Worker containment 失败发生在首次 service configuration 或 legacy graph load/replacement，
而不是 ready-task loop。无效的超八请求、与固定 pool 冲突的正数请求，或未知 type 会映射为
`InvalidParameter`；过渡期进程 worker budget 耗尽时，`GraphErrc::ComputeError` 会通过
