...
`Kernel` 拥有多图 API。`GraphRuntime` 拥有一个图模型、每图 `GraphStateExecutor`、事件服务、
...
`ComputeService::ExecutionStrategy` 承载。新增 identity 与 QoS 仍只是私有 descriptor input，
不会改变 public Host request 或 plugin ABI。
...
固定 service thread 属于基础设施，不是 per-Run reservation。首次选择内建 CPU 的
configuration、load 或 replacement
会把 `0` 解析为 `min(max(1, hardware_concurrency()), 8)`，或保留显式 `1..8`，随后冻结该数量、
启动一个固定 service pool。之后的零或相同请求保持幂等，冲突的正数请求会被拒绝。发布 work
之前，每个内建 CPU Run 会从 service-owned Host ledger 原子预留完整的 CPU、retained-memory、
scratch、ready-entry 与 ready-byte vector。Graph load 使用同一个 ledger，仅预留 legacy HP/RT
scheduler owner 的 CPU 合计需求。内建 `serial_debug` 与 ownerless 内建 CPU Graph binding 在
load 时计费为零；ABI v2 plugin scheduler 按解析后的 CPU grant 计费，内建 GPU/heterogeneous
scheduler 还会把潜在 device worker 作为 CPU capacity 计费。该契约不声称覆盖 compute 及其
operation 使用的全部 thread。
...
fresh opaque id、session identity、target、单 domain intent、full 或 interactive quality、
显式 QoS，以及作为 submission revision 的当前 topology generation。该 topology value 只是
provenance，不是权威 graph-wide `GraphRevision` 或 commit predicate。Sequential、legacy
scheduler、内建 CPU 与 dirty 变体共享此边界。Full HP Run control 拥有 materialized plan 与
runner；每个真实 ready task 在 execution、dependency release、validation 与 commit 期间都会
...
metadata 会选择物理路径：内建 CPU HP、RT、connected-parameter preflight 与 dirty 阶段使用
固定注入 service，且没有 Graph-owned scheduler；serial、GPU 与 plugin route 使用已配置的
per-Graph `IScheduler` runtime。
...
materialize 为 dependency counter、ready value、operation variant 和临时结果槽。对于内建 CPU
route，dispatcher 会创建不可变、由 lease 支撑的 `ReadyTaskSubmission`，并把一个 initial
ready batch 提交到注入的 `ExecutionService`；dependent completion 会通过同一个 active Run
创建后续 submission，并进入同一个 active Run 和有界 store。Legacy full-HP route 保留 owned
scheduler callback。Tiled 操作可能产生微任务，并减少选定 runtime 的 logical completion count。

内建 CPU Run 发布前，service 会原子预留完整且经过检查的 CPU、retained-memory、scratch、
ready-entry 与 ready-byte vector。Initial 与 dependent entry 持有 child ready grant；worker
执行时会把该 authority 交换为 CPU/memory/scratch。Failure、queue purge 与成功 settlement 都会
精确释放容量。固定 pool 绝不会按 Run 调整大小。Legacy scheduler owner 则从同一个 ledger 保留
保守 CPU-slot reservation。Scheduler inspection 与 replacement 和 compute 共用 per-graph
...
result-slot storage 由 Run 拥有。Dispatcher 通过 scheduler task-runtime queue dispatch 已经
planned 的 work；它不会让 scheduler 拥有 dirty propagation、compute-task derivation 或 task
...
`RealTimeUpdate` HP child 会把该 buffer 存入自己的 Run，并且只在 RT sibling 已提交之后写入
可见 `GraphModel`。RT dirty worker 在写入 callback-local `RealtimeProxyWriteBuffer` 时保留
自己的 RT child lease，随后提交到 runtime-owned `RealtimeProxyGraph`。当前实现没有通用
graph-revision 或 interruptible commit policy；ADR 0003 和内核演进 roadmap 会把该已接受方向与
当前行为分开定义；ADR 0007 固定完整的 Run/revision/commit 竞态，而当前 child-Run 与
completion-isolation 切片只是其中一部分。Commit policy 在概念上仍与 `ComputeIntent` 分离，
因为 HP/RT intent 语义不定义可见性或中断。
...
RT，并通过 sibling commit gate 保证 HP 只在 RT proxy commit 成功后才修改 `GraphModel`。
内建 CPU child 共享固定 service；legacy route 使用各自的 intent scheduler。没有 parallel
domain 时，同一批 callback 会按 RT 后 HP 的顺序 inline 执行。每个 child 都发布自己的终态；
目标 aggregate `RunGroup` policy 尚未实现。
...
Configuration 与 legacy graph load/replacement admission 失败发生在 candidate 发布之前。
