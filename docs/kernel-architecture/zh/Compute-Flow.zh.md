...
TODO：优化后的局部 HP dirty planning 尚未覆盖每一个 `GlobalHighPrecision`
入口路径。应把 global compute 上的 dirty ROI 视为兼容性提示，它仍可能触发完整重算；
在该 TODO 关闭前，性能敏感调用方不应假设 HP work 会受 ROI 边界限制。
