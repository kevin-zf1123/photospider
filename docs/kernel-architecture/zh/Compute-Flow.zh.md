...
dirty source state，运行 propagation 刷新 `actual_dirty_region`，并唤醒 dispatcher。Scheduler 只接收带
epoch/generation metadata 和 optional scheduler-specific hint 的 ready task callback；它不接收 task
graph，不拥有 dirty control lane，node 也不拥有 compute-service dirty queue。
...
当前完整重算 fallback 仍是尚未接入优化 HP dirty planning 的入口路径的兼容行为。
...
`GlobalHighPrecision` plan 与 HP dirty snapshot，RT callback 使用 `RealTimeUpdate` plan 与
RT dirty snapshot。Dirty snapshot 会从该路径的 task graph 中裁剪或激活 update work set。
这样能让 `ComputeTaskPlanner` 聚焦稳定拓扑语义，并让未来新增 task pool 或 task mode 时继续复用同一 planner contract。
