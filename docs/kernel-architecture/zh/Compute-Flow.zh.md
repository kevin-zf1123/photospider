...
平台 context 和调度器实例。Embedded composition root 还会在 Kernel 之前创建一个私有 CPU
`ExecutionService`；Kernel 会把该 owner 注入每个 request-local `ComputeService`。
...
Graph-owned scheduler worker，而不是 callback 数，也不是 compute 及其 operation 使用的所有
thread。特别是，issue #68 注入的 CPU service pool 不由这个过渡期 ledger 计费；在 issue #69
删除重复 owner 前，仍被分配的 per-Graph CPU scheduler 保留既有计费。
...
request-owned `ComputeRun`。它捕获 fresh opaque id、session identity、target、
...
每个真实 ready task 在 execution、dependency release、validation 与 commit 期间都会保留
不可伪造的 Run lease 与 `(RunId, RunLocalTaskId)` identity。内建 CPU full HP 会把每个
dependency-ready task 包装为 move-only `ReadyTaskSubmission`，并使用注入 service 的单 Run CPU
batch。Serial、GPU 与 plugin full HP 保留 lease-backed callback 路径。显式 dirty HP 保留独立
的同步 borrowed-handle 路径。`RealTimeUpdate` 不创建 mixed Run 或 child Run；配对
Run/`RunGroup` settlement 仍是后续工作。
...
意图模型是正式的。`ComputeService` 仍是 compute facade 和 planning boundary。私有
scheduler-route metadata 会把已配置 scheduler owner 与物理路径配对：内建 CPU 的非 dirty
full HP 使用注入 service，其他 full-HP、dirty 与 realtime route 使用已配置 per-Graph
`IScheduler` task runtime。
...
在提交 ready work 前，选定的过渡期 scheduler 已完成全生命周期准入。运行中的 compute 因而
不会取得新的 ledger slot。当前 service 同样不执行 ledger admission；它会串行化一个 Run，
并把 concrete CPU runtime 重新配置为可信内建 grant 的精确值。Scheduler inspection 与
replacement 和 compute 共用 per-graph `GraphStateExecutor` 边界；replacement 不能与一串
compute callback 重叠，并且会在一个 transaction 中发布 scheduler ownership 与私有 route
metadata。Candidate planning、attach 或 start 失败会保留旧 scheduler、route 及其 compute
行为，只归还 candidate 容量。
