...
id: CRV-01C
parent_id: CRV-01
function: interpolate_linear_multi
operation_family: curve.interpolate_linear_multi
proposed_operation_keys:
  - curve.interpolate_linear_multi_strict
  - curve.interpolate_linear_multi_accelerated_apple_silicon
  - curve.interpolate_linear_multi_accelerated_x86_64
category: 01-numeric
kind: primitive
status: Proposed
...
# CRV-01C: interpolate_linear_multi
...
Clarification is complete; the target remains Proposed and unimplemented.
...
independently using the linear mathematical function. Preserve query order and
...
[CRV-01A operation](CRV-01A_interpolate_linear.md); do not overload one
...
domain policy, precision and zero handling as CRV-01A at query[i] using the
...
Conceptual public fixture: x=[0,1,3], y=[[0,10],[2,8],[4,4]],
query=[2,0.5,2] -> values=[[3,6],[1,9],[3,6]]. All three profiles match
independent exact per-column linear interpolation and direct rounding bits.
...
through Compiler/ExecutionContext, with actual build/run commands at implementation.
Apply scalar stride, cache, dirty, resource, cancellation and post-context owner
lifetime checks to the rank-2 output. No target runtime or performance test is
