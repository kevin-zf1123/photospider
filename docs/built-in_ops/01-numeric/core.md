...
| --- | --- | --- | --- |
| NUM-01 expression generator family | 单函数、动态 start/end/具名系数 → `values[N]`、`axis[3]` | `start,end,count` 含端点，支持反向；N=1 忽略 end；Float32/64 输出默认 Float64；strict 与两个 CPU 平台 accelerated 独立命名 | [具体规格](op_specs/NUM-01_sample_expression.md)，Proposed；values 按索引请求，axis 独立；当前实现、命令和验证见对应规格与 workflow README |
