...
exported package 或 external consumer build，或者专用 compile target。负责验证这类 nested-build
driver 的破坏性清理与配置布局行为的 safety regression，也使用相同调度分类。所有此类测试都携带
精确且稳定的 CTest 标签 `build-smoke`。
...
integration planning 则解析 `ctest --show-only=json-v1`，为每个带标签测试创建一个独立 matrix
job。因此，新增长期 build smoke 只需要注册 CTest 并添加相同 label，不需要修改 workflow 中的
测试名。Inventory malformed、duplicate、label 形状非法、带标签测试 disabled/commandless 或
带标签集合为空时，planning 会 fail closed。执行前 runner 会重新查询 inventory；所选名称 absent、
duplicate、disabled、commandless 或不再带标签时都会被拒绝。完成该精确 label 校验后，它只使用
经过校验的 CTest 数字索引选择测试，因此任意测试名字符都不会被 shell 或 regular expression 解释。

Published-image workflow 会在恢复同一份可复用 default producer 后扇出这些 entry。每个 CTest
注册保留自身 timeout 与 `RUN_SERIAL` 行为；每个 matrix item 另有独立 workflow timeout 与 result
artifact。Local-image fallback 只有一个 Docker-capable runner，因此读取同一份 NUL 分隔的规划
名称并顺序执行。Nested driver 必须继续使用彼此不重叠的 work directory，校验其接受的任何可复用
producer identity，并且在 cleanup 时不得跟随 symlink 或删除无关 symlink target。
...
- `ci/scripts/ci_routing_test.sh`：精确锁定两份 canonical `protected-ci-paths.if` 表达式；执行真实
  ...
  label-driven build-smoke matrix/full-CTest/fallback 路由、含换行路径 artifact，以及
...
- `ci/scripts/build_smoke_inventory.py` 及其 focused regression：严格解析 CTest JSON，生成确定性
  非空 matrix、安全 artifact key 与 NUL 分隔名称，并按精确索引执行。
- `ci/scripts/integration_plan.sh`：根据精确 label 发现 build smoke 并输出动态 matrix。
- `ci/scripts/build_integrity.sh`：构建 default producer profile，包括 required-target/full build
  与带标签 CTest inventory 校验。
- `ci/scripts/ctest_full.sh`：在排除精确 `build-smoke` label 后运行主 CTest suite。
- `ci/scripts/integration_suite.sh`：顺序执行 integration 行为检查，同时运行每个规划所得 build
  smoke、full CTest、CLI、propagation、plugin 和 scheduler 覆盖。
