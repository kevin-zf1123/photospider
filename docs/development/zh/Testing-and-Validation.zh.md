...
维护中的 driver 有两种互斥输入模式。本机 CTest 仍可传入 `--build`，并可选择配置 fresh
producer；shared CI DAG 则成对传入 `--installed-prefix` 与 `--producer-metadata`。Package-input
mode 禁止 `--build`、fresh producer configure、producer target build 与 `cmake --install`。已校验
role 只包含 installed tree、producer `CMakeCache.txt` 与配置所得 public-header inventory；下游 job
仍会完整保留 daemon help、package/export 与 archive-symbol 检查、全部 embedded/IPC/policy/
operation/OpenCV consumer compile-link-run probe，以及全部 missing/unknown component 负向检查。
恢复的 prefix、producer metadata 与临时 consumer work 两两隔离；cleanup 只拥有临时 work，不能
删除或改写两项已校验输入。
...
在 role attestation 前，独立 protected verifier 会把精确 candidate checkout 到一份不执行代码的 data root，并让它与精确 `workflow_commit` control-code root 及全部下载的 raw/control/artifact root 分离。它在使用前后绑定两份真实 `HEAD` commit 与 directory object，再把 raw bundle 重新绑定到受保护 route digest。它只执行 protected code，并且在不调用 CTest/CMake 的情况下，通过严格的 generated-command/property allowlist 解析归档中的精确 control graph。每个 include 都必须是字节级 canonical 的显式 `.cmake` path，绝不使用 module lookup；每个维护中的 subdirectory 也采用相同的 canonical POSIX 拼写。唯一接受的 assignment 是与该文件注册测试完全相等的惰性 local `<target>_TESTS` list；`PARENT_SCOPE` 与 `CACHE ... [FORCE]` 签名会在 equality 前失败。显式引用的空 command argument 仍作为数据保留，任何影响 search/interpreter 的 `set` 都会 fail closed。Raw dot、重复 separator 与尾随 separator 拼写会直接被拒绝，而不是 normalize 成 retained member。Parser 会保留每个 argument 的 quoted/unquoted/bracket kind 与 raw spelling；path directive 在 decode 前拒绝任何 lexical backslash，惰性 `set` 在 equality 前拒绝 unquoted value 中转义或未转义的 semicolon。Quoted/bracket semicolon value 保持一个 value，普通 command/filter escape 继续按已审查规则 decode。它要求 raw 完整 record、两份 targeted control/runtime closure record 与 control-file digest，以及只排除精确 `build-smoke` 后的普通 closure 全部相等。Unknown 或 side-effect-capable control、coverage reduction/addition/relabeling、identity/digest/path drift 或执行 candidate helper 都会在 attestation 与 readiness 前失败。结构化 root field 只能 normalize 精确 root 或 component descendant；prefix sibling、embedded/quoted/list ambiguity、含混 root topology、物理 CTest relocation 与不受限 string replacement 均 fail closed。
...
不完整 partition。每个 CTest 注册仍保留自己的 timeout 与 `RUN_SERIAL` 行为，每个 matrix item
另有独立 workflow timeout 与 result artifact。
...
缺失或重复，以及 auth、network、发生漂移的 not-found 或未知 inspect failure 都会使 registry 零写入。
`ci_routing_test.sh` 会另行锁定所有 ref 共享的一个 job-scoped repository/CI-image promotion lease、
使用 `cancel-in-progress: false` 保留排队、freshness-before-write 顺序、在 mutable creation 前执行
immutable SHA preflight、明确的 `promoted`/`superseded` output、superseded 时只写 immutable tag，
以及 stable-gate status handling；它不会把 GitHub queue 顺序当作 freshness evidence。该 lease 会
闭合 workflow-owned writer 之间的 race；测试与文档明确不对 out-of-band nonworkflow writer 宣称
GHCR tag compare-and-swap。

普通 full-CTest role 从完整的构建后 JSON inventory 派生，而不是依靠文件名 heuristic。它只排除
精确 `build-smoke` label，保存归一化 command/property，递归跟随非标准文件名与 generated include，
从每项测试的有效 working directory 解析相对 command、argument、`REQUIRED_FILES`、environment 与
environment-modification 路径而不整体复制该目录，并选择全部必需的 build-root executable、data
file、dynamic library、plugin 与 trust material。
Artifact 会拒绝 object、dependency file、无关 static library、link、special file、undeclared
member 及 digest/size drift。恢复到 canonical build path 后会再次执行 discovery；`_NOT_BUILT`、
缺失 include/executable/data/runtime input 或任何 inventory 变化都会在
`ctest --parallel ${CI_JOBS}` 运行并写出 JUnit 结果前失败。Fresh-build smoke 只接收 control role，
因此不会下载 producer object 或普通 runtime closure。

Installed-package consumer 会在 daemon/package/consumer 执行前后，独立按已校验 manifest 重新测量。
精确 member、size、SHA-256 digest 与 executable attribute 必须保持一致；新增、删除、改写或 mode
漂移都会失败。其 evidence artifact 只保留 consumer/verifier log 与可用的 attestation record，
绝不会再次携带恢复后的 package prefix 或 producer metadata tree。
...
`StaticProductConsumerSmoke` 只覆盖已校验的 producer/package input、可选 producer
configure/build/install、外部 `find_package`、public header compile/link/run、安装后的 export 与
依赖边界、平台 archive/link 行为和 multi-configuration target discovery；它的行为判定不得包含
Git identity、staged 或
...
每个共享 Linux ASan、TSan 与有界 fuzz job，以及手动 sanitizer job，都会从精确受保护 sparse checkout 在真实 host 测量其 reviewed `ubuntu-24.04` identity，且该动作发生在 candidate checkout 前。受保护 host wrapper 随后登录、拉取精确 digest-qualified image、移除 token，并在无 network、只读 root filesystem、彼此分离的只读 candidate/受保护 `ci`/inventory/identity mount，以及仅一个 mode-0700、fresh 且直属 `runner.temp` 的可写 work root 下运行 profile。拒绝跟随链接的 directory descriptor 会保留该 work root 的实际 device、inode、mode、UID 与 GID，并在 Docker 前立即重新测量。Docker `--user` 会把 container 绑定到测得的正数 owner pair，包括与 host process primary group 不同的 setgid-inherited GID；受保护 entrypoint 在 candidate configure/build/test 前验证 effective identity 与真实 create/read/remove work probe，且不使用 `0777`。同一个 retained host record 会进入 container 内的 platform preparation；job-container `ImageOS` 缺失、mutable tag、token forwarding 或 candidate-before-verifier 顺序均失败。
...
两个 reviewed Darwin rollout record 还会绑定 Android SDK CMake 3.31.5、其精确 GoogleTest module SHA-256、Homebrew LLVM 18.1.8 C/C++ path 与逐 version vcpkg commit。Preparation 会从唯一 canonical `cmake version` 行严格解析 locked semantic version，仅接受已审查的 Android SDK `-g<7 位小写十六进制>` vendor suffix，然后在 configure 前独立检查真实 CMake executable、报告的 `CMAKE_ROOT` 与 module byte。错误 semantic version、foreign suffix、control byte 或额外行都会失败。该 stage-safe selection 会生成当前 candidate-owned helper 可接受的 counter-suffixed GoogleTest include；在独立 candidate-owned helper task 落地前，hash-suffixed CMake 4.x name、foreign/duplicate/missing include 与 counter drift 均失败。Fuzz preparation 还会编译、链接并运行一个有界的 `fuzzer,address,undefined` probe，并把这些精确 LLVM path 传给 CMake；tool/version 或 probe drift 不会缩减 Darwin fuzz coverage。
...
Published-image 校验先于 `healthcheck.sh`；image-change contract 校验先于其静态 healthcheck。
...
- `ci/scripts/ctest_full.sh`：在排除精确 `build-smoke` label 后，以受控并行度、完整失败输出与 JUnit
  证据运行主 CTest suite。
- `ci/scripts/openexr_smoke_test.sh`：从精确且只含 cache 的 OpenEXR metadata role 运行 source-tree
  option-off smoke。
