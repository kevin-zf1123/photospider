...
Graph/cache 注入契约与 fake 不依赖可选 operation provider。独立的
`DependencyDisabledInstallSmoke` 覆盖完全省略 OpenCV discovery、并选择 unavailable production
codec 的完整 product profile。
...
默认完整 test suite 会注册 `test_stdlib_image_buffer_processing`；即使该 producer 使用 OpenCV，
它仍会直接编译标准库实现。该测试验证 clone independence、stride-safe 且确定性的 bilinear
border 行为、channel conversion 与 ROI copy。默认 CTest inventory 也包含
...
- `ci/scripts/integration_plan.sh`：发现 static-product 与 IPC-disabled build smoke capability。
- `ci/scripts/build_integrity.sh`：构建 default 与 IPC-disabled producer profile，包括必需
  target、全量 build 和 default CTest discovery。
- `ci/scripts/ctest_full.sh`：运行主 CTest suite，其中包含普通、长期维护的
  `DependencyDisabledInstallSmoke`。
- `ci/scripts/integration_suite.sh`：顺序执行 integration 行为检查；dependency-disabled 覆盖由
  完整 CTest 提供，同时运行规划所得 build smoke、CLI、propagation、plugin 和 scheduler 覆盖。
