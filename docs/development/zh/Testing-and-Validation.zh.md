...
- `ci/scripts/integration_plan.sh`：发现 static-product、IPC-disabled 与 dependency-disabled build
  smoke capability。
- `ci/scripts/build_integrity.sh`：执行 configure、必需 target 与全量 build，并完成 CTest
  discovery；同时提供只构建真实 kernel/product 的 `dependency-disabled` profile。
- `ci/scripts/ctest_full.sh`：运行主 CTest suite。
- `ci/scripts/integration_suite.sh`：顺序执行 integration 行为检查，包括 dependency-disabled
  build/install smoke、CLI、propagation、plugin 和 scheduler 覆盖。
