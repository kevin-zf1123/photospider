...
Pull-request local-image healthcheck 会在构建 head Dockerfile 或执行挂载的 workspace 前，从
base-repository URL 拉取目标分支并校验 event 的精确 base SHA，确保容器解析传入的
`CI_BASE_REF` 时不依赖 fork checkout 的 `origin`。
...
- `ci/scripts/change_classification.sh` 与
  ...
  回归矩阵。
- `ci/scripts/ci_routing_test.sh`：维护同仓库/fork 路由、稳定门禁、local-image 精确 base
  准备、image base、少见路径与 detector 失败传播契约。
