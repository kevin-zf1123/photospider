...
Pull-request published-image 与 local-image healthcheck job 都会在各自 job 内从
base-repository URL 拉取目标分支，并把 `CI_BASE_SHA` 校验为 event 的精确 base commit。
Published-image 校验先于 `healthcheck.sh`；local-image 校验先于构建 head Dockerfile 与执行挂载
workspace。Fetch 或解析失败时会在脚本使用 fallback base 选择前停止；已验证的精确 SHA 会作为
`CI_BASE_REF` 传入，不依赖 fork checkout 的 `origin`。
...
- `ci/scripts/ci_routing_test.sh`：精确锁定两份 canonical `protected-ci-paths.if` 表达式；执行真实
  stable-gate、fork-rejection 与 protected-path block；校验 published/local job-scoped exact
  base 顺序、含换行路径 artifact，以及 detector/reader/producer 失败传播。源码锁定不模拟
