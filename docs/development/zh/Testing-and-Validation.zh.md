...
Changed-path 清单使用 NUL 分隔的 Git 输出与父 shell 可见的临时文件。因此 `git diff` 失败时，
镜像检测或 healthcheck 静态范围检测会直接终止，不会输出假阴性路由。
...
- `ci/scripts/change_classification.sh` 与
  ...
  回归矩阵。
- `ci/scripts/ci_routing_test.sh`：维护同仓库/fork 路由、稳定门禁、image base、少见路径与
  detector 失败传播契约。
