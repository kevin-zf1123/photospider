...
Issue #69 至 #72 此后以一个由 Host 组合的 service、原子 resource vector、policy-aware
有界 ready storage 与 revision-safe staged publication，替换了内建 CPU execution 的
worker-only containment。Visible compute 现在会捕获完整的 request-owned state，并在
graph-state 之外执行；第二条有界串行 lane 会保持同一 Graph 的 compute 与 scheduler-owner
lifetime。Legacy serial/GPU/plugin scheduler 仍按 Graph 拥有 worker、queue、epoch 与 policy。
当前有界契约：
...
- 内置 serial 与 ownerless 内置 CPU Graph binding 计费为零，已注册 plugin 按解析后的
  CPU grant 计费，内置
  GPU/heterogeneous 再加一个潜在 device worker；
- 为每个 embedded Host 提供一个 CPU 维度默认为 32 的 ledger；原子 admission legacy HP+RT
  CPU-slot demand，并在 replacement 无法预留 candidate headroom 时保留旧 scheduler；
- 使用完整且经过 checked arithmetic 的 CPU、retained-memory、scratch、ready-entry 与
  ready-byte vector admission 每个内建 CPU Run，并让 initial/dependent work 通过同一个受
  entry/byte 约束的 store；
- 按 work 与 ready-byte quantum 对 dispatch 计费，维护分层 Graph/Run 公平性，让 ready work
  aging，保留 interactive headroom，并在有界 interactive burst 后保证 throughput 进展；
...
- 为每个 Graph 提供另一条具有相同上限的 lane，用于 compute/scheduler lifetime，而 graph-state
  继续作为唯一的 visible capture/mutation/commit 边界；
...
- 让 embedded close 先发布 Host marker、排空 marker 之前的同步 admission，再在等待 async
  placeholder 之前停止 compute-request admission，使满 FIFO producer 无法令 close 死锁；并且
- 在 graph-state 仍可用时排空 compute-request work，随后在 scheduler teardown 前排空
  graph-state；持久 close generation 能让旧 waiter 在 scheduler stop 失败后按依赖顺序重启两条
  lane 并为 retry 重新开放 admission 时仍正常完成。

默认 32 个 CPU slot 覆盖已 admission 的 Run callback 与已计数的 legacy scheduler-worker
owner。固定 `ExecutionService` thread 属于基础设施。Ledger 不计算各自具有独立“每 Graph 一个
worker”上限的 graph-state executor 或 compute-request executor，也不声称覆盖 operation 内部
thread、daemon/frontend worker、全部 OS thread，或尚未声明的 device/I/O/plugin resource。
Issue #70 已完全移除旧的
...
exact-revision commit，以及 RT-first 独立 child publication。Revision-aware preference、取消、
supersession 与用于替换 plugin 的 policy ABI 仍是后续工作。
...
当前截至 Issue #72 的基线会为每次非 realtime HP service call 创建且只创建一个私有 Run。Realtime
call 则会创建彼此分离的 HP `Full` 与 RT `Interactive` 子 Run；它尚未创建目标 `RunGroup`。
...
Scheduler-backed full HP work 会保留稳定且不可伪造的 lease，拥有 runner，并且只通过匹配的
...
当前内建 policy route；intent 与 quality 不会推断它们。`RunGroup`、cancellation 与 supersession
仍属于后续切片。
...
当前 Issue #72 `GraphRuntime` 拥有 `GraphModel`、graph-scoped runtime state、彼此分离的
...
process admission、`ResourceLedger`、`SchedulerPolicy` 或物理 scheduler instance。Run 可以持有
graph-lifetime lease，但不会反转这项所有权。

graph-state lane 只在捕获 immutable revision 和执行经过验证的 visible commit 时持有，不覆盖
长时间 planning/execution。私有 compute-request lane 当前会串行化同一 Graph 的 compute 与
scheduler lifetime；目标 registry/lifetime fence 仍是未来行为。
...
当前截至 Issue #72 的基线已实现共享 CPU/resource、policy 与 staged-commit 边界：
...
Run，同时隔离每个 Run 的 completion、first failure、trace routing 与 Host context。内建 CPU
binding 不再拥有 scheduler，而 legacy scheduler route 仍用于 serial、GPU 与 plugin execution。
Service 独占一个 Host 权威 ledger，在发布前原子 admission 每个完整 Run vector，要求 initial
...
Graph identity/revision validation 与 staged product publication 已是当前行为。Cancellation、
supersession 与 lifecycle registry 仍属于后续工作。
...
reservation/grant；后续 cancellation、close 与 shutdown 切片必须保持该契约。Capacity exhaustion
与 checked overflow 会在无 partial reservation、overcommit 或 silent clamping 的情况下失败。

`SchedulerPolicy` 是内部比较 seam，不是物理 executor 或 resource authority。当前 interactive 与
throughput 策略会排列不可变 ready descriptor；由 service 拥有的 store 保留每个物理 entry 与
Graph/Run fairness row。策略不拥有 worker、ready store、Run、Graph state、budget、reservation、
grant/token、native device handle、executor、completion route 或 lifecycle authority。
...
Latest-generation preference 仍属于 issue #74，revision-safe commit 自 #72 起已是当前行为，
cancellation 属于 #73，用于替换 scheduler 的 policy ABI 属于 #75。Larger quantum 与 device-utilization awareness
仍是后续 profile/device 目标；issue #71 不声称已实现它们。

当前拥有 worker 的 scheduler ABI 是过渡契约。未来 policy ABI 是破坏性替换，不保留永久
forwarding layer。

旧的 worker-only budget 已被完全移除，没有包装、重命名或别名。Worker-request resolution
现在只是纯 planning；Run 与 legacy scheduler owner 的全部 CPU admission authority 都来自唯一
...
完整目标会扩展该 predicate，要求 registry graph row 为 `Open`、Run 已注册且 graph-lifetime lease
有效、supersession generation 仍为 current，并且没有已接受的 cancellation/failure。
Supersession 会选择一个更新 generation，并请求取消此前匹配的 Run，但不会复用其 identity 或修改
其 plan。不可抢占 work 与外部 side effect 可以完成，但 stale、cancelled、failed 或 overdue output
不能 commit。
...
stale HP result 不会回滚 RT。Cancellation、graph-close 与 process-shutdown denial reason 仍属于
完整 gate contract 的后续切片。
...
该图无环。#72 已获准在 #67 后与 #68–#71 并行推进；#75 可在 #71 后与 #73–#74 并行推进。
该表固定所有权依赖，不固定实现算法。
