...
它们不提供 shared execution 或 fairness。当前 issue #68 切片在此前 Run lifetime 与 completion
隔离基础上，增加显式注入的 CPU `ExecutionService`、move-only ready submission，以及同时一个
完整 Run。它有意不替换这个过渡 ledger，也不删除 per-Graph scheduler owner：migrated service
pool 与仍被计费的 Graph-owned CPU pool 暂时共存。
[#69](https://github.com/kevin-zf1123/photospider/issues/69) 跟踪的 shared-executor 切片会删除
这种重复，并引入 multi-Graph/HP/RT CPU 共享，而不是把当前 ledger 当作最终 worker pool。
...
当前 issue #68 切片会为每次非 realtime HP service call 创建且只创建一个私有 Run。它捕获
...
`(RunId, RunLocalTaskId)` 路由 task failure。内建 CPU ready work 现在会作为 move-only
submission 跨越注入的单 Run `ExecutionService` 边界；legacy full HP 保留 owned callback，
独立的 dirty executor 路径仍会同步 drain borrowed handle。捕获的 topology generation 不是目标
权威 `GraphRevision` 或 commit predicate。配对 realtime Run/`RunGroup`、多 Graph 进程执行、
...
当前 issue #68 vertical slice 已实现该边界的第一部分：`EmbeddedHostState` 会在 Kernel 前创建一个
私有 CPU service，Kernel 再把它注入 request-local `ComputeService`。内建 CPU 的非 dirty
full-HP dispatch 只转移不可变、由 lease 支撑的 `ReadyTaskSubmission`，service 会按精确 planning
grant 串行化一个完整 Run。过渡期 per-Graph scheduler、固定 worker budget、serial/GPU/plugin/
dirty/RT execution 与 graph-state serialization 全部保留。Service 尚不拥有 admission、有界
ready storage、policy、revision validation、cancellation 或 lifecycle registry。
...
| [#68](https://github.com/kevin-zf1123/photospider/issues/68) | 当前 injected CPU-only service、一个 Run、ready-only input | #67 |
