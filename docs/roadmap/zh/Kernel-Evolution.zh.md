...
Issue #69 与 #70 此后以一个由 Host 组合的 service、原子 resource vector 和有界 ready storage，
替换了内建 CPU execution 的 worker-only containment。Visible compute 的整个 callback 仍保留
...
存放在有界 ready queue 时持有 entry/byte grant。Fairness 与 policy selection 仍属于后续工作。
...
当前 issue #70 基线会为每次非 realtime HP service call 创建且只创建一个私有 Run。Realtime
...
commit predicate。`RunGroup`、policy selection、revision-safe commit、cancellation 与
...
当前 issue #70 基线已实现共享 CPU/resource 边界：`EmbeddedHostState` 会在 Kernel 前以显式
limit 创建一个固定 pool 的 CPU service，Kernel 再把它注入 request-local `ComputeService`。
...
reservation/grant。Policy selection、revision validation、cancellation 与 lifecycle registry
仍属于后续工作。
