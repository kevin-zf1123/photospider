...
Reserved start 与用于替换 plugin 的 policy ABI 仍属于 Issue #75，registry-owned close/shutdown
cancellation 则仍属于 Issue #76。
...
与 supersession generation 成为当前行为；图中仍包含后续 registry、reserved-start、device、I/O
与 plugin 目标切片。
...
当前截至 Issue #74 的基线会为每次非 realtime HP service call 创建且只创建一个私有 Run。Realtime
...
以及 current-generation commit validation。Public cancellation control、reserved-start admission 与
lifecycle registry wiring 仍属于后续切片。
...
当前截至 Issue #74 的基线已实现共享 CPU/resource、policy 与 staged-commit 边界：
...
一个 pending owner，并要求 commit 匹配 current checked generation；public cancellation control、
reserved-start admission 与 lifecycle registry 仍属于后续工作。
...
自 #72 起已是当前行为，cooperative cancellation 自 #73 起已是当前行为，reserved start 与用于替换
scheduler 的 policy ABI 仍属于 #75。
...
Issue #74 会扩展该 predicate，要求 supersession generation 仍为 current。Supersession 会选择一个
更新 generation，
