...
Issue #69 至 #71 此后以一个由 Host 组合的 service、原子 resource vector 和 policy-aware
有界 ready storage，替换了内建 CPU execution 的 worker-only containment。Visible compute
的整个 callback 仍保留
graph-state lane；legacy serial/GPU/plugin scheduler 仍按 Graph 拥有 worker、queue、epoch 与
policy。当前有界契约：
...
owner。固定 `ExecutionService` thread 属于基础设施。Ledger 不计算具有独立“每 Graph 一个
worker”上限的 graph-state executor，也不声称覆盖 operation 内部 thread、daemon/frontend
worker、全部 OS thread，或尚未声明的 device/I/O/plugin resource。Issue #70 已完全移除旧的
...
进展。Revision-aware preference、取消、supersession 与用于替换 plugin 的 policy ABI 仍是后续工作。
...
当前 Issue #70 与 #71 基线会为每次非 realtime HP service call 创建且只创建一个私有 Run。Realtime
call 则会创建彼此分离的 HP `Full` 与 RT `Interactive` 子 Run；它尚未创建目标 `RunGroup`。
每个 Run 都会捕获 process-lifetime opaque id、session identity、只表示提交时拓扑的 revision、
target、intent、quality 与显式 QoS；拥有单调 phase 和 exact-once terminal state；并通过共享
...
再将其交换为 execution grant。捕获的 topology generation 不是目标权威 `GraphRevision` 或
commit predicate。显式 QoS class、deadline 与 weight 会进入当前内建 policy route；intent 与
quality 不会推断它们。`RunGroup`、revision-safe commit、cancellation 与 supersession 仍属于
后续切片。
...
`GraphRevision`、revision capture、串行化 commit validation/publication、graph event、稳定
graph-instance identity、graph-lifetime anchor，以及 platform/session metadata。
...
当前 Issue #70 与 #71 基线已实现共享 CPU/resource 与 policy 边界：`EmbeddedHostState` 会在
Kernel 前以显式 limit 创建一个固定 pool 的 CPU service，Kernel 再把它注入 request-local
`ComputeService`。
...
Graph/Run 公平性、deadline 偏好、aging、interactive headroom 与有界 throughput 进展。
Revision validation、cancellation、supersession 与 lifecycle registry 仍属于后续工作。
...
#73，用于替换 scheduler 的 policy ABI 属于 #75。Larger quantum 与 device-utilization awareness
...
Run 在 planning 前捕获一个 immutable `GraphRevision`。最小串行化 commit predicate 要求 registry
graph row 为 `Open`、Run 仍已注册且 graph-lifetime lease 有效、captured revision 与当前权威
revision 精确相等、supersession generation 仍为 current、没有已接受的 cancellation/failure，
并且 staged output 对该 Run 有效。
...
配对 RT/HP work 还会使用单调的 `Pending` / `RtCommitted` / `Denied` sibling gate。只有有效 RT
proxy publication 能开放它。RT failure/cancellation、Graph closing 或 process shutdown 可以
永久拒绝它。HP 只有在 `RtCommitted` 之后且自身 Run predicate 成功时，才能进入 `GraphModel`
commit；后续 HP failure/cancellation 不会回滚先获胜的 RT commit。
