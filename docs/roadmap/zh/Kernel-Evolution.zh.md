...
这 32 个 slot 只覆盖已计数的 CPU worker owner。每个 Kernel 的固定 `ExecutionService` pool
计费一次，仍然存在的 legacy Graph-owned scheduler worker 也会计费；但不计算具有独立
“每 Graph 一个 worker”上限的 graph-state executor，也不计算 operation 内部 thread、
daemon/frontend worker 或所有 OS thread。它们提供共享的内建 CPU execution，但不提供生产级
admission 或 fairness。当前 issue #69 切片会在这一个固定 pool 中直接执行来自多个 Graph 的
内建 HP 与 RT work，让这些 CPU binding 不再具有 per-Graph scheduler owner，并且仅保留过渡
ledger，以防 service pool 与 legacy scheduler owner 共同造成进程超额订阅。
[#70](https://github.com/kevin-zf1123/photospider/issues/70) 跟踪的生产级 admission、有界
ready store 与 `ResourceLedger` 切片将替换该 containment，而不是把它当作最终 resource model。
...
当前 issue #69 切片会为每次非 realtime HP service call 创建且只创建一个私有 Run。Realtime
...
注入的 multi-Run `ExecutionService` 边界。捕获的 topology generation 不是目标权威
`GraphRevision` 或 commit predicate。`RunGroup`、生产级 resource、revision-safe commit、
cancellation 与 supersession 仍属于后续切片。
...
当前 issue #69 vertical slice 已实现该共享 CPU 边界：`EmbeddedHostState` 会在 Kernel 前创建一个
固定 pool 的 CPU service，Kernel 再把它注入 request-local `ComputeService`。内建 CPU 的
full HP、full RT、standalone dirty HP/RT 与 preflight dispatch 都会转移不可变、由 lease
支撑且具有 owned callback context 的 `ReadyTaskSubmission`。Service 会并发执行多个 Run，
同时隔离每个 Run 的 completion、first failure、trace routing 与 Host context。内建 CPU
binding 不再拥有 scheduler，而 legacy scheduler route 仍用于 serial、GPU 与 plugin execution。
`SchedulerWorkerBudget` 会为 service pool 计费一次，同时计入任何 legacy scheduler owner。
Service 尚不拥有生产级 admission、有界 ready storage、policy、revision validation、
cancellation 或 lifecycle registry。
...
`SchedulerWorkerBudget` 同样是过渡契约。它当前会为固定 service pool 计费一次，同时计入
legacy Graph-owned scheduler worker。它会被移除，而不会包装、重命名或别名为目标
`ResourceLedger`。
...
| [#69](https://github.com/kevin-zf1123/photospider/issues/69) | 当前 shared multi-Graph/HP/RT CPU domain，且无 per-Graph CPU worker | #68 |
