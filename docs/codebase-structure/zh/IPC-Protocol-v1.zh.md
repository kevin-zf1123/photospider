...
result 与 release。当前 image-mode job 会把 output artifact 保持为 private，并让 nullable
`output` 返回 JSON null；output metadata 与 delivery lease 尚未路由。版本 1 不暴露 compute
cancellation、scheduler/plugin/event route、`daemon.shutdown`、TCP、Windows named pipe 或
`graph_cli --connect`。现有 `graph_cli` 继续创建 embedded Host，本地命令语义不变。
...
`OperationStatus`。本切片中 `output` 始终为 null，image-mode success 也不例外；private output
reference 与 filesystem identity 绝不会进入 JSON。Terminal release 成功时使用独立、stable 的
acknowledgement：`{"compute_id":"...","released":true}`。Task 3.3 不解释、也不使用
`delivery_id`；即使提供该字段，也会把它当作未知的 forward-compatible field 忽略。Task 3.4
才会为该字段赋予 lease-aware semantics。
...
`internal_error`。本切片中，只有 malformed params、submit/admission failure、absent job 与
premature result/release 属 top-level failure。Queue commit 后的任何异常都会使用 commit 前已
分配的 `internal_error` fallback，因此 worker 可继续执行后续 job。
...
下文的 private protected store 支撑。当前 wire result 会刻意隐藏它的 metadata；当前 release
会在不解释 `delivery_id` 的情况下删除 job ownership；即使提供该字段，也会被忽略，直到
task 3.4 为其赋予 lease-aware semantics。

## Private Protected Output Store

Wire surface 接受 image-mode compute，但尚不暴露 output metadata 或 delivery operation：其
stable nullable `output` field 仍为 null。Router 背后有一个 private `OutputStore`，并与 joined
compute publisher 和 socket lifecycle 绑定。它会在 Unix socket 与持久 lifecycle lock 已归属
之后、session/compute/client admission 之前启动；因此 startup failure 不会放入任何 request。
