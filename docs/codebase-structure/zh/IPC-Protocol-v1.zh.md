...
`ps::ipc::Client` 为全部 55 个 method 提供 owned call，且没有 public raw-JSON escape hatch。
...
- `scheduler.types` 使用有序 string `types`；
- `scheduler.loaded_plugins` 使用有序 diagnostic string `plugins`。
...
八个 scheduler control/discovery method 与现有 `scheduler.trace` observation route 共同构成
scheduler wire surface。Installed typed Client 暴露全部九个 scheduler route，聚合两个 stable
collection view，并让 trace 保持为一次 bounded non-destructive sequence-page call。全部九个
名称仍属于精确 55-method `daemon.version.methods` inventory。每个 params object 的 unknown
member 都会被忽略，以保持 forward compatibility。

六个 process-global method 不定义、不读取、也不解析 `session_id`：

- `scheduler.types` 使用公共 first-page/continuation control，在 `types` 中返回有序
  scheduler type string；
- `scheduler.description` 要求 `{"type":"scheduler_type"}`，并返回
  `{"type":"scheduler_type","description":"display text"}`；
- `scheduler.scan` 要求 `{"directories":["scheduler/plugin/directory"]}`，
  并通过 `{"loaded":N}` 返回精确的非负 Host count；
- `scheduler.load` 要求 `{"path":"scheduler/plugin/library"}`，并返回 `{}`；
- `scheduler.loaded_plugins` 使用公共 page control，在 `plugins` 中返回有序的
  copied diagnostic string；
- `scheduler.configure_defaults` 要求
  `{"hp_type":"hp","rt_type":"rt","worker_count":N}`，并返回 `{}`。

Scan directory array 是 required，可为空，最多包含 256 个 entry；每个 entry 必须是
...
返回的 scheduler name 必须是 nonempty、有效 UTF-8 且最多 1,024 byte；stats 只是
display-only text，必须是最多 4,096 byte 的有效 UTF-8，并可为空。返回 intent 必须
等于 request intent。违反这些 invariant 的 Host value 会成为 daemon `internal_error`，
绝不会返回 partial value。`scheduler.replace` 还需要 nonempty bounded `type`：
...
再将其 freeze 到 collection registry。Continuation 精确绑定 global method 与 offset，只读取
frozen value，且不调用 Host。所有通用 page、entry/byte quota、cursor、TTL 与 shutdown
规则保持不变。

每个 direct scheduler request 与每个 first-page Host access 都使用与 compute 及其他 routed
family 相同的 daemon Host mutex。Per-session info 与 replacement 还会在 opaque session admission
存活期内执行，同时 embedded Host 会把 scheduler copy/replacement 与 compute、graph close
串行化。Mutation 只调用一次，绝不 retry。成功的 scheduler DSO 在 client 断开与
graph session 之间仍由 process owner 持有；JSON 不会暴露 scheduler、factory、registry、loader、
callback、DSO handle 或 mutable ownership token。`scheduler.trace` 保留其独立的 bounded、
