...
每个 remote failure 都有 string `domain`、signed integer `code`、string `name` 与 diagnostic
`message`；message 不是 branching contract。
...
本地 connect/read/write/peer failure 使用 `IpcErrorDomain::Transport`，绝不转换成 graph IO
error；daemon 不能发送 transport domain。未知的未来 remote numeric code/name 仍会保留在
`IpcStatus` 中。Allocation failure 可以传播 `std::bad_alloc`；其他 recoverable failure 均返回
status。
...
- `inspect.node` 要求 integer `node_id`，返回 `{session_id, node}`；
- `inspect.dependency_tree` 接受 optional integer/null `node_id` 与 optional boolean
  `include_metadata`，然后返回 `{session_id, ...flattened tree fields}`。
