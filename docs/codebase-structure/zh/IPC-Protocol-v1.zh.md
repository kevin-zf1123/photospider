...
`IpcErrorDomain` 是稳定的顶层分类。Error 来源为：
...
error；daemon 不能发送 transport domain。Transport domain 是可编程稳定区分的分类，但其本地
numeric code/name 只是诊断类别；client 不得依赖或持久化某个长期 Transport code/name mapping。
未知的未来 remote numeric code/name 仍会保留在 `IpcStatus` 中。Allocation failure 可以传播
`std::bad_alloc`；其他 recoverable failure 均返回 status。
