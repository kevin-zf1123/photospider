...
Client 当前不暴露这些 plugin method。Scheduler-plugin discovery/default与
per-session scheduler inspection/replacement 同样只在 daemon router boundary 可用；installed
typed Client 不暴露它们。Bounded `events.drain` 与 `scheduler.trace` observation route
仍只在 daemon router boundary 可用。现有 `graph_cli` 继续创建 embedded Host，
本地命令语义不变。
