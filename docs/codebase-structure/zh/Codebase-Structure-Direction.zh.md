...
cursor page 聚合为完整的 Host-shaped value。完整 IPC-backed Host、client-side artifact mapping
与最终 package-consumer contract 仍在进行中；plugin SDK 遵循下文记录的 extension contract。
...
| `photospider_ipc_client` | 可安装 static typed Unix IPC client。 | 为 version 1 全部 55 个方法实现 typed owned call，包括严格 job/output/page 验证与 stable collection 聚合；不链接 backend，也不暴露 JSON/POSIX type。 |
...
7. **进行中的 IPC version 1 complete-Host slice：** Installable typed client 为全部 55 个
   method 提供 owned call，严格验证 common 与 method-specific result shape，对每个 status 或
   mutation RPC 只尝试一次，聚合全部 private stable cursor page，并保留 output/delivery lease
   metadata。完整 installed IPC Host、安全 client-side artifact mapping 与最终 package-consumer
   surface 仍不可用；cancellation 也不可用。
