...
event/trace observation router slice。
完整 installed IPC Host、剩余 Host method family、最终 internal target 形态与过渡性 plugin SDK
仍需要后续迁移阶段完成。
...
| scheduler | `scheduler.trace` 与 scheduler control name | Bounded trace paging 已通过 Host 路由，但要等 task 3.8 的精确 table 落地后才会广告。Scheduler control 尚未路由。 |
| plugins | operation-plugin control 与 view | 不路由、也不广告 plugin method；process ownership 仍属于 Host。 |
| events | `events.drain` | Bounded destructive event drain 已通过 Host 路由，但要等 task 3.8 的精确 table 落地后才会广告。 |
...
7. **进行中的 IPC version 1 complete-Host slice：** Installable typed client 仍只暴露原有
   ...
   destructive compute event 与 bounded non-destructive scheduler trace。剩余 plugin/
   scheduler-control route 与完整 installed IPC Host 属后续 issue-#37 task；cancellation 仍明确
   不在范围内。
...
- 将 non-installed `ipc_output_fixture_daemon` 只作为该 integration test 的 dependency，而不为
  ...
  提前实现后续 plugin route 的前提下，覆盖 protected image delivery、bounded observation
  multiclient behavior 与 restart cleanup。它的
