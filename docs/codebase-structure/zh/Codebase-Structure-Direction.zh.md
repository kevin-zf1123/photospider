...
version 1 daemon/IPC graph、inspection、polling-compute 与 protected image-output router slice。
...
7. **进行中的 IPC version 1 complete-Host slice：** Installable typed client 仍只暴露原有
   ...
   polling compute，以及带 stable delivery lease 的 protected metadata-only image result。剩余
   event/plugin/scheduler route 与完整 installed IPC Host 属后续 issue-#37 task；cancellation 仍
   明确不在范围内。
...
- 将 non-installed `ipc_output_fixture_daemon` 只作为该 integration test 的 dependency，而不为
  ...
  提前实现后续 plugin route 的前提下，覆盖 protected image delivery 与 restart cleanup。它的
