...
7. **已完成 task 4.4 IPC Host adapter、制品与 package slice：** Installable typed
   ...
   unmap 和 close。Installed package 门禁会编译每个 public header，再把一个 IPC-only external
   consumer 链接到导出的 adapter archive。该 consumer 覆盖每个 Client lifecycle symbol、全部
   55 个 typed Client call、`create_ipc_host` 与全部 53 个 Host virtual 引用；export 只允许
   `Threads::Threads`，并拒绝 backend、JSON、server-internal、object、source-tree 或
   POSIX-private 泄漏。Cancellation 仍不可用。
