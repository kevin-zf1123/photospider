...
- 当前 issue #70 Run/service 实现位于
  ...
  admission；initial 与 dependent work 都会在 child grant 下进入同一个 entry/byte-bounded ready
  store，worker 再将其交换为 execution grant。Legacy scheduler route 保留 owned callback，
  并从同一个 service ledger 获取 CPU reservation。
...
Host 权威 ledger 与 move-only reservation/grant 实现放在 `src/lib/runtime/`。这些 implementation
owner 都不会成为 public Host、SDK 或 IPC type。
...
- 当前 `ExecutionService` 拥有一个固定的内建 CPU worker pool、一个 Host 权威 ledger、
  受 entry/byte 约束的 high/normal ready store、checked full-vector Run admission、并发的
  multi-Graph Run、exact reservation/grant release，以及按 Run 隔离的 completion、
  first-failure、trace 与 Host-context routing；目标会进一步增加可信 policy validation 与
  通用 resource execution；
...
随后 #71 增加内建 policy；#72–#74 增加 revision、cancellation 与 supersession；#75 替换拥有
worker 的 ABI；#76 收束 lifecycle 与 telemetry 不变量。权威的无环依赖表位于
