...
supersession 已是当前布局；reserved-start admission 与 scheduler ABI replacement（#75），以及最终
...
contention 与 RT-denies-HP 契约现在均已成为当前行为。#74 现在增加 latest-wins supersession、
realtime `RunGroup` 与 current-generation commit authority；#75 替换拥有 worker 的 ABI，#76 收束
registry、graph-close/process-shutdown 与 telemetry 不变量。权威的无环
依赖表位于
