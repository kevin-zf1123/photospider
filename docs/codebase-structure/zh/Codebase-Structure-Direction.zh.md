...
执行一次 unmap 和 close。最终 package-consumer contract 仍在进行中；plugin SDK 遵循下文
...
7. **已完成 task 4.3 IPC Host adapter 与制品所有权 slice：** Installable typed client 为全部 55 个 method
   ...
   unmap 和 close。最终 installed package-consumer 覆盖仍属于 task 4.4。Cancellation 仍不可用。
