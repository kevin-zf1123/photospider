...
- 当前 issue #67 Run 实现只位于 `src/lib/compute/compute_run.*`。`Kernel` 把 session identity
  与显式默认 QoS 传给私有 service request；`ComputeService` 为每次非 realtime HP call 创建一个
  Run。共享 Run control 会持有 full-plan/temporary storage 或 standalone dirty-HP staging
  storage，直到唯一 terminal publication。Scheduler-backed full HP work 会保留不可伪造的
  lease，并按 `(RunId, RunLocalTaskId)` 路由 owned-callback failure。Installed header、
  Host value、operation ABI 与 scheduler ABI 都不会命名这个私有对象。
...
固定完整的进程执行所有权。其 issue #67 私有 HP Run 源码、稳定 lease 与 full-HP completion
隔离切片现在已经位于当前 `src/lib/compute/`；配对 Run、process service composition、
resource ownership 与 revision-safe commit 仍是目标布局。产品 composition root 后续会根据
进程配置构造一个显式 `ExecutionService`，并把它注入参与其中的 backend owner；它不会使用
static singleton。
...
该所有权目标不选择新的源码目录、build target、queue implementation 或 plugin ABI shape；这些选择
属于对应 implementation slice。当前拥有 worker 的 scheduler SDK 与 `SchedulerWorkerBudget`
会作为完整迁移被移除；二者都不会保留在永久 compatibility wrapper 后方。
...
containment。Per-Graph worker 与过渡性的 process function-static
`SchedulerWorkerBudget::process()` owner 会继续存在，直到
[ADR 0007](../../adr/zh/0007-compute-runs-and-process-execution-have-separate-owners.zh.md)
和 issue #68/#69 引入 injected process-owned `ExecutionService` 与 shared executor。随后 #70/#71
增加最终的 Host 权威 ledger、有界 ready store 与内建 policy；#72–#74 增加 revision、cancellation
