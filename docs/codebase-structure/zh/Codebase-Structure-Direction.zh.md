...
Host virtual。Image mode 当前会严格重新校验同用户 regular file，并在 lease-aware release 前通过
`pread` 生成独立 heap copy；这一 task 4.2 过渡实现会在 task 4.3 中替换为 read-only
mmap/shared-deleter ownership。最终 package-consumer contract 仍在进行中；plugin SDK 遵循下文
记录的 extension contract。
...
7. **已完成 task 4.2 IPC Host adapter slice：** Installable typed client 为全部 55 个 method
   ...
   embedded execution。当前 image consumer 会安全校验并通过 `pread` 生成 owned copy，再执行
   lease-aware release；read-only mmap/shared-deleter ownership 与最终 installed package-consumer
   覆盖仍分别属于 task 4.3/4.4。Cancellation 仍不可用。
