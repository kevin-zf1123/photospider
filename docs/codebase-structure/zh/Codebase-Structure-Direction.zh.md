...
| scheduler | `scheduler.types`、`scheduler.description`、`scheduler.scan`、`scheduler.load`、`scheduler.loaded_plugins`、`scheduler.configure_defaults`、`scheduler.info`、`scheduler.replace`、`scheduler.trace` | 只通过匹配 Host call 实现。Discovery/default control 是 process-global；info/replacement 使用 opaque session admission，并与 compute/close 共享 graph-state 串行化。Type/plugin view 是有序 stable cursor snapshot，trace paging 保持 bounded 与 non-destructive，JSON 不包含 scheduler/plugin ownership。当前精确的八名称 `daemon.version.methods` 子集不广告这些名称。 |
| plugins | `plugins.load_report`、`plugins.unload_all`、`plugins.seed_builtins`、`plugins.ops_sources`、`plugins.ops_combined_keys`、`plugins.ops_combined_sources` | 只通过匹配 Host call 实现。Control 是 process-global，view 是按 key 排序的 stable cursor snapshot；client 断开或 Host-adapter 销毁不会 unload 成功的 DSO。当前精确的八名称 `daemon.version.methods` 子集不广告这些名称。 |
| events | `events.drain` | Bounded destructive event drain 已通过 Host 路由。当前精确的八名称 `daemon.version.methods` 子集不广告此名称。 |
