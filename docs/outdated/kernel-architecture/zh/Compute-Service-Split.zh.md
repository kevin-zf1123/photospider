...
cache migration 或 planner plugin change 中处理的工作。

## 当前问题

`ComputeService` 是经过 public Host 边界后才会到达的内部 compute facade，但它的实现
同时包含依赖解析、缓存策略、monolithic 与 tiled 分发、脏区状态规划、compute task 推导、
scheduler task-runtime dispatch 协调、计时、benchmark 事件和输出 debug 元数据。

拆分应先保持行为不变。它不是重写图引擎，也不是插件 ABI 变更。

compute path 现在在每一层边界都使用结构化请求对象。Frontend 把 public
`ps::HostComputeRequest` 提交给 `ps::Host`；embedded Host adapter 再把这个 value 转换为
内部 `Kernel::ComputeRequest`。`ComputeService::Request` 随后只接收 target node、cache、
...
| `ComputeCachePolicy` | 集中 HP/RT 缓存选择。 | 已在 `src/lib/compute/request/compute_cache_policy.*` 实现 |
| `NodeInputResolver` | 构建运行时参数并收集已就绪图像输入。 | 已在 `src/lib/compute/dirty/node_input_resolver.*` 实现 |
| `NodeExecutor` | 一致地执行 monolithic 与 tiled 算子。 | 已在 `src/lib/compute/dirty/node_executor.*` 实现 |
| `DirtyRegionPlanner` | 基于 node-local dirty report 和算子 propagation 构建图级脏区状态。 | 已在 `src/lib/compute/dirty/dirty_region_planner.*` 实现 |
...
| `FullTaskGraphExpander` | 将原始 graph 展开为一个 compute domain 的完整 node/tile `FullTaskGraph`，不使用请求目标、cache 状态或 dirty snapshot。 | 已在 `src/lib/compute/dispatch/task_graph_planning.*` 实现 |
| `NodeCacheTaskGraphPruner` | 将 `FullTaskGraph` 裁剪到请求目标/依赖锥，并记录所选节点的 cache 可用性。 | 已在 `src/lib/compute/dispatch/task_graph_planning.*` 实现 |
| `ComputeDispatchPlanBuilder` | 构建并记录 scheduler-backed dispatcher execution 使用的 cache-pruned high-precision plan。 | 已在 `src/lib/compute/dispatch/compute_dispatch_plan_builder.*` 实现 |
| `TaskPopulationStrategy` 和 task population helpers | 填充 graph-backed 或 graphless planned task record 以及 task dependency；dirty snapshot 不得用来创建新的 task shape。 | 已在 `src/lib/compute/dispatch/task_population_strategy.*` 和 `task_graph_planning.*` 实现 |
| `DirtySnapshotTaskGraphPruner` | 将 `DirtyRegionSnapshot` 应用于 node/cache-pruned `ComputeTaskGraph`，并 materialize 活跃的 `DirtyUpdateWorkSet`。 | 已在 `src/lib/compute/dispatch/task_graph_planning.*` 实现；plugin ABI 仍是 TODO |
| `IntentUpdateCoordinator` | 协调 `GlobalHighPrecision` 与 `RealTimeUpdate` intent 语义，包括与执行模式无关的 realtime HP/RT 双路径行为。 | 已在 `src/lib/compute/dirty/intent_update_coordinator.*` 实现 |
| Connected-parameter preflight | 在依赖它的 parameter、extent、ROI 和 task planning 前稳定同一个 request-local HP producer closure；并行时使用 scheduler initial-handle batch。 | 已在 `src/lib/compute/dirty/dirty_update_executor.*` 实现，并由 `ComputeService` 协调 |
| `ComputeTaskDispatcher` | 执行 node/cache-pruned task graph 语义：收集 source task、检查 task-graph readiness、通过 `SchedulerTaskRuntime` dispatch ready task，并提交结果。 | 已在 `src/lib/compute/dispatch/compute_task_dispatcher.*` 实现 |
| `TaskSubmissionPlan` 和 `dispatch_planned_tasks` | 将 cache-pruned plan 转为一次 dispatcher 调用所需的 scheduler closure、dependency counter、ready handle 和 empty-plan validation。 | 已在 `src/lib/compute/dispatch/compute_task_submission.*` 实现 |
| `ComputeMetricsRecorder` | 集中事件、计时、benchmark 事件和 debug 元数据。 | 已在 `src/lib/compute/request/compute_metrics_recorder.*` 实现 |
...
的 `DirtyRegionSnapshot` 暴露状态。`ComputeService` 会在 graph 上保存 inspection 摘要；内部
`InteractionService` 把摘要返回给 embedded Host adapter，后者复制成 public Host value，供
frontend/debug 查询。
...
当前面向 production 的 lifecycle 写入通过 public `ps::Host` 的 begin/update/end dirty-source
方法进入。embedded adapter 将其转换到内部 `Kernel` / `InteractionService` 方法；这些方法通过
`GraphStateExecutor` 串行化 graph-state mutation，并将 node/frontend lifecycle event 路由到
`DirtyControlLane`。该 lane 复用 `DirtyRegionPlanner` 更新 membership、lifecycle 和 actual dirty
ROI。`dirty_updating_count` 存在于内部 `DirtyRegionSnapshot`，并复制到
`DirtyControlLaneResult`；wakeup/cutoff 决策只存在于 `DirtyControlLaneResult`。这三项都仍是内部
evidence，由 compute-service work-set materialization 消费。embedded adapter 调用返回 snapshot 的内部
lifecycle 方法，并且只复制 public dirty snapshot 字段。Frontend event subscription 和可复用
request-plan coalescing 仍是后续工作。
...
request-local dirty `TaskExecutor` handle，然后调用内部 class 的 public C++ static
`ComputeTaskDispatcher::submit_dirty_ready_tasks_source_first` helper 作为 source-first
submission 边界。这里的 public C++ access 只允许 private backend translation unit 之间调用，
并不表示 product public API。运行期依赖计数器、task 引用计数和 ready queue 在
compute-service dispatcher state 内拥有。Scheduler 接收带 scheduler priority 和 completion
accounting 的具体 ready task handle；它不接收 task graph，不拥有 dirty-state lookup 或
...
Dependency-tree inspect 是结构化的：`GraphInspectService` 基于 topology adjacency 构建
dependency-tree snapshot；内部 `Kernel`/`InteractionService` 将其返回给 embedded Host adapter；
adapter 复制出 public Host snapshot，再由 CLI/TUI/frontend code 渲染人类可读文本。

## Host 与内部交互边界

`ps::Host` 是 CLI/TUI 与 embedded frontend 使用的 public interface。`InteractionService` 是
embedded Host adapter 与 `Kernel` 之间的内部 wrapper；frontend code 不会直接包含或调用它。
Dirty-region 边界通过 Host 暴露复制后的图级 inspection 和 lifecycle value；Host 与
`InteractionService` 都不是 dirty-region generation 或 propagation 的权威来源。

在内部，`DirtyRegionSnapshot` 保留 `dirty_updating_count`；`DirtyControlLaneResult` 会复制该计数，
并增加 dispatcher wakeup intent 与 downstream-completion cutoff 决策。只有后两项决策是 control
result 特有字段；这些内部字段都不跨越 public 边界。`InteractionService` 为 public lifecycle 方法把当前 dirty snapshot 返回给 embedded adapter；
Host 只复制 `DirtyRegionInspectionSnapshot` 实际具有的字段：generation、source lifecycle/ROI、
dirty tile、monolithic region、传播后的 ROI 与 edge mapping。Host 还会返回结构化
dependency-tree 与 graph-inspection snapshot，使 frontend 可以先解析图结构，再选择展示格式。

Public Host 异常边界会将可恢复的 graph、parser、filesystem、标准与未知 backend failure 转换为
`OperationStatus` 或 `Result<T>` failure。资源耗尽有意采用不同语义：每个非析构 Host method 都
可能传播 `std::bad_alloc`，async compute future 也可能在被消费时重新抛出该异常。可安装 Doxygen
契约逐方法记录这一行为；caller 不得把 Host 视为无条件 non-throwing。
...
TODO：设计缺失的 node 到 backend realtime dirty update interface。该 interface 必须允许 node
提供 dirty-region lifecycle event、realtime update event 和 update request，同时保持内部
`InteractionService` 与 dirty-region generation、compute scheduling ownership 分离；frontend
交付仍由 public Host 负责。
