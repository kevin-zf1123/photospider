// FILE: apps/graph_cli/src/run_repl.cpp
...
#include "graph_cli/cli_history.hpp"
#include "graph_cli/input_match_state.hpp"  // NOLINT(build/include_subdir)
#include "graph_cli/process_command.hpp"
#include "graph_cli/terminal_input.hpp"
...
void run_repl(ps::Host& svc, CliConfig& config,
  ...
  auto redraw_line =
      [&](const std::vector<std::string>& options_to_display = {}) {
  ...
  redraw_line_impl = [&](const std::vector<std::string>&) {
    ...
    if (completion_state.IsActive()) {
      size_t start_idx = 0;
      if (completion_state.original_cursor_pos >=
          completion_state.original_prefix.length())
        start_idx = completion_state.original_cursor_pos -
                    completion_state.original_prefix.length();
      ...
      std::cout << line_buffer.substr(0, left_len) << "\x1B[7m"
                << line_buffer.substr(left_len, mid_len) << "\x1B[0m"
                << line_buffer.substr(left_len + mid_len) << std::flush;
    } else {
  ...
  std::cout << "Photospider dynamic graph shell (decoupled). Type 'help' for "
               "commands.\n";
  ...
  while (true) {
    ...
    switch (key) {
      case ps::ENTER: {
        ...
        bool continue_repl =
            process_command(line_buffer, svc, current_graph, modified, config);
        completer.SetCurrentGraph(current_graph);
        term_input.SetRaw();
        if (!continue_repl) {
          return;
      ...
      case ps::CTRL_C: {
        if (line_buffer.empty()) {
          std::cout << "\r\n(To exit, type 'exit' or press Ctrl+C again on an "
                       "empty line)\r\n";
          redraw_line();
      ...
      case ps::UP: {
        if (!history_match_state.active) {
          history_match_state.Begin(line_buffer.substr(0, cursor_pos),
                                    cursor_pos);
        }
      ...
      case ps::TAB: {
        completer.SetCurrentGraph(current_graph);
        if (completion_state.IsActive()) {
          completion_state.current_index =
              (completion_state.current_index + 1) %
              completion_state.options.size();
          size_t token_start = completion_state.original_cursor_pos -
                               completion_state.original_prefix.length();
