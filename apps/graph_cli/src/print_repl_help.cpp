// FILE: apps/graph_cli/src/print_repl_help.cpp
...
void print_repl_help(const CliConfig& config) {
  std::cout
      ...
      << "  load <name> [yaml]\n"
      << "    Load a graph (from sessions/<name>/content.yaml if no yaml "
         "provided).\n"
      << "  load <yaml>\n"
      << "    Load YAML into the current session; if none, into [default].\n\n"

      << "  switch <name> [c]\n"
      << "    Switch current graph. With 'c', copy current session to <name> "
         "and switch.\n\n"
      ...
      << "  clear-cache [m|d|md]\n"
      << "    Clear memory/disk/both caches. Default: '"
      << config.default_cache_clear_arg << "'.\n\n"

      << "  free\n"
      << "    Free memory used by non-essential intermediate nodes.\n\n"

      << "  compute <id|all> [force] [force-deep] [parallel] [t] [tl [path]] "
         "[m] [nosave]\n"
      ...
      << "      m | -m:    Mute node result output (timers still print when "
         "enabled).\n"
      ...
      << "  save <id> <output> <file> <uint8|uint16> "
