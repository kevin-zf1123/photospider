...
std::filesystem::path CliHistory::GetHistoryFilePath() const {
  ...
  if (home == nullptr) {
    struct passwd* pw = getpwuid(
        getuid());  // NOLINT(runtime/threadsafe_fn): single CLI thread.
    if (pw) {
      home = pw->pw_dir;
    }
  }
...
void CliHistory::Save() const {
  ...
  if (!file) {
    std::cerr << "Warning: Could not save command history to "
              << history_file_path_ << std::endl;
    return;
  }
