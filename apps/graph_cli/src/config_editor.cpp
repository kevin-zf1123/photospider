...
#include "graph_cli/tui_editor.hpp"

using namespace ftxui;  // NOLINT(build/namespaces)
using namespace ps;     // NOLINT(build/namespaces)
namespace fs = std::filesystem;

// Local editor UI (moved from apps/graph_cli/main.cpp), trimmed for
// encapsulation.
  ...
  void Run() override {
    ...
    auto component = Renderer(main_container, [&] {
      ...
      for (size_t i = 0; i < editable_lines_.size(); ++i) {
        ...
        if (mode_ == Mode::Edit && selected_ == static_cast<int>(i)) {
          ...
          if (line.is_radio)
            display_element =
                text((*line.radio_options)[*line.radio_selected_idx]);
          else if (line.is_toggle && line.toggle_ptr)
            display_element = text((*line.toggle_ptr) ? "[x]" : "[ ]");
    ...
    component |= CatchEvent([&](Event event) {
      if (mode_ == Mode::Command) {
        ...
        if (event == Event::Backspace && !command_buffer_.empty()) {
        ...
        if (editable_lines_[selected_].is_radio) {
          ...
          auto is_path_like = [&]() {
            ...
            return lbl.find("path") != std::string::npos ||
                   lbl.find("file") != std::string::npos ||
                   lbl.find("dir") != std::string::npos;
          };
 ...
 private:
  void SyncModelToUiState() {
    ...
    selected_cache_precision_idx_ =
        find_index(cache_precision_entries_, original_config_.cache_precision);
    selected_print_mode_idx_ =
        find_index(print_mode_entries_, original_config_.default_print_mode);
    selected_ops_list_mode_idx_ = find_index(
        ops_list_mode_entries_, original_config_.default_ops_list_mode);
    selected_path_mode_idx_ =
        find_index(path_mode_entries_, original_config_.ops_plugin_path_mode);
    selected_cache_clear_idx_ = find_index(
        cache_clear_entries_, original_config_.default_cache_clear_arg);
    selected_exit_sync_idx_ =
        find_index(exit_sync_entries_, original_config_.exit_prompt_sync
                                           ? std::string("true")
                                           : std::string("false"));
    selected_config_save_idx_ =
        find_index(config_save_entries_, original_config_.config_save_behavior);
    selected_editor_save_idx_ =
        find_index(editor_save_entries_, original_config_.editor_save_behavior);

    // Parse traversal defaults into UI state: tree radio + cache checkboxes +
    // check action
    selected_traversal_tree_idx_ = 2;  // no_tree by default
    traversal_cache_m_ = false;
    traversal_cache_d_ = false;
    ...
    compute_force_ = compute_force_deep_ = compute_parallel_ =
        compute_timer_console_ = compute_timer_log_ = compute_mute_ = false;
    {
  ...
  bool SyncUiStateToModel() {
    ...
    original_config_.cache_precision =
        cache_precision_entries_[selected_cache_precision_idx_];
    original_config_.default_print_mode =
        print_mode_entries_[selected_print_mode_idx_];
    original_config_.default_ops_list_mode =
        ops_list_mode_entries_[selected_ops_list_mode_idx_];
    original_config_.ops_plugin_path_mode =
        path_mode_entries_[selected_path_mode_idx_];
    original_config_.default_cache_clear_arg =
        cache_clear_entries_[selected_cache_clear_idx_];
    original_config_.exit_prompt_sync =
        (exit_sync_entries_[selected_exit_sync_idx_] == "true");
    original_config_.config_save_behavior =
  ...
  void RebuildLineView() {
    ...
    auto add_line = [&](const std::string& label, std::string* ptr) {
      editable_lines_.push_back({label, ptr});
    };
    auto add_radio_line = [&](const std::string& label,
                              std::vector<std::string>* opts, int* idx) {
      editable_lines_.push_back({label, nullptr, true, opts, idx});
    };
    auto add_toggle_line = [&](const std::string& label, bool* ptr) {
      editable_lines_.push_back({label, nullptr, false, nullptr, nullptr,
                                 nullptr, nullptr, true, ptr});
    };
    ...
    add_radio_line("cache_precision", &cache_precision_entries_,
                   &selected_cache_precision_idx_);
    // Traversal defaults: tree mode + cache flags + check action
    add_radio_line("traversal_tree_mode", &traversal_tree_entries_,
                   &selected_traversal_tree_idx_);
    add_toggle_line("traversal_cache_memory(m)", &traversal_cache_m_);
    add_toggle_line("traversal_cache_disk(d)", &traversal_cache_d_);
    add_radio_line("traversal_check", &traversal_check_entries_,
                   &selected_traversal_check_idx_);
    add_line("default_exit_save_path",
             &original_config_.default_exit_save_path);
    add_line("default_timer_log_path",
             &original_config_.default_timer_log_path);
    ...
    add_radio_line("config_save_behavior", &config_save_entries_,
                   &selected_config_save_idx_);
    add_radio_line("editor_save_behavior", &editor_save_entries_,
                   &selected_editor_save_idx_);
    add_radio_line("default_print_mode", &print_mode_entries_,
                   &selected_print_mode_idx_);
    add_radio_line("default_ops_list_mode", &ops_list_mode_entries_,
                   &selected_ops_list_mode_idx_);
    add_radio_line("ops_plugin_path_mode", &path_mode_entries_,
                   &selected_path_mode_idx_);
    ...
    if (cmd == "a" || cmd == "apply") {
      ...
      if (new_path != old_path) {
        if (fs::exists(new_path)) {
          ...
          RebuildLineView();
          status_message_ =
              "Applied changes and set active config to: " + new_path;
        }
      } else {
      ...
      if (!original_config_.loaded_config_path.empty()) {
        ...
        status_message_ =
            "Error: No config file loaded. Use 'apply' to set a path first.";
      }
  ...
  std::vector<std::string> ops_list_mode_entries_ = {"all", "builtin",
                                                     "plugins"};
  std::vector<std::string> path_mode_entries_ = {"name_only", "relative_path",
                                                 "absolute_path"};
  std::vector<std::string> cache_clear_entries_ = {"md", "d", "m"};
  std::vector<std::string> exit_sync_entries_ = {"true", "false"};
  std::vector<std::string> config_save_entries_ = {"current", "default", "ask",
                                                   "none"};
  std::vector<std::string> editor_save_entries_ = {"ask", "auto_save_on_apply",
                                                   "manual"};
  ...
  std::vector<std::string> traversal_tree_entries_ = {"full", "simplified",
                                                      "no_tree"};
...
bool run_config_editor(CliConfig& config) {
  ...
  if (config.config_save_behavior == "ask") {
    if (ask_yesno("Save configuration changes to a file?", true)) {
      ...
      if (!path.empty()) {
        if (write_config_to_file(config, path)) {
          ...
          std::cout << "Error: Failed to save configuration to " << path
                    << std::endl;
        }
      }
    }
    ...
    if (config.loaded_config_path.empty()) {
      std::cout << "Config save behavior is 'current', but no config file was "
                   "loaded. Saving to default 'config.yaml'."
                << std::endl;
      if (write_config_to_file(config, "config.yaml")) {
        config.loaded_config_path = fs::absolute("config.yaml").string();
        std::cout << "Configuration saved to " << config.loaded_config_path
                  << std::endl;
      }
    } else {
