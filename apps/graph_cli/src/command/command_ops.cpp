// FILE: apps/graph_cli/src/command/command_ops.cpp
#include <algorithm>
#include <filesystem>
#include <iostream>
#include <map>
...
bool handle_ops(std::istringstream& iss, ps::Host& svc,
  ...
  } else if (mode_arg == "plugins" || mode_arg == "custom" || mode_arg == "p" ||
             mode_arg == "c") {
    display_mode = "plugins";
    display_title = "plugins";
  } else {
    std::cout << "Error: Invalid mode for 'ops'. Use: all (a), builtin (b), or "
                 "plugins (p/c)."
              << std::endl;
    return true;
  }
  ...
  for (const auto& pair : op_sources.value) {
    ...
    if ((display_mode == "builtin" && !is_builtin) ||
        (display_mode == "plugins" && is_builtin))
      continue;
  ...
  if (op_count == 0) {
    if (display_mode == "plugins")
      std::cout << "No plugin operations are registered." << std::endl;
    else
      std::cout << "No operations are registered." << std::endl;
    return true;
  }
