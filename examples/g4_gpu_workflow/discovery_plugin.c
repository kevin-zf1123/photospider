...
static const char discovery_source[] =
    PS_FRAGMENT_ATLAS_MSL_V8 PS_GPU_DISCOVERY_MSL_V8
...
static const char value_source[] = PS_FRAGMENT_ATLAS_MSL_V8
...
static int validate(const ps_dependency_metadata_query_v8* q, void* user) {
  ...
  return q->input_count == 2 && q->inputs[0].rank == 1 &&
                 ...
                 q->inputs[1].element_type == PS_OPERATION_ELEMENT_INT64_V8
...
static int discover(const ps_dependency_discovery_services_v8* host,
  ...
  const ps_gpu_buffer_binding_v8 buffers[] = {
      ...
      {sizeof(ps_gpu_buffer_binding_v8), 2, token, 0, size, 1}};
...
static int needs(const ps_dependency_query_v8* q,
                 const ps_dependency_services_v8* host, uint32_t port,
                 uint64_t first, uint32_t count) {
  ps_dependency_run_v8 runs[2] = {{0}};
...
static int finish(const ps_dependency_query_v8* q,
                  const ps_dependency_services_v8* host) {
  if (host->fragment_count(host->context, 0) != 2)
    return PS_OPERATION_RESULT_FAILURE_V8;
  uint64_t coordinates[2] = {0};
  for (uint32_t i = 0; i < 2; ++i) {
    ps_dependency_fragment_v8 fragment = {0};
  ...
  if (q->backend == 2) {
    ...
    const ps_gpu_buffer_binding_v8 buffers[] = {
        ...
        {sizeof(ps_gpu_buffer_binding_v8), 1, atlas.directory_token, 0,
         atlas.directory_byte_size, 0},
        {sizeof(ps_gpu_buffer_binding_v8), 2, token, 0, 8, 1}};
...
static int poll(const ps_dependency_query_v8* q, void* bytes,
  ...
  if (s->stage == 2) {
    ...
    if (!host->read(host->context, 1, &s->coordinate, 1, &base, 8))
      return PS_OPERATION_RESULT_FAILURE_V8;
    if (base < 0 || base >= 4096)
      return PS_DEPENDENCY_INVALID_ARGUMENT_V8;
...
static const ps_operation_parameter_descriptor_v8 parameters[] = {
    {sizeof(ps_operation_parameter_descriptor_v8), "capacity", 8,
     PS_OPERATION_PARAMETER_INT64_V8, 1, 1, 1, 65536},
    {sizeof(ps_operation_parameter_descriptor_v8), "mode", 4,
     PS_OPERATION_PARAMETER_INT64_V8, 1, 1, 0, 1}};
...
static const ps_operation_descriptor_v8 operations[] = {
    {.struct_size = sizeof(ps_operation_descriptor_v8),
     ...
     .key_size = sizeof("example.c_discovery") - 1,
     .input_count = 2,
