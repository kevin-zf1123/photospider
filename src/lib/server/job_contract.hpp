 ...
 * @brief Declares the source-private Issue #98 Job and artifact value model.
 */
#pragma once
 ...
 * path separator, or contains a byte outside the accepted opaque-token set.
...
class OpaqueTextId final {
 public:
  /** @brief Creates an invalid empty sentinel. @throws Nothing. */
 ...
 private:
   ...
   * @return True only for one bounded ASCII token without path separators.
   * @throws Nothing.
   */
  static bool valid_identity_text(std::string_view text) noexcept {
    if (text.empty() || text.size() > kMaximumOpaqueIdentityBytes) {
      return false;
    }
...
/** @brief One non-reused attempt within the current process lifetime. */
...
/** @brief One fresh in-process Issue #98 worker instance identity. */
...
/** @brief Tag for process-lifetime committed artifact identities. */
...
/** @brief Tag for one process-lifetime output commit identity. */
...
/**
 * @brief Closed execution profile supported by the Issue #98 vertical.
 * @throws Nothing for value operations.
 */
...
enum class ArtifactDurability : std::uint8_t {
  /** @brief Immutable lookup only until the owning service is destroyed. */
  ProcessLifetime,
};
...
/**
 * @brief Immutable supported Issue #98 Job specification.
 *
 * Construction validates every field, derives exact canonical bytes, and
 * records their SHA-256 digest. Canonical `jobspec-v1` bytes contain six
 * decimal-length-framed textual fields; integer fields use canonical decimal
 * spellings before framing. The type intentionally has no path, session,
 * descriptor, pointer, native handle, credential, plugin, or IPC-id field.
 *
 * @throws std::invalid_argument for an empty identity, negative target,
 * invalid enum representation, or zero maximum parallelism.
...
class JobSpec final {
 public:
   ...
   * @param maximum_parallelism Positive Host Run callback cap.
  ...
  JobSpec(GraphArtifactId graph_artifact_id, int target_node,
          ...
          ArtifactDurability requested_durability =
              ArtifactDurability::ProcessLifetime);

  /** @brief Returns immutable graph identity. @throws Nothing. */
  const GraphArtifactId& graph_artifact_id() const noexcept {
    return graph_artifact_id_;
  }

  /** @brief Returns nonnegative graph node selector. @throws Nothing. */
  int target_node() const noexcept { return target_node_; }

  /** @brief Returns the one declared required output slot. @throws Nothing. */
  const OutputSlotId& output_slot_id() const noexcept {
    return output_slot_id_;
  }

  /** @brief Returns the positive Host Run callback cap. @throws Nothing. */
  std::uint32_t maximum_parallelism() const noexcept {
    return maximum_parallelism_;
  }

  /** @brief Returns the closed execution profile. @throws Nothing. */
  JobExecutionProfile execution_profile() const noexcept {
    return execution_profile_;
  }

  /** @brief Returns the requested artifact durability. @throws Nothing. */
  ArtifactDurability requested_durability() const noexcept {
    return requested_durability_;
  }

  /**
   * @brief Returns the frozen canonical `jobspec-v1` bytes.
 ...
 private:
  ...
  /** @brief Positive per-Run maximum callback count. */
  std::uint32_t maximum_parallelism_ = 0U;
  ...
  ArtifactDurability requested_durability_ =
      ArtifactDurability::ProcessLifetime;
 ...
 * allocate.
 * @note This receipt claims only process-lifetime durability.
 */
struct OutputCommitReceipt final {
  ...
  /** @brief Exact achieved durability, currently process lifetime only. */
  ArtifactDurability achieved_durability = ArtifactDurability::ProcessLifetime;
};

/**
 * @brief Read-only process-lifetime artifact record returned by lookup.
