...
class Device final : public std::enable_shared_from_this<Device> {
 public:
  ...
  Status execute(const std::vector<BufferView>& views,
                 const ps_gpu_dispatch_v6* commands, std::uint32_t count,
...
class Invocation final {
 public:
  ...
  const ps_gpu_service_v6* service() const noexcept { return &service_; }
 ...
 private:
  ...
  static int execute(void* context, const ps_gpu_dispatch_v6* commands,
                     std::uint32_t count) noexcept;
  int fail(Status status) noexcept;
  std::shared_ptr<Device> device_;
  CancellationToken cancellation_;
  ps_gpu_service_v6 service_{};
