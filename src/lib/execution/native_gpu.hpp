...
class Invocation final {
 public:
  ...
  const ps_gpu_service_v7* service() const noexcept { return &service_; }
 ...
 private:
  ...
  static int execute(void* context, const ps_gpu_dispatch_v7* commands,
                     std::uint32_t count) noexcept;
  int fail(Status status) noexcept;
  std::shared_ptr<Device> device_;
  CancellationToken cancellation_;
  ps_gpu_service_v7 service_{};
