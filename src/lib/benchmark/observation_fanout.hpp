 ...
 * subsequent product callback publishes to the workload-specific mirror and
 * then enters the sequence authority with that exact coordinate. The final
 * authority return is the completion barrier proving that the mirror's source
 * history is already published, without inventing a second causal clock.
 *
 ...
 * observation-only on every callback. Reservation and abort remain
 * authority-only. This adapter owns no Run, task, Value, cancellation, queue,
 * resource, or scheduler authority.
 */
class ComputeRunObservationFanout final
 ...
 public:
   ...
   * and whose product callback return closes the shared completion frontier.
   * @param mirror Workload-specific sink publishing each identical coordinate
   * before the sequence authority receives that product callback.
   ...
   * @note Only the sequence authority receives this lifecycle call.
   */
   ...
   * @note Publishes to the mirror before the sequence authority.
   */
   ...
   * @note Copies the immutable Value to the mirror before moving the retained
   * handle into the sequence authority, preserving one revision on both sides.
   */
 ...
 private:
  /**
   * @brief Sole causal-coordinate allocator and final completion recipient.
   */
  std::shared_ptr<compute::ComputeRunObservationSink> sequence_authority_;

  /** @brief Source-history recipient published before completion closes. */
  std::shared_ptr<compute::ComputeRunObservationSink> mirror_;
};

/**
 * @brief Creates one shared same-coordinate observation fanout.
 * @param sequence_authority Sink defining the shared causal coordinate and
 * receiving each product callback after mirror publication.
 * @param mirror Workload-specific observer published before the authority.
