...
struct MtlTextureHolder {
  ...
  ~MtlTextureHolder() {
    // Under ARC, objects created with +new already autoreleased; explicit
    // release for safety only If compiled without ARC, this still safely
...
static void ensure_supported_format_or_throw(const ImageBuffer& buf) {
  ...
  if (!(buf.channels == 1 || buf.channels == 4)) {
    throw std::runtime_error(
        "upload_to_metal: only 1 or 4 channels supported for now");
  }
}

ImageBuffer upload_to_metal(const ImageBuffer& cpu_buffer, void* device) {
  if (!device)
    throw std::runtime_error("upload_to_metal: Metal device is null");
  ...
  MTLPixelFormat fmt =
      (channels == 1) ? MTLPixelFormatR32Float : MTLPixelFormatRGBA32Float;
  ...
  id<MTLTexture> texture =
      [(id<MTLDevice>)device newTextureWithDescriptor:desc];
  if (!texture)
    throw std::runtime_error("upload_to_metal: failed to create MTLTexture");
  ...
  if (src_bpr == expected_bpr) {
    ...
    [texture replaceRegion:region
               mipmapLevel:0
                 withBytes:cpu_buffer.data.get()
               bytesPerRow:expected_bpr];
  } else {
    // Pack row-by-row into a contiguous vector
    std::vector<unsigned char> tmp(height * expected_bpr);
    const unsigned char* src =
        static_cast<const unsigned char*>(cpu_buffer.data.get());
    for (int y = 0; y < height; ++y) {
      std::memcpy(&tmp[size_t(y) * expected_bpr], src + size_t(y) * src_bpr,
                  expected_bpr);
    }
    MTLRegion region = MTLRegionMake2D(0, 0, width, height);
    [texture replaceRegion:region
  ...
  out.context.reset(new MtlTextureHolder(texture),
                    [](void* p) { delete static_cast<MtlTextureHolder*>(p); });
  return out;
}
...
ImageBuffer download_from_metal(const ImageBuffer& gpu_buffer) {
  ...
  if (!gpu_buffer.context)
    throw std::runtime_error("download_from_metal: missing Metal context");
  auto holder = static_cast<MtlTextureHolder*>(gpu_buffer.context.get());
  id<MTLTexture> texture = holder->tex;
  if (!texture)
    throw std::runtime_error("download_from_metal: MTLTexture is null");
  ...
  int channels = 0;
  if (fmt == MTLPixelFormatR32Float)
    channels = 1;
  else if (fmt == MTLPixelFormatRGBA32Float)
    channels = 4;
  else
    throw std::runtime_error("download_from_metal: unsupported texture format");
  ...
  out.data.reset(mem.get(),
                 [mem](void*) mutable {  // NOLINT(readability/casting)
                   mem.reset();
                 });
  return out;
}
...
ImageBuffer upload_to_metal(const ImageBuffer&, id) {
  throw std::runtime_error(
      "upload_to_metal: Metal not supported on this platform");
}
ImageBuffer download_from_metal(const ImageBuffer&) {
  throw std::runtime_error(
      "download_from_metal: Metal not supported on this platform");
}
