  ...
  if (fs::exists(config_path)) {
    ...
    try {
      ...
      if (root["cache_root_dir"])
        config.cache_root_dir = root["cache_root_dir"].as<std::string>();
      if (root["cache_precision"])
        config.cache_precision = root["cache_precision"].as<std::string>();
      if (root["history_size"])
        config.history_size = root["history_size"].as<int>();
      ...
      if (root["default_print_mode"])
        config.default_print_mode =
            root["default_print_mode"].as<std::string>();
      // Backward-compat: map legacy values
      if (config.default_print_mode == "detailed" ||
          config.default_print_mode == "d")
        config.default_print_mode = "full";
      if (config.default_print_mode == "s")
        config.default_print_mode = "simplified";
      if (root["default_traversal_arg"])
        config.default_traversal_arg =
      ...
      std::cerr << "Warning: Could not parse config file '" << config_path
                << "'. Using default settings. Error: " << e.what()
                << std::endl;
    }
