 ...
 *       · RebuildSessionList()：根据 configs_ 重建左侧会话列表，支持启用/禁用切换
 *       · RebuildDetailsPane()：根据当前选中会话渲染右侧详情面板，支持自动生成与手动 YAML 模式
 *       · SyncStatisticsModel()：同步统计复选框状态到当前会话配置
 ...
 *
 * 支持配置项：
 *   - Generator Config：input_op_type、main_op_type、output_op_type、width、height、chain_length、num_outputs
 ...
 *
 * 该函数首先检查输入字符串是否为空，若为空返回 "none"。否则提取冒号 (':') 之后的子串作为简单名称，
 * 并对特定的操作名称进行映射：
 *   - "gaussian_blur"    -> "gblur"
 *   - "constant"         -> "const"
 *   - "perlin_noise"     -> "perlin"
...
static std::string GenerateSessionName(const BenchmarkSessionConfig& cfg, const std::string& benchmark_dir_name) {
    ...
    return benchmark_dir_name + "-" + input_op_sanitized + "-" + main_op_sanitized + "-" + output_op_sanitized + "-" + resolution + "-" + chain;
}
 ...
 *    - 若为 true，则将 generator_config 中的 input_op_type、main_op_type、output_op_type、
 *      width、height、chain_length、num_outputs 等字段写入子节点 "config"
 *    - 若为 false，则将 yaml_path 字段写入节点
 *  - execution：执行配置，包括 runs（运行次数）、threads（线程数）、parallel（并行标志）
 *  - statistics（可选）：统计数据列表，若非空则一并写入
 *
 * @param path    输出文件的路径（fs::path）
 * @param configs 要保存的基准会话配置向量（std::vector<BenchmarkSessionConfig>）
 */
 ...
 *
 * 成员函数：
 *  - BenchmarkConfigEditor(ScreenInteractive&, ps::InteractionService&, const std::string&)
 *    · 构造函数，初始化界面与服务引用，加载配置文件，构建初始会话列表
 *    · @param screen 终端交互屏幕实例
 *    · @param svc    基准测试交互服务，用于获取可用操作类型
 ...
 *  - 输入框、下拉框、单选框、复选框等 UI 状态变量（如 input_op_selected_, width_input_str_ 等）
    ...
    void Run() {
        ...
        main_container |= CatchEvent([this](Event event) {
            if (event == Event::Character('q') || event == Event::Special({"C-c"})) {
                screen_.Exit(); return true;
            }
            if (event == Event::Character('s') || event == Event::Special({"C-s"})) {
                SaveConfig(); return true;
            }
            if (event == Event::Character('a')) {
                AddNewSession(); return true;
            }
            if (event == Event::Character('d')) {
                DeleteSelectedSession(); return true;
            }
            return false;
        });
...
private:
    // ... (RebuildSessionList, SyncStatisticsModel, AddNewSession, DeleteSelectedSession, SaveConfig, RenderStatusBar 等函数无需修改) ...
    void RebuildSessionList() {
        ...
        if (selected_session_ < 0) { selected_session_ = 0; }
        MenuOption option;
    ...
    void RebuildDetailsPane() {
        ...
        runs_opt.on_change = [this]{ try { configs_[selected_session_].execution.runs = std::stoi(runs_input_str_); } catch(...) {} };
        ...
        threads_opt.on_change = [this]{ try { configs_[selected_session_].execution.threads = std::stoi(threads_input_str_); } catch(...) {} };
        ...
        auto details_container = Container::Vertical({
            auto_gen_radio, input_op_dropdown, main_op_dropdown, output_op_dropdown,
            width_input, height_input, chain_input, outputs_input,
            manual_yaml_input,
            runs_input, threads_input, parallel_checkbox, // <-- 新增
