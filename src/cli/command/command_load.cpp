...
bool handle_load(std::istringstream& iss,
    ...
    if (args.size() == 1) {
        ...
        if (!ok) { std::cout << "Error: failed to load session '" << name << "'.\n"; return true; }
        if (config.switch_after_load) current_graph = *ok;
    ...
    if (args.size() >= 2) {
        ...
        auto ok = svc.cmd_load_graph(name, "sessions", yaml_path, config.loaded_config_path);
        if (!ok) { std::cout << "Error: failed to load session '" << name << "' from '" << yaml_path << "'.\n"; return true; }
        if (config.switch_after_load) current_graph = *ok;
        config.loaded_config_path = (ps::fs::absolute(ps::fs::path("sessions")/name/"config.yaml")).string();
