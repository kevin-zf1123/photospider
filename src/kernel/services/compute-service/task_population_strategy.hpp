 ...
 * populate_tasks(): graph-backed full expansion, graphless node-only fallback,
 * and dirty-snapshot-only materialization. It also routes HP/RT implementation
 * checks through a domain shape strategy so task creation remains
 * single-domain.
 *
...
class TaskPopulationStrategy {
 public:
   ...
   * @param snapshot Optional dirty snapshot used for dirty metadata and
   * snapshot-only task shape.
   ...
   * @note When graph is null and snapshot is present, dirty monolithic/tile
   * entries define the explicit task shape before node fallback tasks are
   * appended for untouched planned work.
   */
