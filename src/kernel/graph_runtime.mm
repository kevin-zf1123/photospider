...
void GraphRuntime::start() {
  ...
  // Reset scheduler counters to a known baseline in case of prior stop/start
  // cycles
...
void GraphRuntime::stop() {
  ...
  // Drain any pending global tasks to avoid dangling function targets after
  // shutdown
  {
    ...
    if (found_task) {
      ...
      try {
        if (scheduled) {
          ...
          // Unexpected empty task – surface as exception so the scheduler
          // unwinds safely
          set_exception(std::make_exception_ptr(
              std::runtime_error("GraphRuntime: empty task invoked")));
        }
      } catch (...) {
      ...
      cv_task_available_.wait(lock, [&] {
        return ready_task_count_.load(std::memory_order_acquire) > 0 ||
               !running_ || has_exception_.load(std::memory_order_acquire);
      });
...
void GraphRuntime::submit_initial_tasks(std::vector<Task>&& tasks,
  ...
  if (num_threads == 0 || tasks.empty()) {
    ...
    // total_task_count was set by caller. Unblock waiters and surface error
    // upstream.
...
void GraphRuntime::wait_for_completion() {
  {
    ...
    cv_completion_.wait(lock, [&] {
      return tasks_to_complete_.load(std::memory_order_acquire) == 0 ||
             has_exception_.load(std::memory_order_acquire) || !running_;
    });
  }

  if (has_exception_.load(std::memory_order_relaxed)) {
    // Capture and clear the exception so the runtime can continue serving
    // future tasks.
    std::exception_ptr e;
    {
