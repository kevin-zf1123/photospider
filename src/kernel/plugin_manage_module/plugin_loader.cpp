...
PluginLoadResult load_plugins(const std::vector<std::string>& plugin_dir_paths,
  ...
  auto iter_and_load = [&](const fs::path& base_dir, bool recursive) {
    if (!fs::exists(base_dir) || !fs::is_directory(base_dir))
      return;
    // Kernel layer: avoid terminal output; frontends can report scanning if
    // needed.
    ...
    auto process_path = [&](const fs::path& path) {
      ...
      if (path.extension() != extension)
        return;  // skip non-shared libraries
      ...
      if (!handle) {
        result.errors.push_back(
            {fs::absolute(path).string(), GraphErrc::Io, "LoadLibrary failed"});
        return;
      }
      using RegisterFunc = void (*)();
      RegisterFunc register_func =
          (RegisterFunc)GetProcAddress(handle, "register_photospider_ops");
      if (!register_func) {
        result.errors.push_back({fs::absolute(path).string(),
                                 GraphErrc::InvalidParameter,
                                 "Missing register_photospider_ops"});
        FreeLibrary(handle);
        return;
      }
#else
      ...
      if (!handle) {
        ...
        result.errors.push_back({fs::absolute(path).string(), GraphErrc::Io,
                                 e ? e : "dlopen failed"});
        return;
      }
      void (*register_func)();
      *(void**)(&register_func) = dlsym(handle, "register_photospider_ops");
      ...
      if (dlsym_error) {
        result.errors.push_back({fs::absolute(path).string(),
                                 GraphErrc::InvalidParameter, dlsym_error});
        dlclose(handle);
        return;
      }
#endif
      ...
      try {
        ...
        result.errors.push_back(
            {fs::absolute(path).string(), GraphErrc::Unknown, e.what()});
#ifdef _WIN32
        FreeLibrary(handle);
#else
        dlclose(handle);
  ...
  for (const auto& raw_path : plugin_dir_paths) {
    ...
    } else if (path_str.size() >= 2 &&
               path_str.substr(path_str.size() - 2) == "/*") {
      recursive = false;
