...
int PluginManager::unload_by_plugin_path(const std::string& absolute_plugin_path) {
    std::vector<std::string> to_remove;
    for (const auto& [key, src] : op_sources_) if (src == absolute_plugin_path) to_remove.push_back(key);
    if (to_remove.empty()) return 0;

    // Attempt to unregister from the live registry (best-effort; see comments above)
    ...
    for (const auto& k : to_remove) op_sources_.erase(k);
    return removed;
}

int PluginManager::unload_all_plugins() {
    std::vector<std::string> plugin_keys;
    for (const auto& [key, src] : op_sources_) if (src != "built-in") plugin_keys.push_back(key);
    if (plugin_keys.empty()) return 0;
    int removed = RegistryEditor::unregister_keys(plugin_keys);
    for (const auto& k : plugin_keys) op_sources_.erase(k);
    return removed;
}
