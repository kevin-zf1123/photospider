 ...
 *
 * 此函数遍历所提供 vector 中的每个 ImageInput 对象，并为每个对象创建相应的 YAML 节点，节点包含以下键：
 * - "from_node_id"：设置为 ImageInput 对象的 from_node_id 值。
 * - "from_output_name"：如果 ImageInput 对象的 from_output_name 不为 "image"，则设置为该值。
 *
 ...
 *
 * 本函数处理一组 ParameterInput 对象，并构造一个包含这些数据的 YAML::Node，从而使其适合 YAML 序列化。
 *
 ...
 *
 * 此函数遍历传入的 OutputPort 向量，为每个对象创建一个 YAML 节点，并设置 "output_id" 和 "output_type" 字段。
 * 如果 output_parameters 字段有效，也会将其添加到对应的 YAML 节点中。
 *
 ...
 * 它创建一个 YAML 节点，并根据 CacheEntry 的属性设置键 "cache_type" 和 "location"。
 * 所有这些节点会被聚合到一个 YAML 序列节点中并返回。
 *
 ...
 *   其中 'from_output_name' 和 'to_parameter_name' 均不能为空；否则抛出 GraphError 异常。
 ...
 *
 * 如果任何必需的参数输入字段缺失，则该函数会抛出带有 GraphErrc::InvalidParameter 的 GraphError 异常。
 *
        ...
        for (const auto& it : n["parameter_inputs"]) {
            ...
            if (p.from_output_name.empty() || p.to_parameter_name.empty()) {
                throw GraphError(
                    ...
                    "Parameter input for node " + std::to_string(node.id) + " is missing required fields."
                );
            }
 ...
 * - "parameters": 如果 parameters 字段为非空映射，则直接赋值；否则创建一个空的 YAML 映射。
