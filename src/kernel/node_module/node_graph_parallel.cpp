...
// } // namespace ps
// in: src/kernel/node_module/node_graph_parallel.cpp (REPLACE WITH THIS)
...
/**
 * @brief 并行计算的入口点（当前版本回退到顺序执行以保证正确性）。
 * 
 * @details 
 * 这是一个占位实现。在当前的重构阶段，为了确保所有分块计算逻辑
 * (Tiled Ops) 能够被正确处理，我们将并行计算请求直接转发到我们已经
 * 验证过的、支持分块的顺序计算函数 `NodeGraph::compute`。
 * 
 * 真正的分块并行化将在未来的里程碑中实现，届时将重写此函数，
 * 引入一个能够将 TileTask 分发到线程池的调度器。
 */
NodeOutput& NodeGraph::compute_parallel(int node_id, const std::string& cache_precision,
                                        bool force_recache, bool enable_timing,
                                        bool disable_disk_cache,
